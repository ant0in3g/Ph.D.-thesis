...
\href{http://www.unige.it/}{Università degli Studi di Genova,}\\[1pt]
\href{http://www.dima.unige.it/didattica/dottorato/}{Dottorato di ricerca in Matematica e Applicazioni, ciclo XXVIII}
\end{minipage}
%
%
\vspace*{10pt} \\
...
In this PhD thesis we develop a regularization scheme for time--ordered products in interacting quantum field theories on curved spacetimes. We recall here that time--ordered products of fields are necessary tools to treat interacting theories by means of perturbation methods. In the naive construction of this products, by means of products of Feynman propagators, divergences on diagonals occur and they need to be tamed preserving the good properties of the underlying spacetime.
...
In particular in chapter \ref{p:COV_REG} we extend the scheme proposed in \cite{DFKR_2014} to curved spacetimes. Motivated by \cite{BG_1972} and the form of Feynman propagators on curved spacetimes, we introduce an analytic regularization $\Delta^{(\alpha)}_\fsf$ of a Feynman propagator $\Delta_\fsf$ which can be written as follows using the Hadamard representation
%
\begin{equation*}
...
\addcontentsline{toc}{section}{Aspects of quantum field theory on noncommutative spacetimes}
%----------------------------------------------------------------------------%
...
Noncommutative Geometry introduced by Connes \cite{CONNES_1994} could be a possibility to overcome physical obstructions to the existence of continuous spacetime and commuting coordinates at the Planck scale \cite{DFR_1994}. It gives a new approach to field theories, usually called noncommutative field theories (NCFT), which have been considered long ago within string field theory, see e.g. \cite{WITTEN_1986}. Models have been built for the fuzzy spheres \cite{MADORE_1999}, and also for gauge theories on almost commutative geometries. In particular NCFT on noncommutative Moyal spaces received a lot of attention from the viewpoint of perturbative properties and renormalizability \cite{MVRS_2000}.\par%
...
\end{equation}
%
where the seminorms $p_{\Ksf,r}$ are defined as follows
%
\begin{eqnarray}
...
\end{equation*}
%
with $\phi \in \Ecal\left(\Mcal \right)$. It defines the Fréchet topology on $\Ecal\left(\Mcal \right)$. In particular, a sequence $(\phi_n)$ converges to $\phi$ with respect to this topology if and only if for any compact set $\Ksf \subset U_i$, the sequence $\left(\partial^a\phi_n\right)$ converges uniformly on $\Ksf$ to $\partial^a\phi$.
...
\end{equation*}
%
where in the right hand side the union is taken over all compact set $\Ksf \subset \Mcal$. $\Dcal_\Ksf(\Mcal) \subset \Ecal(\Mcal)$ is the space of all smooth functions supported in $\Ksf$, endowed with the topology induced from $\Ecal(\Mcal)$. On $\Dcal(\Mcal)$ we have the inductive limit topology. It is a locally convex topological vector space but non metrizable, therefore it is not a Fréchet space. 
...
If instead of considering generic locally convex topological vector spaces $\Xsf$ and $\Ysf$ in the previous definition, we take $\Ecal(\Mcal)$ or $\Dcal(\Mcal)$ and $\Cbb$ respectively, then we have a precise definition of the derivative of an observable described by a functional.
...
We recall that $\Mcal$ is chosen to be a globally hyperbolic spacetime (cf. definition \ref{def:cst}), therefore the differential equation \eqref{eq:kg_eq} admits a unique solution once we fix enough initial conditions on a generic Cauchy surface $\Sigma$. Namely, 
%
\begin{equation}
...
The support of $\Delta_{\mathsf{\rsf/\asf}} f$ is in the causal future (respectively past) of the test function $f$. The difference of the two fundamental solutions is called \textbf{causal propagator} of $\Psf$, actually it is defined as follows
%
\begin{equation*}
\Delta := \Delta_\rsf - \Delta_\asf \ , \quad \mbox{thus} \qquad \Psf_x \Delta(x,y) = 0 \ .
\end{equation*}
%
It is a map from the  set of compactly supported  test functions to the one of smooth compactly supported test functions. Its support is the union of the causal future and the causal past of the element applied to it (cf. figure \ref{fig:supp_causal_prop}). 
...
%
\begin{equation*}
\sup_{\Ksf}\abs{\partial^\alpha\phi_j} \to 0 \ ,
\end{equation*}
%
...
\item A first example is the Dirac distribution $\delta \in \Dcal^\prime(\Rbb)$ defined as follows
%
\begin{equation*}
\sm{\delta , \phi} = \phi(0) \ , 
\end{equation*}
...
\end{equation}
%
for $u \in \Scal^\prime(\Rbb^ n)$ and $\phi \in \Scal(\Rbb^n)$. In order to give an adapted version of \eqref{eq:criterion_smoothness_function}  for the distributions, we notice that the Fourier transform of $u \in \Ecal^\prime(\Rbb^n)$ can be equivalently seen as a smooth function in the dual variable $k$ defined as follows
%
\begin{equation}
...
%
Then we write explicitly the $\star_{\Delta_+}$ product as follows
%
\begin{eqnarray}
...
%
In order to define completely the time ordered product for more involved functionals others requirements are needed, we shall present them later on. However, in the case of linear functionals the previous condition is sufficient to define the product also for functionals with overlapping support. We shall now analyze this case in more details.
%
...
\end{eqnarray}
%
Each edge corresponds to a propagator (cf. \eqref{eq:prop_vertices}), every internal vertices to $\lambda$ or $\mu(x)$ (cf. \eqref{eq:prop_vertices}), and any closed loop represent an integration of the propagator(s) in the loop against suitable test function(s). All graphs are a pointwise product of ``vertices'' and ``edges''.
\end{example}
...
\item[Partition.] We shall need a partition of $\Mcal^{n+1}\setminus d_{n+1}$. Let $\Jcal$ be the set of all non empty proper subset $I$ of $\{1,\dots,n+1\}$, and $I^c$ the complement set of $I$ in $\{1,\dots,n+1\}$. We define the subset $\Ccal_I$ of $\Mcal^{n+1}$ as follows
%
\begin{equation*}
...
\end{equation*}
%
which does not depend on the partition of unity chosen, cf. proposition $4.2$ and theorem $4.3$ in \cite{BF_2000}. It is well defined on the full space up to the total diagonal $\Mcal^{n+1} \setminus d_{n+1}$.
...
The scaling degree has the the following properties whose proof can be found in \cite[theorem 5.1]{BF_2000}. Let $u, v \in\Dcal^\prime(\Rbb^n)$ and $a \in \Nbb^n$, then
%
\begin{itemize}
\setlength\itemsep{0pt}
\item $\sd(\partial^a u) \leq \sd(u) + \abs{a}$,
\item $\sd(x^a u) \leq \sd(u) - \abs{a}$,
...
%
\begin{equation*}
\Dcal_{\lambda}(\Rbb^n) \ = \ \left\{ \phi \in \Dcal(\Rbb^n) \ | \ \forall \abs{a} \leq \lambda \ , \ \ \left(\partial^{a}\phi\right)(0)=0 \right\} \ ,
\end{equation*}
%
...
\end{equation}
%
with $\lambda \leq \sd(u)-n$, and the remainder term $r_W(\phi)$ defined as follows
%
\begin{equation*}
...
\end{equation*}
%
for some $\varphi_\gamma \in \Dcal(\Rbb^d)$ depending on $\phi$. Then for any distribution $u\in\Dcal^\prime(\Rbb^n\setminus\{0\})$, with $\sd(u) = \lambda +n$, an extension with same scaling degree, $\exte{u} \in \Dcal(\Rbb^n)$, can be written as follows
% 
\begin{eqnarray}
...
%
\begin{eqnarray*}
&& \sm{\left(x_a \partial^a - \delta \right) u ,\phi} =  0 \ , \\
&& \mbox{for all} \ \ \phi \in \Dcal \ , \ \ \mbox{and} \ \ x_a \partial^a = x_1 \dfrac{\partial^{a_1}}{\partial x_1^{a_1}} \dots x_n \dfrac{\partial^{a_n}}{\partial x_n^{a_n}} \ .
\end{eqnarray*}
%
...
%
\begin{equation}
\sm{\left(x_a \partial^a - \delta \right)^{m+1} u , \phi } =  0
...
%
\begin{equation*}
0 = \left( x_a \partial^a + n + \alpha \right)^{m+1} u \ .
\end{equation*}
%
Using the binomial expansion formula we see that we can restrict ourselves to 
%
\begin{eqnarray}
0 &=&\left(x_a \partial^a + \alpha \right)^{m+1} u \nonumber \\
&=& \alpha^{m+1} u + \sum_{k=1}^{m+1} \binom{m+1}{k} \ \alpha^{m+1-k} \ \left(x_a \partial^a \right)^{k} \circ u \nonumber \\
&=& \alpha^{m+1} u + (-1)^{m+1} \sum_{k=1}^{m+1} \binom{m+1}{k} \alpha^{m+1-k} \ \bigg( \left(\partial^b x_b \right) \circ \left(x_a \partial^a \right)^{k-1} \circ u \bigg) \ .
...
%
\begin{equation*}
\sd\bigg(x_a \circ \left(x_b \partial^b \right)^{k-1} \circ u\bigg) = \sd(u) - 1 = n + \Re(\alpha) - 1 < n \ .
\end{equation*}
%
...
%
\begin{equation*}
u_\ms = - \sum_{k=1}^{n+1} \binom{n+1}{k} \alpha^{-k} x_a \left( \overline{ x^a \circ \left(x_b \partial^b \right)^{k-1} \circ u } \right) \ , 
\end{equation*}
%
...
\end{equation}
%
where $g(x)$ is the the determinant of the metric $g$ expressed in normal coordinates. For $\lambda > 1$ it may happen that $(x_1(\lambda),\dots, x_n(\lambda))$ lies outside $\Ncal_n$ and is thus not well defined in general. In this case we may set $\phi_\lambda = 0$  for points lying outside $\Ncal_n$ by means of some smooth partitions of unit.
...
\end{equation}
%
we say that $\WF(u)$ is \textbf{transversal} to $d_n$. We may now introduce a definition of scaling degree inspired from the results of section 6 in \cite{BF_2000} .


\begin{definition}\label{def:scaling_degree}
Consider a distribution $u \in \Dcal^\prime(\Mcal^n)$ or $u \in \Dcal^\prime(\Mcal^n \setminus d_n)$ with transversal wave front set to $d_n$. The \textbf{scaling degree} of $u$ towards $d_n$ is defined as 
%
\begin{equation*}
...
We recall here the definition of $\mathcal{D}^\prime_\Gamma(\Mcal^n\setminus d_n)$
%
\begin{equation*}
...
%
%
\item We define for each graph $\gamma$ its corresponding degree of divergence as follows
%
\begin{equation}
...
Therefore the time ordered product can be written as follows in order to encode the full combinatorics of the Feynman diagrams,
%
\begin{eqnarray}
...
We can use the notion of scaling degree previously introduced in definition \ref{def:scaling_degree} to give the scaling degree of $\tsf_\gamma$ \eqref{eq:kernel} as follows
%
\begin{equation}
...
The regularized time ordered product of $n$ arguments can be written in the $\MS$ scheme by means of the \textbf{forest formula} as follows
%
\begin{equation}
...
\begin{eqnarray*}
\FtwoGoneHoneF 
&& \mbox{Its regularized integral kernel \eqref{eq:kernel_reg} can be written as follows} \\
...
\end{eqnarray*}
% 
The corresponding regularized integral kernel can be written as follows
%
\begin{equation*}
...
The analytic regularization of the local Hadamard expansion \ref{eq:hadamard_rep} of $\Delta_\fsf$ is defined for $\alpha \in \Omega \subset \Cbb \setminus \{0\}$ as follows 
%
\begin{equation}
...
\begin{proof}
\begin{description}
\item[\ref{item:1_sigma_1}] For every $x \in \Mcal$ we fix a normal coordinate system as follows
%
\begin{equation}
\xi_x : y \to \Rbb^4 
...
\end{equation}
%
in order to parametrize the point $y$ in a normal neighborhood of $x$. Consequently, on $\Ncal_2$ half of the squared geodesic distance, also called the Synge's world function, can be expressed as follows
%
\begin{equation}
...
\end{equation}
%
where $\eta$ is the standard Minkowski metric (cf. definition \ref{def:minkowski}) given in cartesian coordinates. We shall first show that the function $f$ defined as follows
...
We can write $\xi^{a}\xi_{a}$ and $f$, for $\xi \in \Sigma \subset \Cbb^4$, $n \in \Nbb$, and $\alpha \in \Omega \subset \Cbb$, as follows,
%
\begin{eqnarray}
...
\begin{proof}
For $x_1 \in \Mcal$ we fix a normal coordinate system as follows
%
\begin{equation*}
\xi_{x_1} : y \to \Rbb^4 
\end{equation*}
%
in order to parametrize points $y$ in a normal neighborhood of $x_1$. We then write the action of $\rho$ on $\sigma_{23}:=\sigma(x_2,x_3)$ as follows
%
\begin{equation*}
...
\end{equation*}
%
We call \textbf{tangent space} at $x$, denoted by $T_x\Mcal$, the set formed by all possible equivalence classes at $x$. The tangent space to a point of a manifold has the same dimension as the given manifold.
...
There are important particular smooth real vector bundles, the \textbf{tangent bundle} and the \textbf{real line bundle}. 
