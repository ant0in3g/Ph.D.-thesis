...
this is that the wave front set of the integral kernel of $\Delta_\asf$ contains the wave front set the $\delta$ distribution because $\Psf \Delta_\asf = \Ibb$, and then pointwise product of $\Delta_\fsf$ are ill defined on coinciding points. 
...
\left\{
\begin{array}{lcl}
\Fcal(\Mcal)^{\otimes n} & \to & \Fcal(\Mcal) \\
...
%
\begin{equation*}
\Tsf_n(\Fsf_1 \otimes \ ... \ \otimes \Fsf_2)(\phi) \ = \ \Tsf \ \msf_\nsf \circ \left( \Tsf^{-1} \Fsf_1 \otimes \ ... \ \otimes \Tsf^{-1} \Fsf_n \right)(\phi) \ ,
\end{equation*}
%
where
%
...
\end{equation*}
%
We shall now introduce a graphical way of working with this. Let $\Gcal_\usf$ be the set of unoriented grap, with vertices $V(\Gcal_\usf)$ and edges $E(\Gcal_\usf)$. We will consider only the Epstein Glaser subgraphs $\gamma$ of $\Gcal_\usf$ with vertices $V(\gamma)$ and edges $E(\gamma)$.
%
\begin{equation*}
V(\gamma) \ = \ \left\{ 1, ... , n \right\} \ , \quad \mathsf{and} \quad E(\gamma) \ = \ \bigg\{ l_{ij} \ \bigg| \  i \in V(\gamma) , j \in \partial l_{ij} , \ l_{ii} =0 \bigg\}
\end{equation*}
%
with $\partial l_{ij} = \{i,j\}$. We will denote by $\abs{E(\gamma)}$ and $\abs{V(\gamma)}$ respectively the number of elements in the set $E(\gamma)$ and $V(\gamma)$. Therefore the time ordered product can be written as follow
%
\begin{equation*}
\Tcal_n \ = \ \sum_{\gamma \in \Gcal_n} \Tsf_\gamma \ , \qquad \mbox{with} \quad \Tsf_\gamma \ = \ \frac{1}{\mathsf{Sym}(\gamma)} \ \sm{\tsf_\gamma \ , \ \delta_\gamma}
\end{equation*}
%
and
%
\begin{equation*}
\mathsf{Sym}(\gamma) \ = \ \abs{\Esf(\gamma)} ! \ , 
...
We presented in the previous chapter the time ordered product of two functionals. Whereas the $\star$ product is always well defined, $\cdot_\Tsf$ can be ill defined. Therefore we need a procedure to extend it. We shall first introduce tools to characterize precisely the divergence we are dealing with, then present why a new approach to this problem is needed, and finally give our new regularization sheme.
...
Let us illustrate this new notion. 
...
The time ordered product can be expanded in terms of distributions smeared with compactly supported smooth functions which play the role of coupling constants (multiplied by a spacetime--cutoff). At each recursion step the causal factorisation property %eqref{eq:causal-factorisation}
...
%----------------------------------------------------------------------------%
\section{An analytic regularization procedure}
%----------------------------------------------------------------------------%
...
%----------------------------------------------------------------------------%
\subsection{Analytic regularisation of the Feynman propagator on curved spacetimes}
%----------------------------------------------------------------------------%
