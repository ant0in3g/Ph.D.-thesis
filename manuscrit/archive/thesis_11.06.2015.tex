...
\href{http://www.comune.genova.it/}{Genova}, %%TODO DATE
2016 \\[10pt]
%
%
...
%----------------------------------------------------------------------------%
\section*{Quantum field \com{theory} and regularization}
\addcontentsline{toc}{section}{Quantum field \com{theory} and regularization}
%----------------------------------------------------------------------------%

\com{Physical systems with a finite numbers of degrees of freedom like the harmonic oscillator are nowadays very well understood. 
However, in order to describe systems at very high energy like the phenomena occurring in accelerators, infinite degrees of freedom are necessary. 
}
\sbar{The physical sytem most well known is maybe the oscillator harmonic, but it has a finite degree of freedom. However more complicated systems have attracted attentions, in particular systems with an infinite degree of freedom.}
We can naively imagine to \sbar{appropriate} \com{approximate} \sbar{a} such system\com{s} with a large number of \sbar{oscillator} harmonic \com{oscillators} which are interacting.
\com{Furthermore, because of the presence of interaction it is not enough to know the dyncamics of a single element to find the dynamics of the whole model. }
\sbar{Knowing completely the physics of one should permit us to determine the physcis of a large number, but the computation appears to be to be simply too complicated. }
\com{In other words, }those \com{kind of} systems \sbar{could not} \com{cannot}  be treated as usual, the notion of fields \sbar{had} \com{has} to be introduced. 
Fields are objects which associate a value to every point in space and time. One on the most popular example of field is the \com{(classical)} electromagnetic field governed by Maxwell’s equations, \sbar{where} \com{in this case} the theoretical predictions coincide \sbar{wery} \com{very} well with the experimental results. 

\com{In order to describe atomic physics and electromagnetism in a comprehensive framework quantum field theories on Minkowski spacetime 
have been introduced in the past. We recall here that Minkowski spacetime is a four dimensional manifold (three spatial coordinates and one time) equipped with a flat Lorentzian metric. 
}
\sbar{At this point quantum structure was remaining to add in order to be able to describe atomic physics and electromagnetism with\com{in} the same framework.
It is what we call now quantum field theory on Minkowski spacetime. Minkowski spacetime is a four dimensional space where the coordinates denote the three spatial coordinates and the time. It is an euclidean space of dimension four endowed with a metric of Lorentzian signature. 
}
\com{The quantization of free fields i.e. fields which satisfy a linear equation of motion, on Minkowski spacetime is very well understood. However, in order to describe realistic physical models, fields satisfying non linear dynamics have to be studied. Even if there is no rigorous mathematical theory of interacting fields the theory of perturbations developed in the last century, permits to make prediction which are extremely accurate. 
}
\sbar{On this spacetime it is possible to define a vacuum, therefore a perturbation of the theory around this lowest state of energy permit to go beyong the case of free theory and treat interactions. 
}
\com{ In the perturbative treatment of interacting fields the relevant objects are described by power series in the coupling constants, however, three kind of problems arises: 1) the element of this series are plagued by ultraviolet (UV) divergences occurring at very high energy; 2) the element of this series are plagued by infrared (IR) divergences occurring at very low energy; 3) the series usually do not converge in any rigorous way.} 


\com{The first kind of problems are nowadays well understood and a solution to those problems is furnished by the theory of renormalization. Although a complete and generic solution of the second kind of problems is not available, the infrared divergences can be tamed restricting the theory to local portions of the spacetime. For the last point no solution is available, so the elements of the theory are considered to be formal power series in the coupling constants. 
In this work we shall concentrate on the first class of problems. }
\sbar{the infrared (IR) one at very low energy and ultraviolet (UV) at very high energy. This problems are known as the regularization problems. }
In order to treat (UV) divergences on Minkowski spacetime different methods are available, the momentum cuttoff, the Pauli Villar \cite{PV_1949}, the analytic \cite{S_1968}, and the dimensional regularizations. The latest attracted our \sbar{atention} \com{attention}, it has \sbar{benn} \com{been} developed by Bollini and Giambiagi \cite{BGP_2008}, and, 't Hooft and Veltman \cite{HV_1972}.\par%


In the present work we shall \com{study } \sbar{work with} quantum field on curved spacetime, \com{namely we shall analyze a theory} where the spacetime is \com{described by } \sbar{treated as }a Lorentzian manifold with some further assumptions. Quantum field theory on curved background can be view\com{ed} as a first \com{step} to enlarge the theory to general \sbar{realtivity} \com{relaticity} \com{to encompass quantum effects}, or more modestly just to a generalization of theory in Minkowski spacetime. 
\com{In the last decades, perturbative constructions of interacting theories have been extended to the curved case and as expected the same kind of problems present in the flat case remains and some time becomes even more difficult. 
In particular, several}
\sbar{In these situation the perturbative construction of models encounters time--ordered products of field polynomials which are a priori ill--defined due to the appearance of UV divergences. Several}  renormalization schemes which deal with \com{UF divergences} \sbar{these divergences} in the presence of non--trivial spacetime curvature have been discussed in the literature, such as for example local momentum space methods \cite{B_1981}, dimensional regularisation in combination with heat kernel techniques \cite{L_1982,toms_renormalization_1982}, differential renormalisation \cite{comellas_generic_1995,prange_epstein-glaser_1999}, zeta--function renormalisation \cite{bilal_multi-loop_2013}, generic Epstein--Glaser renormalisation \cite{brunetti_microlocal_2000,hollands_local_2001,HW_2005}, and, on cosmological spacetimes, Mellin--Barnes techniques \cite{hollands_correlators_2010} and dimensional regularisation with respect to the comoving spatial coordinates \cite{baacke_coupled_2010}. Some of these schemes, such as heat kernel approaches, zeta--function techniques and local momentum space methods  are based on constructions which are initially only well--defined for spacetimes with Euclidean signature. These constructions can be partly transported to general Lorentzian spacetimes by local Wick--rotation techniques developed in \cite{moretti_proof_2000}. However, whereas the Feynman propagator is essentially unique on Euclidean spacetimes, this is not the case on Lorentzian spacetimes where this propagator has a non--unique contribution depending on the quantum state of the field model. Consequently, the Euclidean renormalisation techniques are able to capture the correct divergent and geometric parts of Feynman amplitudes, but a priori not their non--geometric and state--dependent contributions.
...
In order to have a full understanding of quantum field theory axiomatic formulations have been developed. One of them is the one introduced by Haag and Kastler \cite{HK_1964}, entirely based on local concepts. For this reason it is well suited to approach quantum field theory on curved background. Indeed in \sbar{such a} \com{every} spacetime physical quantities are only known locally.\par%


To treat regularization problem on curved spacetimes with Lorentzian signature Brunetti, Fredenhagen, Hollands, and Wald \cite{brunetti_microlocal_2000,hollands_local_2001,HW_2005} have developed a procedure compatible with the axiomatic approach initiated by Haag and Kastler, which is based on ideas of Steinmann, Epstein, and Glaser \cite{S_1971,EG_1973}. In this approach regularization can be view\com{ed} in some way as extension of distributions. It is the reason why microlocal techniques are important, they  have been introduced to quantum field theory in curved spacetime by the seminal work \cite{R_1996}. It replaces the momentum space methods available in Minkowski spacetime. In particular it uses the notion of wave front set which is well suited to work on curved spacetime owing it does not depend on the coordinates system chosen. The wave front set characterize the singularities in position and momentum spaces of distributions. 


However, although the generalized (Epstein--Glaser) regularization scheme developed in \cite{brunetti_microlocal_2000,hollands_local_2001,HW_2005} is conceptually clear and mathematically rigorous, it is not easily applicable in practical computations. On the other hand, Lorentzian schemes which are better suited for this purpose have not been developed to all orders in perturbation theory \cite{comellas_generic_1995,prange_epstein-glaser_1999}, are tailored to specific spacetimes \cite{hollands_correlators_2010} or are not manifestly covariant \cite{baacke_coupled_2010}. We shall mentione that the the state of the art of the mathematical formulation of regularization of quantum field theory in curve spacetime can be found in \cite{D_2013}.\par%


Motivated by this, we develop a regularization scheme in interacting field theories on curved spacetimes which is directly applicable to spacetimes with Lorentzian signature. This regularization scheme is inspired by the works \cite{keller_dimensional_2010,DFKR_2014} which deal with perturbative QFT in Minkowski spacetime. In these works, the authors introduce an analytic regularisation of the position--space Feynman propagator in Minkowski spacetime which is similar to the one discussed in \cite{BGP_2008}. The scheme shall appear to be manifestly generally covariant, invariant under any spacetime isometries present and constructed to all orders in perturbation theory. Moreover, it captures correctly the non--geometric state--dependent contribution of Feynman amplitudes and it is well--suited for practical computations. In this work, we discuss only scalar fields in four spacetime dimensions, but we shall argue that the renormalisation scheme can be directly generalised to other spacetime dimensions and field theories with higher spin, as well as to theories with local gauge invariance.\par%


Our analysis will take place in the framework of perturbative algebraic quantum field theory (pAQFT) \cite{brunetti_microlocal_2000,hollands_local_2001,HW_2005,brunetti_perturbative_2009,fredenhagen_perturbative_2015,fredenhagen_qft_2014}, which is a conceptually clear framework in which fundamental physical properties of perturbative interacting models on curved spacetimes can be discussed. Fields shall be view as smooth function which maps regions of spacetime to real numbers. Observable which are in some way the action of performing an experiment on a physical system shall be view as functional mapping fiels to complex numbers. In the pertubative approach causality shall be implement by time--ordered products which are constructed recursively by an Epstein--Glaser type procedure and it is shown that this recursion can be resolved by a position--space forest formula similar to the one of Zimmermann used in BPHZ renormalisation in momentum space. In order to extend the scheme proposed in \cite{DFKR_2014} to curved spacetimes, and motivated by \cite{BGP_2008} and by the form of Feynman propagators on curved spacetimes, we introduce an analytic regularisation $\Delta^{(\alpha)}_\fsf$ of a Feynman propagator $\Delta_\fsf$ which can be written as follow using the Hadamard representation
%
\begin{equation*}
\Delta^{(\alpha)}_\fsf := \lim_{\epsilon\to 0^+} \frac{1}{8\pi^2} \left(\frac{\usf}{(\sigma+i\epsilon )^{1+\alpha}} + \frac{v}{\alpha} \left(1-\frac{1}{(\sigma+i\epsilon )^{\alpha}}\right)\right) + w \ ,
\end{equation*}
%
where $\usf$, $v$ and $w$ are the so--called Hadamard coefficients and $\sigma$ is half of the squared geodesic distance. This analytic regularisation, namely the construction of certain distributions by means of powers of the geodesic distance, is reminiscent of the use of Riesz distributions to define advanced and retarded Greens functions on Minkowski spacetime. A careful discussion of Riesz distributions and their extension to the curved case is presented in \cite{BGP_2008}. The regularisation we use is loosely related to dimensional regularisation because the leading singularity of a Feynman propagator in $N$ spacetime dimensions is proportional to $(\sigma+i\epsilon)^{1-N/2}$, see e.g. \cite[Appendix A]{moretti_comments_2003}.  A regularisation of the Feynman propagator similar to the one above has recently been discussed in \cite{D_2015}. In this work, we shall combine the analytic regularisation of the Feynman propagator with the minimal subtraction scheme encoded in a forest formula of the kind discussed in \cite{hollands_correlators_2010,keller_dimensional_2010,DFKR_2014} in order to obtain a time--ordered product which satisfies the causal factorisation property, i.e. a product which is indeed ``time--ordered''. In order to prove that the analytically regularised amplitudes constructed out of $\Delta^{(\alpha)}_\fsf$ have the meromorphic structure necessary for the application of the forest formula and in order to show how the corresponding Laurent series can be computed explicitly, we shall make use of generalised Euler operators.
The practical feasibility of the renormalisation scheme shall be demonstrated by computing a few examples. In quantum field theory on cosmological spacetimes, i.e. Friedmann--Lema\^itre--Robertson--Walker (FLRW) spacetimes, one usually exploits the high symmetry of these spacetimes in order to evaluate analytical expressions in spatial Fourier space. However, the renormalisation scheme discussed in this work operates on quantities such as the geodesic distance and the Hadamard coefficients, whose explicit position space and momentum space forms are not even explicitly known in FLRW spacetimes. Notwithstanding, we shall devote a large part of this work in order to develop simple methods to evaluate quantities renormalised in our scheme on FLRW spacetimes in momentum space, and we shall illustrate these methods by explicit examples.


The present work is organised as follows. In the first chapter we present a the framework of pertubative algebraic quantum field theory, and recall procedures of regularization developed in Minkowski spacetime. Afterwards we introduce the renormalisation scheme, demonstrate that it is well--defined and analyse its properties. Finally in the last chapter we illustrate the scheme by computing examples on generic spacetime, bu also on cosmological spacetimes. 
...
\com{Although the main subject of this PhD thesis is the development of an analytic regularization scheme for interacting quantum field theory on curved spacetime, }
during \sbar{these} \com{the} three years \com{of my PhD course I was also interested in another project} \sbar{i also worked on another} \sbar{pojects}. In \sbar{praticular i} \com{particular I} was interested \sbar{with} \com{in} the noncommutative approach to quantum field theory. These \sbar{works} \com{studies} \sbar{has} \com{have} been done in \sbar{colaboration} \com{collaboration} with Tajron Juri\'c\footnote{Ru\dj er Bo\v{s}kovi\'c Institute, Theoretical Physics Division, Zagreb, Croatia}, Patrizia Vitale\footnote{Dipartimento di Fisica, Universit\`a di Napoli Federico II, Italy}, and Jean-Christophe Wallet\footnote{Laboratoire de Physique Th\'eorique, CNRS and Universit\'e Paris-Sud 11, Orsay, France}. The results have been subject to three articles, \cite{GW_2014}, \cite{GJW_2015}, and \cite{GVW_2014}. Here, I would like to briefly to summarize the result obtained in those papers.


\bigskip


Noncommutative Geometry (NCG) \cite{C_1994} may be a way to overcome physical obstructions to the existence of continuous spacetime and commuting coordinates at the Planck scale \cite{DFR_1994}, triggering a new impulse in the studies on noncommutative field theories (NCFT). Actually, they appeared in their modern formulation a long time ago within String Field Theory \cite{W_1986}. This was followed by models on the fuzzy sphere \cite{M_1995}, gauge theories on almost commutative geometries \cite{DV_1999}. NCFT on noncommutative Moyal spaces received a lot of attention from the end of the 90's, in particular from the viewpoint of perturbative properties and renormalisability \cite{MVRS_2000}.\par%


Jacobi operators appear as kinetic operators of several classes of noncommutative field theories (NCFT) considered recently. In \cite{GW_2014} we deal with the case of bounded Jacobi operators. A set of tools mainly issued from operator and spectral theory is given in a way applicable to the study of NCFT. As an illustration, this is applied to a gauge-fixed version of the induced gauge theory on the Moyal plane expanded around a symmetric vacuum. The characterization of the spectrum of the kinetic operator is given, showing a behavior somewhat similar to a massless theory. An attempt to characterize the noncommutative geometry related to the gauge fixed action is presented. Using a Dirac operator obtained from the kinetic operator, it is shown that one can construct an even, regular, weakly real spectral triple. This spectral triple does not define a noncommutative metric space for the Connes spectral distance.\par%
...
%----------------------------------------------------------------------------%
%\chapter*{Digression}
%\addcontentsline{toc}{chapter}{Digression}
%----------------------------------------------------------------------------%


%During these three years i also worked on another pojects. In praticular i was interested with the noncommutative approach to quantum field theory. These work has been done in colaboration with Jean-Christophe Wallet\footnote{Laboratoire de Physique Th\'eorique, CNRS and Universit\'e Paris-Sud 11, Orsay, France}, Patrizia Vitale\footnote{Dipartimento di Fisica, Universit\`a di Napoli Federico II, Italy}, and Tajron Juri\'c\footnote{Ru\dj er Bo\v{s}kovi\'c Institute, Theoretical Physics Division, Zagreb, Croatia}. The results have been subject to three articles, \cite{GW_2014}, \cite{GJW_2015}, and \cite{GVW_2014}.
...
%\addcontentsline{toc}{section}{Three dimensional noncommutative space}
%----------------------------------------------------------------------------%


%We shall consider a particular case of three dimensional noncommutative space, which has been first introduced in \cite{hammou_coherent_2002} and further considered in various works \cite{gracia-bondia_infinitely_2002,VW_2015,GVW_2014}. This space is in fact the algebra $\Rbb^3_\lambda$ which can be view as \cite{VW_2015,GVW_2014}
%
%\begin{equation*}
...
%\end{equation}
%
%where $\phi^j_{mn}\in\mathbb{C}$, and the family $\{v^j_{mn} \ , \ j\in\frac{\mathbb{N}}{2} \ ,\ -j\le m,n\le j\}$ is the natural orthogonal basis of $\mathbb{R}^3_\lambda$ introduced in \cite{VW_2015}. The following fusion relation and conjugation hold true%
%
%\begin{equation*}
%v^{j_1}_{mn} v^{j_2}_{qp} = \delta^{j_1j_2} \delta_{nq} \ v^{j_1}_{mp} \ , \ \ (v^j_{mn})^\dag=v^j_{nm} \ , \ \ \forall j\in\frac{\mathbb{N}}{2} \ , \ -j\le m,n,q,p\le j \ . 
%\end{equation*}
%
%The orthogonality among the $v^j_{mn}$'s is taken with respect to the usual scalar product $\langle a,b\rangle:=\tr(a^\dag b)$, for any $a,b\in\mathbb{R}^3_\lambda$. Here, the trace functional $\tr$ can be defined \cite{GVW_2014} for any $\Phi,\Psi\in\mathbb{R}^3_\lambda$ as
%
%\begin{equation}
...
%\addcontentsline{toc}{section}{Massless gauge models with quantum instabilities of the vacuum}
%----------------------------------------------------------------------------%


%----------------------------------------------------------------------------%
%\subsection*{Gauge fixed action}
%\addcontentsline{toc}{subsection}{Gauge fixed action}
%----------------------------------------------------------------------------%
...
%\end{equation}
%
%The explicit expression for the propagator can be readily obtained by an  adaptation of the results obtained in \cite{VW_2015} (note the factor $(j_1+1)$ in the denominator due to the weighted trace we have introduced). It can be written as
%\begin{equation}
...
%\addcontentsline{toc}{section}{Perturbatively finite gauge models}
%----------------------------------------------------------------------------%


%----------------------------------------------------------------------------%
%\subsection*{Gauge-fixing}\label{subsection31}
%\addcontentsline{toc}{subsection}{Gauge-fixing}
%----------------------------------------------------------------------------%
...
%\addcontentsline{toc}{subsection}{Finitude of the diagram amplitudes to all orders}
%----------------------------------------------------------------------------%
...
\end{equation}
%
and all multiindices $\alpha$ and $\beta$, where we used the same convention as in \eqref{eq:conv_multiindex}. We call this function space the \textbf{Schwartz space}\index{Schwartz space} and denote it by $\Scal(\Rbb^n)$. It is a Fréchet space with the semi norm defined in \eqref{eq:norm_schwartz}. We can say the Schwartz space is a Fréchet space. 
...
%
%
\item We define the \textbf{singular support}\index{distribution!singular support} of a distribution $u$, $\singsupp(u)$, as the set of points in $\Xsf \subset \Rbb^n$ having no open neighborhood to which the restriction of $u$ is a smooth function (cf. lemma \ref{lem:smooth_criterion_distrib}). 
%
%
%
%
...
In the perturbative construction of models in quantum field theory on curved spacetimes we have to construct time ordered products of field polynomials which are \sbar{à} \com{a} priori ill defined due to the appearance of UV divergences. Several regularization schemes which deal with these divergences in the presence of non trivial spacetime curvature have been discussed in the literature. \com{give some references} The Epstein--Glaser regularization discussed in section \ref{p:REG_PB} is one of them. Although this scheme is conceptually clear and mathematically rigorous, it is not easily applicable in practical computations. 
...
We have seen in section \ref{p:EXT_UP_TOT} that the time ordered product of local functionals can be extended up to the total diagonal via a recursive method. Therefore even if the regularization scheme we shall develop in the present chapter proceed in another way we know that the only relevant divergence is the one on the total diagonal. \com{For this reason, in this section, we shall focus to the extension of generic distributions $u \in \Dcal^\prime(\Mcal^n \setminus d_n )$ over the total diagonal.  } \sbar{ It is the reason we shall focus in this section to regularize generic distribution such as $u \in \Dcal^\prime(\Mcal^n \setminus d_n )$.}


\sbar{Drawing on the path follow} \com{Following the path adopted} in section \ref{p:REG_NUMERRIC} \com{in section \ref{p:EXT_SD}, we shall }  characterize the \com{possible} divergences occurring \com{on $d_n$}. \com{Later on in section \ref{p:EULER} we shall construct an  explicit extension over the diagonal.} \sbar{up to the explicit construction of an extension which shall be done in the following section \ref{p:EULER}}.
...
The distribution $u \in \Dcal^\prime(\Mcal^n \setminus d_n )$ is defined for all test functions supported outside the the total diagonal. We recall that the \textbf{total diagonal} $d_n$ \com{is} \sbar{refers to} the following subspace of $\Mcal^n$
%
\begin{equation}
d_n = \left\{ (x,\dots,x) \subset \Mcal^n \right\} \ .
...
\end{equation*}
%
In order to obtain an extension $\exte{u}$ we need relevant informations about the behavior of \sbar{a such} \com{such a} distribution in the neighborhood of the total diagonal $d_n$. For this reason\com{,} and having in mind the path followed in section \ref{p:REG_NUMERRIC}, we shall introduce a geometric scaling transformation. However\com{,} before \com{doing this, }let us define the neighborhood of the total diagonal.


First we shall show that $\Mcal$ admits a covering of open geodesically convex sets. \com{ In this section if you write lemma or proposition or theorem you should add a proof. If a result is taken from somewhere else you should add a reference..... so write something like:    Actually, we have the following lemma taken from O.Neill????   } 
...
Therefore we consider \com{such a} cover $\Ccal$ of $\Mcal$, and then we define the set
%
\begin{equation}
...
\end{equation*}
%
is affinely parametrized and that $x_i(0) =x_1$ whereas $x_i(1) = x_i$. For all $\lambda \geq 0$ and all $\phi \in \Dcal(\Ncal_{n})$ with $\Ncal_n \subset \Mcal^n$ a normal neighborhood of the total diagonal $d_n$ \eqref{eq:neib_tot_diag}, \com{we shall consider }the \com{following} \textbf{geometric scaling transformation} \sbar{we shall consider is}
%
\begin{equation}
...
\end{equation}
%
where $g(x)$ is the the determinant of the metric $g$ expressed in normal coordinates. For $\lambda > 1$ it may happen that \com{$(x_1(\lambda),\dots, x_n(\lambda))$} lies outside $\Ncal_n$ and is thus not well defined in general. In this case we \com{may} set $\phi_\lambda = 0$  \com{for points lying outside $\Ncal_n$ by means of some smooth partition of unit.} \sbar{which is well defined because $\phi = 0$ outside of $\Ncal_n$.}
...
\end{equation*}
%
where $\usf$ is the Hadamard coefficient introduced in the definition of the two point function of an Hadamard state in section \ref{p:STATES}, and $\usf^2$ is the van Vleck--Morette determinant, see e.g. \cite{poisson_motion_2011} \com{for a derivation of this formula}.

\com{remove space}
We notice that the geometric scaling transformation \eqref{eq:geo_scaling_transfo} is similar to the one defined for numerical distributions in \eqref{eq:geo_scaling_transfo_numeric} \com{the difference here is that the scaling towards the point $x_1$ is done along geodesics.}. 
\com{remove space}


\com{We can now introduce a notion of scaling degree towards submanifolds which generalizes the  Steinmann scaling degree \ref{def:steinmann_scaling_degree}. \com{Let us first of all introduce some notation.  Consider a distribution $u \in \Dcal^\prime(\Mcal^n)$ or $u \in \Dcal^\prime(\Mcal^n \setminus d_n)$, employing the scaling transformations \eqref{eq:geo_scaling_transfo} we might introduce the family of {\bf scaled distributions} 
\[
\sm{u_\lambda,\phi} := \sm{u,\phi_{1/\lambda}}.
\] 
Furthermore, if 
\begin{equation}\label{eq:transversal-wf}
\overline{\WF(u)}\cap \{(x_1,\dots, x_n,k,0,\dots,0)\in T^*\Mcal^n, \forall k\neq 0 \}=\emptyset,   
\end{equation}
 we say that the $\WF(u)$ {\bf is transversal to} $d_n$.}
\com{We may now introduce a definition of scaling degree inspired from the results of section 6 in \cite{BF00} }.}
\begin{definition}\label{def:scaling_degree}
\com{Consider a distribution $u \in \Dcal^\prime(\Mcal^n)$ or $u \in \Dcal^\prime(\Mcal^n \setminus d_n)$ with scaling degree transversal to $d_n$.
The \textbf{scaling degree} of $u$} towards $d_n$ is defined as 
%
\begin{equation*}
\sd(u) \ := \ \inf\bigg\{ \omega \in \Rbb \ \bigg| \ \lim_{\lambda \downarrow 0} \ \lambda^\omega \ u_\lambda \ = \ 0 \bigg\} \ .
\end{equation*}
\com{where the limit is taken in the H\"ormander topology of $\mathcal{D}'_\Gamma(\Mcal^n\setminus d_n)$, and 
...
If a distribution $u \in\Dcal^\prime(\Mcal^n)$ or \com{$u\in\Dcal^\prime(\Mcal^n\setminus d_n)$} has scaling degree towards $d_n$ strictly lower than the total dimension of the scaled coordinates $4(n-1)$, then $u$ possesses a unique extension towards \com{$d_n$} with the same scaling degree. 
\end{corollary}

\com{A proof of this corollary can be obtained along the lines of the proof of Theorem 6.9 in the paper of Brunetti and Fredenhagen \cite{...}.}



In the same way as we did for numerical distributions in section \ref{p:REG_NUMERRIC}, we define the \textbf{analytic regularization} of a distribution \com{$u \in \Dcal^\prime(\Mcal^n\setminus d_n)$} \sbar{. It is} \com{as} a family $\left\{ u^{(\alphabd)}\right\}$ of distributions \com{in $\Dcal^\prime(\Mcal^n)$}  such that
%
\begin{center}
$u^{(\alphabd)}$ is weakly analytic with respect to $\alphabd$ \com{in some domain with $0$ in its boundary}, and $\ \underset{\alphabd\to 0}{\lim} u^{(\alphabd)} = u$ \  \com{on $\Dcal(\Mcal^n\setminus d_n)$} ,
\end{center}
%
...
A result permits to simplify the previous definition. If $u^{(\alphabd)}$ is continuous and analytic in every single variables, where each of them is \com{an} element of the complex plane $\Cbb$, then $u^{(\alphabd)}$ is analytic in $\alphabd$. We are then back to analyticity on $\Cbb$.
...
We consider the complex function \com{$f(\alphabd)$} in $\alphabd$ which is defined in an open subset $\Omega^d \subset \Cbb^d$ and which is continuously differentiable in the underlying real coordinates of $\Cbb^d$ (real and imaginary parts of $\alphabd$). Then $f(\alphabd)$ is analytic in $\Omega^d$ if and only if \sbar{is} it satisfies the system of partial differential equations
%
\begin{equation}
...
Any analytic regularization \com{$\{u^{(\alpha)}\}$} of a distribution $u \in \Dcal^\prime(\Mcal^n\setminus d_n)$ \com{ with $\alpha\in\Omega\subset \Cbb$ provides } \sbar{implies} an extension of $u$ which is defined by 
%
\begin{equation*}
\com{\sm{u_\ms,\phi} = \lim_{\alpha \to 0} \ \bigg( \sm{u^{(\alpha)},\phi} - \pp\sm{u^{(\alpha)},\phi}\bigg) \ ,}
\end{equation*}
%
where $\pp$ is the principal part. \sbar{We require the same scaling degree, $\sd(u_\ms) = \sd(u)$.} The extension $u_\ms$ is called the minimal subtraction of $u$.
\end{theorem}

\com{For a generic analytic regularization \com{$\{u^{(\alphabd)}\}$ where $\alphabd$ is in $\mathbb{C}^n$} we could also find an extension in a similar way as the one introduced in the previous theorem, however this extension could not be unique, actually, the }
\sbar{The} procedure of taking the limit $\alphabd \to 0$ \com{and the extraction of the principal part }in the $\MS$ subtraction can be tricky. It the case of the time ordered product it shall be done with the help of the Forest formula, cf. theorem \ref{theo:renorm_t_prod_ms_forest}.
...
A distribution $u \in \Dcal^\prime(\Mcal^n)$ or \com{$u\in \Dcal^\prime(\Mcal^n\setminus d_n)$}, which satisfies the equality 
%
\begin{equation}
...
\end{equation}
%
under geometric scaling transformations of the form \eqref{eq:geo_scaling_transfo} for all \com{ $\phi\in\Dcal(\Ncal_n\setminus d_n)$} and for a $\delta\in\Cbb$, is called \textbf{homogeneous of degree} $\delta$. 
\end{definition}

\com{According to this definition, }
\sbar{It implies that} a distribution $u$ which is homogeneous of degree $\delta$ \com{and with $\WF{(u)}$ transversal to $d_n$ }has scaling degree $-\Re(\delta)$, indeed for all \com{$\phi\in\Dcal(\Ncal_n\setminus \{x\})$} we have
%
\com{
\begin{equation*}
...
= - \Re(\delta) \ ,
\end{equation*}}
\com{hence, $\sd(u) -\Re(\delta)$ because, for homogeneous distribution with wave front set transversal to $d_n$, the limit $\lambda\to 0$ of $\lambda^\omega u_\lambda$ in the topology of distribution is equivalent the limit in the H\"ormander topology $\mathcal{D}'_\Gamma(\Mcal^n\setminus d_n)$ with $\Gamma$ transversal to $d_n$.}

\com{Finally, for homogeneous distributions towards $d_n$ with wave front set transversal to $d_n$ we have actually the following lemma whose proof can be obtained following the one of   \cite[theorem 3.2.3]{hormander_analysis_1990} due to L. Hörmander.}

\com{
\begin{lemma}\label{lemma:exte_homo}
If \com{$u\in\Dcal(\Mcal^n\setminus d_n)$} has wave front set transversal to $d_n$, is homogeneous of degree $\delta$, and $-(\delta+4(n-1)) \notin \Nbb$, then $u$ possess a unique extension $u \in \Dcal^\prime(\Mcal^n)$ with the same degree of homogeneity.
\end{lemma}
\begin{proof}
The existence of an extension can be obtained in two ways. It is possible to write a proof along the lines of the proof of 
 \cite[theorem 3.2.3]{hormander_analysis_1990} due to L. Hörmander or, thanks to the previous observation about the relation of the scaling degree towards $d_n$ and the degree of homogeneity, we might directly use \cite[theorem 6.9]{brunetti fredenhagen 2000}.
 The uniqueness of the extension descends form the uniqueness of the extension of the homogeneous distributions $u_x$ obtained restricting $u$ to the set $\{ (x,y_2,\dots, y_n)\in \Mcal^n, \forall y_i\in \Mcal\}$.  
\end{proof}
}

Unfortunately the relevant \com{distributions we have }\sbar{objects} to regularize in order to extend the time ordered product are not homogeneous. Nonetheless\com{, } as we mention in section \ref{p:REG_NUMERRIC}\com{, } they are ``almost homogeneous''. 

\com{nospace}
Therefore we consider a family of distributions \com{$u^{(\alpha)} \in \Dcal^\prime(\Ncal_n\setminus d_n)$} defined for $\alpha$ in \com{$\Omega\setminus 0$ for} some neighborhood $\Omega \subset \Cbb$ of $0$ and assume that $u^{(\alpha)}$ \com{have wave front set transversal to $d_n$ and that they} can be expanded as
%
\begin{equation*}
u^{(\alpha)}  = \sum_{k=0}^m u^{(\alpha)}_k + r^{(\alpha)} \ ,
\end{equation*}
%
where $u^{(\alpha)}_k$ are homogeneous terms \com{with transversal wave front set and} with degree \sbar{with degree} $a_k = - \delta_\alpha + k$\footnote{The choice on how to write $a_k$ is arbitrary but independent from the following construction.}, thus 
%
\begin{equation*}
\sd(u^{(\alpha)}_k) = - \Re(a_k)  =\Re\left(\delta_\alpha\right) - k \geq 4(n-1) \ .
\end{equation*}
%
We place ourselves in the worst case where $\sd(u^{(\alpha)}_k) \geq 4(n-1)$, otherwise the extension is straightforward due to corollary \ref{corol:theo_sd_M}. The remainder \com{$r^{(\alpha)}\in \Dcal^\prime(\Ncal_n\setminus d_n)$} has scaling degree \com{towards $d_n$} strictly smaller than $4(n-1)$ and thus according to corollary \ref{corol:theo_sd_M} it can be uniquely extended to \com{$x$} for every $\alpha \in \Omega$.


\bigskip


As we did for numerical distributions when we introduced a notion of ``almost homogeneity'' by theorem \ref{theo:almost_homo_numeric}, here we shall define the \textbf{generalized Euler operator}\index{generalized Euler operator}\index{$\Esf_p$}. We consider a normal neighborhood \com{$\Ncal_n$ } of the total diagonal $d_n$, \com{see e.g.\eqref{eq:neib_tot_diag} } and define the operator $\Esf_p$ as follow
%
\begin{equation}
\Esf_p : \left\{
\begin{array}{lcl}
\com{\Dcal(\Ncal_n)} & \to & \com{\Dcal(\Ncal_n)} \\
...
\end{equation*}
%
possesses a unique extension towards \com{$d_n$} by applying Lemma \ref{lemma:exte_homo}. 
...
\com{Consider $\Ncal_n$, a normal neighborhood of the diagonal $d_n$} and $\Omega \subset \Cbb$ a neighborhood of the origin. Assume that $u^{(\alpha)}\in\Dcal^\prime(\Ncal^n\setminus d_n)$ is an \textbf{analytic regularization} of $u\in\Dcal^\prime(\Ncal^n\setminus d_n)$. Moreover, assume that $u^{(\alpha)}$ can be decomposed as 
%
\begin{equation*}
u^{(\alpha)} = \sum_{k=0}^m u^{(\alpha)}_k + r^{(\alpha)} \ ,
\end{equation*}
...
If  \com{$u^{(\alpha)}_k$ and $r^{(\alpha)}_k$ have wave front set transversal to $d_n$}, then the following statements hold.
%
\begin{enumerate}
...
%
%
\item[\ref{item:3_regularization}] We know that the original distribution $u^{(\alpha)}$ defined on \com{$\Ncal^n\setminus d_n$} is weakly analytic. An explicit construction of the weakly meromorphic extension $\dot{u}^{(\alpha)}$ to \com{$\Ncal^n$} is provided by 
...
\end{equation*}%
%
Indeed if we choose for every component $u^{(\alpha)}_k$ a sufficiently large $p$, then according corollary \ref{corol:theo_sd_M} we shall be able to construct a weakly meromorphic extension $\exte{u}^{(\alpha)}$ to \com{$\Ncal^n$}. Hence, the poles of $\exte{u}^{(\alpha)}$ can only be supported on $d_n$.%
%
%
\item[\ref{item:4_regularization}] Because the pole of $\exte{u}^{(\alpha)}$ in $0$ is supported on \com{$d_n$}, after subtracting the principal part of the distribution the limit $\alpha \to 0$ can be safely taken. The such obtained distribution prior to considering the limit $\alpha \to 0$ coincides with $u^{(\alpha)}$ on \com{$\Ncal^n\setminus d_n$} and the same holds in the limit $\alpha\to 0$. Consequently $u_\ms$ is an extension of $u$.\par%
%
Finally, due to \ref{item:1_regularization} we have by the \com{lemma \ref{lemma:exte_homo}} that the extension of each $u^{(\alpha)}_k$ keeps the degree of homogeneity, and therefore keeps also the scaling degree. By corollary \ref{corol:theo_sd_M} the extension of the remainder $r^\alpha$ conserves the scaling degree. Therefore $\sd(\exte{u}^{(\alpha)})=\sd(u^{(\alpha)})$. We have by definition $\lim_{\alpha\to 0}u^{(\alpha)} = \sd(u)$, thus because the subtraction of the principal part is a linear operation we can conclude $\sd(u_\ms)=\sd(u)$.
%
%
\end{description}
\end{proof}
...
Let \com{$\Ncal_n$} be a normal neighborhood of the total diagonal $d_n$ and $u \in \Dcal^\prime(\Ncal_n)$ with scaling degree $s_1$ towards $d_n$. If there exists an $\delta\in\Cbb$ satisfying $\Re(\alpha)=-s_1$ such that
%
\begin{equation*}
...
\hilight{GIVE A REFERENCE FOR $\Esf_1$} \com{you could avoid it}\\
...
\end{equation*}
%
\hilight{GIVE A REFERENCE THIS RELATION} \com{you could avoid it}\\
we find for $p=1$
%
\begin{eqnarray}
...
The  time ordered product of ``causal'' functionals can be view\com{ed}  as the following map
%
\begin{equation}
\Tcal_n \ : \ 
\left\{
...
\com{We aim to rewrite the time ordered product as a sum of graphs. To this end we start giving some standard definitions.}
\sbar{In order to write the time ordered product as a sum of graphs we should give some first requirements regarding graphs.}
A generic graph $\gamma$ is a set of points, called vertices $V(\gamma)$, which can be connected by some \com{(unoriented)} lines called edges $E(\gamma)$. We shall denote by $\abs{E(\gamma)}$ and $\abs{V(\gamma)}$ the number of elements in the set $E(\gamma)$ and $V(\gamma)$ respectively. 
...
\end{enumerate}
%
In the case of a graph with only two vertices (e.g. $\FtwoG$ or $\FthreeG$) the graphs is divergent for $\abs{V(\gamma)} \geq 2$. Indeed the scaling degree \com{towards the diagonal} of $\Delta_\fsf^r$ \com{defined in definition XXXX} is lower than $4$ only for $r=1$.
\end{itemize}
...
We can use the notion of scaling degree previously \com{introduced in definition XXXX} to give the scaling degree of $\tsf_\gamma$ \eqref{eq:kernel} as follow
%
\begin{equation}
...
\end{equation}
%
Without giving a general proof we can \com{see that this is really the case for} \sbar{understand it by looking at} the triangular graph with one intern\com{al } loop, on Minkowski spacetime. 
%
\com{In particular considering}
\begin{eqnarray*}
...
\end{equation*}
%
Thus \com{assuming for the moment that} \sbar{admitting} the product in the expression of $\tsf_\gamma$ is well defined, its scaling degree is the sum of the scaling degree of each term. Therefore $\sd(\tsf_\gamma)$ is given by \eqref{eq:sd_graph}. 
...
We have just presented a convenient way to represent the product $\cdot_\Tsf$ of ``causal'' \com{local} functionals. However, in the previous expansion and in particular in equation \eqref{eq:kernel},
%
\begin{equation*}
...
In order to complete the construction of \eqref{eq:time_ordered_prod_graph} we need to extend the obtained distributions $\tsf_\gamma$ to all diagonals $D_n$. The extension is not  straightforward because of the singular structure of the Feynman propagator $\Delta_\fsf$. Indeed\com{,} as already explained in section \ref{p:INT_Q_ALG}\com{,} pointwise products of Feynman distribution are ill defined on the coinciding points. Consequently we do need a regularization procedure in order to extend $\tsf_\gamma$ to the full space $\Dcal(\Mcal^n)$. We shall see that this extension is in general not unique, but subject to a so-called regularization freedom.
...
\com{ nospace }
An analytic regularization of the Feynman propagator $\Delta_\fsf$ shall be studied later on. Nonetheless the idea of the $\MS$ scheme is independent of the details of the analytic regularization. Namely, given any analytic regularization $\Delta^{(\alpha)}_\fsf$ of $\Delta_\fsf$ we repeat the formal construction of $\Tsf_n$ in \eqref{eq:time_ordered_prod_graph} by replacing $\Delta_{ij}$ by $\Delta^{(\alpha_{ij})}_{ij}$ in the integral kernels $\tsf^{(\alphabd)}_\gamma$ \com{given in} \eqref{eq:kernel} \com{in particular we obtain} 
%
\com{
\begin{equation}
\tsf^{(\alphabd)}_\gamma = \prod_{(i,j) \in \gamma} {\Delta^{(\alpha_{ij})}_{ij}}^{\abs{\ell_{ij}}} \ ,
\label{eq:kernel_reg}
\end{equation}
}
\com{Antoine, pay attention, we are using a different analytic regularizaiton. It is not simply the power, you have to change also the subsequent formulas.....  }
%
\sbar{with} \com{where} $\alpha_{ij} \in \Omega \subset \Cbb$, and $\alphabd=\{\alpha_{ij}\}_{(i,j) \in \gamma}$ are multivariate complex parameters contained in some neighborhood of the origin. \sbar{For} \com{To} each Feynman distribution $\Delta_{ij}$, corresponding to one line between the vertices $i$ and $j$, we associate a complex parameter $\alpha_{ij}$ in order to be able to treat \com{separately the divergences occurring on partial diagonals.} \sbar{every divergences occurring separately.} In the case of a triangular graph with sub divergences we have the following situation.
%
\com{For the particular graph}
\begin{eqnarray*}
\FnGnHnF
...
\end{eqnarray*}
%
Proceeding in this way \com{for every graph} we can define 
%
\begin{equation}
...
We shall show that the distributions $\tsf^{(\alphabd)}_\gamma$ \com{introduced in} \eqref{eq:kernel_reg} are \textbf{multivariate meromorphic functions} which have poles at the origin for some of the $\alpha_{ij}$, cf. section \ref{p:EXT_SD}. 
\com{nospace}
...
For every subset $I\subset \overline{n}$ we indicate by $\Rsf_I$\index{$\Rsf_I $} the ``\textbf{principal part operator}'' which extracts the principal part of \sbar{of} a multivariate meromorphic function
%
\begin{eqnarray}
...
The regularized time ordered product of $n$ arguments can be written in the $\MS$ scheme \com{by means of the {\bf forest formula}} as follow
%
\begin{equation}
...
\end{equation*}
%
\com{I think you should use $\{1,2\}$ instead of $(12)$ everywhere....}
...
%
%
\item We shall look at a particular term in $\Tcal_3$, \com{namely at }the triangular \com{term/graph} with one subdivergence with \com{vanishing} degree of divergence. \com{ is equal to $\omega_\gamma = 0$}. Therefore the \com{considered} graph \sbar{considered} is logarithmically divergent.
%
\com{In particular, we shall consider}
\begin{eqnarray*}
\FtwoGoneHoneF 
&& \mbox{\sbar{The} Its regularized integral kernel\sbar{s} \eqref{eq:kernel_reg} can be written as follow} \\
&& \tsf_\gamma = \Delta_{12}^{2+\alpha_{12}} \ \Delta_{13} \ \Delta_{23} \ , \\
&& \mbox{where we \com{have introduced }\sbar{implement} a complex parameter $\alpha_{12}$ \sbar{only} for the divergent part \com{only}}
\end{eqnarray*}
%
\sbar{The regularized integral kernels $\tsf^{(\alphabd)}_\gamma$ \com{given in} \eqref{eq:kernel_reg} for this graph can be written as follow}
%
\par
%
In \sbar{this} \com{the} particular case of the triangular graph with one fish graph as subgraph the subsets which correspond to divergent contributions are $(12)$ and $(123)$, therefore the relevant forests are 
%
\begin{equation*}
...
\end{eqnarray*}
%
This expression is only well defined locally because $\sigma$ used in the expression of $\Hsf_\fsf$ \eqref{eq:hadamard_rep} is defined on geodesically convex normal neighborhood. \com{However,} in section \ref{p:EXT_SD} we \com{recalled} that $\Mcal$ admits a covering of open geodesically convex sets. Therefore\com{,} in order to define $\tsf_\gamma$ globally, we may employ suitable partitions of unity. 
...
\item \com{In order to show that $\Hsf^{(\alpha)}_\fsf$ is well defined we } prove in proposition \ref{prop:amplitude_sigma_prop_analyt} that the relevant distributions 
%
\begin{equation*}
...
%
are multivariate analytic functions. The distribution \com{of that form} \sbar{\eqref{eq:amplitude_sigma_reg}} only shows the most singular contribution of \eqref{eq:feynman_reg}. The others contributions are of the same form up to replacing some of the factors $(1+\alpha_{ij})$ in the exponents by $\alpha_{ij}$ or $0$.
...
We first argue\com{d} in proposition \ref{prop:regularization} that an analytically regularized distribution $u^{(\alpha)}\in\Dcal^\prime(\Mcal^n\setminus d_n)$, which can be written as a sum of homogeneous terms with respect to this scaling transformation plus a sufficiently regular remainder, can be uniquely extended to $\Mcal^n$ in a weakly meromorphic way.

In proposition \ref{prop:set} we \com{gave} a sufficient condition for the existence of such a homogeneous expansion and we \com{shall} show in proposition \ref{prop:almost_homo} that the distributions $\Asf_\gamma^{(\alphabd)}$ satisfy this condition.

\end{itemize}


\item The above mentioned results are proved by means of generalized Euler operators which can be written abstractly in terms of a scaling transformation, but also in terms of covariant differential operators whose explicit form can be straightforwardly computed as we argue\com{d} in section \ref{p:EULER}. 

In proposition \ref{prop:expose_poles} we use\com{d} these operators in order to demonstrate how the full relevant pole structure of $\Asf_\gamma^{(\alphabd)}$ can be computed, thus showing the practical feasibility of the $\MS$ scheme. We \com{have found} \sbar{find} that our regularization scheme corresponds in fact to a particular form of differential regularization.
...
\com{you should remove everywhere the numbers of the proofs everywhere!!!  }
\begin{proof}
\begin{description}
...
\end{equation}
%
in order to parametrize \com{the point} $y$ in a normal neighborhood of $x$. Consequently, on $\Ncal_2$ half of the squared geodesic distance, also called the Synge's world function, can be expressed as follow
%
\begin{equation}
...
for $\xi^{a} \in \{z\in \mathbb{C}^4 \,|\,\Im(z) \in V^\pm \}$, where $V^\pm$ is the forward or past light cone \com{(taken without the tip)} with respect to the Minkowski metric, is  analytic both in $\xi$ and $\alpha$.
...
\end{equation}
%
The logarithm \eqref{eq:log_complex} gives a single valued function on the principal branch $(k=0)$ in the complex plane. The principal branch has a discontinuity along the negative real axis ($\theta=\pi$). \sbar{Therefore if $\xi^{a}\xi_{a} \notin \mathbb{R}^0_{-}$, then $f$ is analytic.}
\com{ Therefore $f$ is analytic if $\xi^{a}\xi_{a} \notin \mathbb{R}^0_{-}$. }
%
\begin{eqnarray*}
...
\end{equation*}
which is well defined for $\phi\in\Dcal(\Ncal_2)$, \com{it follows that }that the distribution \eqref{eq:sigma_reg} is homogeneous of degree $-2\alpha$ with respect to transformations of the form \eqref{eq:geo_scaling_transfo} for $\phi \in \Dcal(\Ncal_2\setminus d_2)$.
...
\end{equation*}
%
\com{We} recall the following relation for the wave front set of $\Hsf_\fsf$
%
\begin{equation*}
...
Owing to the form of $\Hsf^{(\alpha)}_\fsf$ given in \eqref{eq:feynman_reg} the relevant distributions which need to be discussed are $\Asf^{(\alphabd)}_\gamma$ introduced in \eqref{eq:amplitude_sigma_reg}. We analyze\sbar{d} \eqref{eq:amplitude_sigma_reg} in the following proposition.
...
\end{equation*}
%
towards the thin diagonal $d_n$ \com{furthermore, thanks to the form of the integral kernel of the distribution we have 
that the wave front set of $\Asf_\gamma^{(\alphabd)}$ is transversal to $d_n$}. When we apply $\Esf^\dagger_1$, introduced in \eqref{eq:euler_operator}, to distribution $\Asf_\gamma^{(\alphabd)}$, we obtain\sbar{ed}
%
\begin{equation}
...
We conclude the general analysis of the regularization scheme introduced in this work by demonstrating that this scheme satisfies (up to one property we shall mention at the end of this section) all axioms of \cite{hollands_local_2001,hollands_existence_2002,hollands_conservation_2005} that, as argued by Hollands and Wald, any physically meaningful scheme to regularize time ordered products should satisfy. These axioms are recalled in section \ref{p:INT_Q_ALG}. In addition to showing these properties of the scheme, we also argue that it preserves invariance under any spacetime isometries present.
...
After having defined a regularization procedure in the previous chapter, we shall perform explicit computations using our regularization scheme, first on generic curved scpacetime as defined in \ref{def:cst}, and then on cosmological spacetime, in particular on spatially flat Friedmann-Lemaître-Robertson-Walker spacetime, that we presented in section \ref{p:FLRW}.
...
We call $g$ a \textbf{riemannian metric}\index{riemannian metric} if for every point of the manifold $g$ has signature $n$, the dimension of $\Mcal$. If the associated scalar product has signature equal to $n-1$ for every point of the spacetime we call $g$ a \textbf{pseudo riemannian}\index{pseudo riemannian metric} (or \textbf{lorentzian}\index{lorentzian metric}) metric. We shall use for now on only lorentzian metric $g$.
...
In every tangent space $T_x\Mcal$ the set of timelike vectors, is called light cone, it consists of two connected components. A time orientation on $\Mcal$ is a choice of one of these connected components. Then the light cone is referred as the union of the forward and backward lightcones, 
%
\begin{equation*}
