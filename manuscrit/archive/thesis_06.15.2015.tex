...
We will now describe the physcis on the spacetime we introduced previously. One starts to define the off shell space of configuration $\Ecal(\Mcal)$ as the set of all smooth real scalar field $\phi \in \Ccal^\infty\left(\Mcal,\Rbb\right)$. For now we do not implement any dynamic. An important subspace of $\Ecal(\Mcal)$ is the space of compactly supported smooth function $\Dcal(\Mcal)$. Physically speaking assigning a space of configuration to regions of the spacetime, is implementing physical properties to this regions.\par% 
%
...
Therefore we need a way to measure these physical properties, it is done by introducing the notion of observable. We will call observable a functional which map the fieds to complex numbers%
%
\begin{equation*}
\Fsf : \left\{
\begin{array}{ccc}
...
\end{equation*}
%
We will work with particular sets of functionals, denoted $\Fcal_\sharp$, which will be defined using regularity and support properties. \par%


\bigskip


We will first look to characterize the regularity of a functional. We would like to choosee $\Fcal_\sharp(\Mcal)$ such that it gives us ``smooth functionals''. Therefore we need a careful definition of differentiability.%
%
\begin{definition}[Functional derivatives] \label{def:func-deriv}
The $n$-th derivative of $\Fsf$ at $\phi\in\Ecal(\Mcal)$ with respect to the directions $\psi_1, \dots, \psi_n \in\Ecal(\Mcal)$ is defined as%
%
\begin{equation*}%
...
\end{equation*}
%
whenever the limit exists. We call it smooth if $\Fsf(\phi)^{(n)}[\psi_1,\dots ,\psi_n]$ exists as jointly continuous map from $\Ecal(\Mcal) \times \Ecal(\Mcal)^{\otimes n}$ to $\Cbb$. We denote by $\Fcal^\infty(\Mcal)$ the space of smooth functionals.
\end{definition}
%
A direct consequence from the characterization of smooth functionals in \ref{def:func-deriv} is $\Fsf(\phi)^{(n)}$ is a distribution of compact support on $\Mcal^n$.  Let us illustrate this definition via a simple example.%
\begin{example}
%
...
%
%
We have alrealdy noticed that a derivative of a functional, e.g. $\Fsf(\phi)^{(1)}$, is a distribution. Thus to characterize its regularity we will use the notion of wave front set of a distribution, introduced in the first chapter.%


\bigskip


One needs to give sense of the localization of an observable in a particular region of the spacetime $\Mcal$. We define for that the spacetime support of an observable. It is the set of points $x \in \Mcal$, such that for all neighborhood $U_x$ of $x$, there is two fields $\phi$ and $\psi$, with the property $\supp\left(\psi\right) \subset U_x$, and $\Fsf(\phi+\psi) \neq \Fsf(\phi)$.
%
