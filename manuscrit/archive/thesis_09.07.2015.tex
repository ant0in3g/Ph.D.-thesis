...
Before going further we recall what is a \textbf{seminorm}\index{seminorm} on a vector space $E$. It is a real valued function $p : E \to \Rbb$ such that
%
\begin{itemize}
...
We now define the notion of pseudo distance (also called pseudometric) between two points on a topological space. This notion will appear to be useful to ``build'' topologies. It is a map $d : \Xcal \times \Xcal \to \Rbb^+$ such that for all $x, y, z \in \Xcal$,%
%
\begin{itemize}
...
\end{itemize}
%
then $d$ is called a metric. We can show that a pseudometric space is also a topological space for which the topology is induced by the pseudo distance. Let us detail this in the following lemma.
...
Every locally integrable function (section) $\phi \in L^1_{loc}(\Mcal,E)$ can be seen as a distribution in $\Dcal^\prime(\Mcal,E)$ by writing
%
\begin{equation*}
\Fsf(\phi) = \phi(f) = \sm{\phi,f} = \int_\Mcal \dsf^n x \ \sqrt{\abs{g}} \ \phi(x) f(x) \ ,
\end{equation*}
%
with $f \in \Dcal(\Mcal,E)$.
...
%
\begin{equation*}
\hat{f}(\xi) = \int \dsf x \ f(x) \ \esf^{-i\sm{x,\xi}} \ , \quad \xi \in \Rbb^n \ .
\end{equation*}
%
If $\hat{f}$ also happens to be integrable one can wite $f$ in terms of $\hat{f}$ by Fourier's inversion formula
%
\begin{equation*}
f(x) = \left(\frac{1}{2\pi}\right)^n \int dx \ \hat{f}(\xi) \ \esf^{i\sm{x,\xi}} \ .
\end{equation*}
%
We say that $\hat{f}(\xi)$ is the density of the character $\esf^{i\sm{x,\xi}}$ in the harmonic decomposition of $f$. To study the Fourier transform it is useul to consider functions in a subset of $\Ecal(\Xsf)$ containing $\Dcal(\Xsf)$. We call this function space the \textbf{Schwartz space}\index{Schwartz space}. It is the set of all $f \in \Ecal(\Xsf)$ such that 
%
\begin{equation*}
...
\end{equation*}
%
The Fourier transform can be used to give criterion on the smoothness of functions. Smooth functions have Fourier transforms that decay rapidly to zero at infnity, and functions that decay rapidly to zero at infnity have smooth Fourier transforms. In other words if $u\in\Dcal(\Xsf)$ then
%
\begin{equation}
...
\end{equation}
%
with $n \in \Nbb \backslash \{0\}$ and $\xi \in \Rbb^n$. And conservely if \eqref{eq:crit_smooth_fourier} is fullfilled then $u$ is a smooth and compaclty supported function.


We can extend to the distribution framework. A continuous linear form $u$ on $\Scal(\Xsf)$ is called a \textbf{tempered distribution}\index{tempered distribution}. It is denoted by $\Scal^\prime(\Xsf)$. 
%
...
\end{equation*}
%
for $f \in \Scal$. The space of compaclty supported distribution $\Ecal^\prime(\Xsf)$ is a subspace of $\Scal^\prime(\Xsf)$, therefore the results holdind for $\Scal^\prime(\Xsf)$ also hold for $\Ecal^\prime(\Xsf)$. 
...
We will give in this section a way to characterize the singularities of a distribution. It is called the wave front set. It gives a precise description of singularities. It tells us not only at which points a singularity occurs, but it also indicates the directions in the dual space from which the singularities are ``coming''. Before defining the wave front set of a distribution we will recall some impotant notions.


\bigskip


We define the \textbf{singular support}\index{distribution!singular support} of a distribution $u$, $\singsupp(u)$, as the set of points in $X$ having no open neighborhood to which the restriction of $u$ is a smooth function. 


\bigskip


We have given in \eqref{eq:crit_smooth_fourier} a smooothness condition of a function on its Fourier transform. Let us recal it, the Fourier transform of a smooth compaclty supported function $u$ decays faster than any negative power of the dual variable $\xi$. We can reformulate this property saying that if the Fourier transform of a distribution with compact support satisfies \eqref{eq:crit_smooth_fourier} then this distribution is induced by a smooth map. 


\bigskip


A set $\Sigma \subset \Rbb^n \backslash \{0\}$ is called a conic set if for any point $\xi \in \Sigma$, it contains all the points $a \xi$ with $a > 0$. A conic set is completely determined by its intersection with the unit sphere $S^{n-1}$ in $\Rbb^n$. By a conic neighborhood of a point $\xi \in \Xsf \backslash \{0\}$ we mean an open conic set that contains $\xi$. Using this new notion we can define what is a regular direction of a compaclty suppoted distribution $u\in\Ecal^\prime(\Xsf)$. It is a vector $\xi \Rbb\backslash\{0\}$ such that there exists an open canonical neighborhood $\Sigma$ of $\xi$ and such that \eqref{eq:crit_smooth_fourier} holds. Conservely a singular direction of a distribution $u$ is the set of all directions which are not regular. 
...
Let $u \in \Ecal^\prime(\Rbb^n)$ and $\phi_1, \phi_2 \in \Ecal(\Rbb^n)$. If $\phi_2(x) \neq 0$ when $x \in \supp(\phi_1)$, then $\Sigma(\phi_1 u) \subset \Sigma(\phi_2 u)$. Due to this result the set of singular directions of $u\in\Dcal^\prime(\Xsf)$ at a point $x$ is defined as
%
\begin{equation*}
...
\end{equation*}
is called the \textbf{wave front set}\index{wave front set} of $u$. The projection in $X$ is $\mathsf{singsupp}(u)$. And if$u \in \Ecal^\prime(\Rbb^n)$ then the projection of $\WF(u)$ on the second variable is $\Sigma(u)$. 
...
We would like now being able to compute the wave front set of the product of two distributions which have singularities at the same points. In order to do it we notice $u \cdot v$ is the projection of the tensor product $u \otimes v$ to the diagonal. Therefore we shall first define the wave front set of the tensor product of two distributions. For $\Xsf$ and $\Ysf$ two subsets of $\Rbb^n$, the tensor product of $u\in\Dcal^\prime(\Xsf)$ and $v\in\Dcal^\prime(\Ysf)$ has the following wave front set
%
\begin{equation*}
...
\end{equation*}
%
Then if $\Xsf$ is taken equal to $\Ysf$, we have $u, v \Dcal^\prime(\Xsf)$, and $u\otimes v$ can be mapped with $d$ from $\Xsf \times \Xsf$ to $\Xsf$ if 
%
\begin{equation*}
\WF(u\otimes v) \cap N_d \ , 
...
%An algebraic state on is a complex valued linear functional $\omega$ such that
%
%\begin{equation*}
...
%An algebraic state $\omega : \Usf \to \Cbb$ on the $\Cbb^\ast$ algebra $\Usf$ (with unit)  is called a pure algebraic state if it is extreme in the set of algebraic states. An algebraic state that is not pure is called mixed.  
%\end{definition}
...
We are going to look at interactng theories where the time evolution is solved perturbatively. It means interacting observables are writen in terms of power series. We choose to write the interaction functionals using a time ordering prescription. This prescription is done by defining a time ordered product between observables, defined in a similar way as the $\star$ product where we require the following conditions
%
\begin{equation*}
\Fsf \cdot_\Tsf \Gsf = 
\left\{
...
\end{equation*}
%
It is called the causal factorisation property. It fixes uniquely the time ordered products among regular functionals, considered in order to avoid possible problems coming from prodcts of distributions.
%
\begin{equation}
...
\end{equation}
%
where $\Delta_\fsf$ is the time ordered (Feynman) version of $\Delta_+$, i.e. $\Delta_\fsf = \Delta_+ + i \Delta_\asf$ with $\Delta_\asf$ the advanced propagator of the free theory. The product defined in \eqref{eq:time_ordered_prod} can be ill defined. A reason for
this is the fact that the wave front set of the integral kernel of $\Delta_\asf$ contains the wave front set $\delta$ distribution because $\Psf \Delta_\asf = \Ibb$, and then pointwise product of $\Delta_\fsf$ are be ill defined on coinciding points. The Feynman propagator of $\Delta_\fsf$ can be writen using the Hadamard representation. For even dimension $d$ of the spacetime $\Mcal$ 
%
\begin{equation*}
\Hsf_\fsf(x,y) = \frac{(d/2-2)!}{2(2\pi)^{d/2}} \bigg( \frac{u(x,y)}{\sigma_\fsf(x,y)^{d/2-1}} + v(x,y) \log\left( \sigma_\fsf(x,y) \right) + w(x,y) \bigg) 
\end{equation*}
%
with $\sigma_\fsf(x,y) = \sigma(x,y) + i \epsilon$ and, $u(x,y)$, $v(x,y)$, and $w(x,y)$ are smooth symmetric biscalars, regular for $y \to x$ and which possesses expansion of the form
%
\begin{eqnarray*}
...
\end{equation*}
%
where $\Gamma$ is the Gamma function, anf again $u$ and $w$ are smooth, symmetric biscalar functions which have the following expansions
%
\begin{eqnarray*}
...
To understand the reason why we need a time ordering prescription let us go to the situation on Minkowski spacetime $\Mbb$. As we know now the Borchers Uhlmman algebra of the free scalar field contains only regular observables. It considers only linear combinations of products of free fields at separate points. But we want to study the interacting situation, we need to make sense of product of fields at the same point. And this is done the normal ordering prescription. The two point function of the massless scalar field on Minkowski spacetime. 
%
\begin{equation}
\omega_2(x,y) = \omega(\phi(x),\phi(y)) = \lim_{\epsilon \to 0} \frac{1}{(x-y)^2 + i \epsilon (x_0-y_0) + \epsilon^2} \ , 
\end{equation}
%
where $(x-y)^2$ denotes the Minkowski product, and the limit has to be taken after integrating $\omega_2$ with a test function. The two point function $\omega_2$ is smooth if $(x-y)^2 \neq 0$, it means $x$ and $y$ have to be timelike separated. If $x$ and $y$ are not timelike separated then $\omega_2$ has a singularity. However even with this singularity we can work on it, i.e. integrate $\omega_2(x,y)$ agianst two test functions and it gives us something finite. We can check that it is a distribution. When we have to make sense of observable like $\phi^2(x)$, if we we define it like
%
\begin{equation*}
...
In the interacting approach the interacting algebra is represented on the free algebra by means of the Bogoliubov formula. It is given in terms of the local $S$ matrix, 
%
\begin{equation*}
S(\Vsf) = \sum^\infty_{n=0} \frac{i^n}{n!\ \hbar^n} \ \Vsf(\phi) \cdot_\Tsf \cdots \cdot_\Tsf \Vsf(\phi) \ .
\end{equation*}
%
where $\Vsf(\phi)$ is a (functional) interacting potential. The observables are mapped from the interacting off shell algebra to the free off shell algebra with the Bogoliubov formula defined as follow 
%
\begin{equation*}
\Rcal_\Vsf(\Fsf(\phi)) = S(\Vsf)^{\star-1} \star \left( S(\Vsf) \cdot_\Tsf \Fsf(\phi) \right) \ ,
\end{equation*}
%
where $S^{\star-1}(\Vsf)$ is the inverse of $S(V)$ with respect to the $\star$ product. Considering regular functionals is not enough. It does not contain the interaction functionals, those functionals that we would like to work with. Therefore we have to defined a new space of functionals, it is done by imposing a less restrictive condition on the wave front set, we set that the wave front set of the functional derivatives of $\Fsf$. We call this new space the space of microcausal functionals.
%
\begin{equation*}
...
Let us recall the relevant formulae in perturbative algebraic quantum field theory where we shall always try to write expressions both in the pAQFT and in the more standard notation, indicating the latter by a $\doteq$. Given a local potential $\Vsf$, such as 
%
\begin{equation*}
\Vsf(\phi) = \int_\Mcal \dsf^4x \sqrt{\abs{\sqrt{g}}} \ \frac{\lambda}{4!} \phi(x)^4 \ f(x) \ ,
\end{equation*}
%
with $f \in \Dcal^(\Mcal)$. The corresponding $S$ matrix, which is the interacting field, i.e. ``the field in the interaction picture'' $\phi_I(x)$, is defined by the Bogoliubov formula.
%
\begin{equation*}
\phi_I(x) = \mathcal{R}_\Vsf(\phi) = S(V)^{\star-1} \star \left( S(V) \cdot_\Tsf \phi(x) \right) \doteq \Tsf\left(\esf^{\frac{i}{\hbar} \Vsf(\phi)}\right)^{\star-1} \Tsf\left(\esf^{\frac{i}{\hbar} \ . \Vsf}\phi(x)\right)
\end{equation*}
%
Interacting versions of more complicated expressions in the field, e.g. polynomials at different and coinciding points, are defined analogously. We remark that, in the Minkowski vacuum state $\Omega_0$, the expectation value of the Bogoliubov formula can be shown to read (also for more general expressions in the field)
