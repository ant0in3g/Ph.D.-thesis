...
\end{equation}
%
where $\supp(\Fsf)$ is said to be  later than   $\supp(\Gsf)$ if $J^+(\supp(\Fsf)) \cap \supp(\Gsf)=\emptyset$. If the supports of $\Fsf$ and $\Gsf$ coincide, we simply have $\Fsf \cdot_\Tsf \Gsf = \Fsf \star \Gsf$. Let us look at this product for linear functionals.
...
\end{equation*}
%
which does not depend on the partition of unity chosen. It is well defined on the full space up to the total diagonal $\Mcal^{n+1} \setminus d_{n+1}$.
...
\end{equation}
%
for all $\lambda \geq 0$ and all $\phi \in \Dcal(\Rbb^n)$, where every point $x_{i=1,\dots,n}(\lambda)$, which is connected to $x_1$, is affinely parametrized such that $x_{i}(0) = x_{1}$ and $x_{i}(1) = x_{i}$. We can now give the definition the Steinmann scaling degree of a distribution.
...
As we know for every pair of points $x_1,x_i$ in a normal neighborhood $\Ncal \subset \Mcal$ there exists a unique geodesic $\gamma$ connecting $x_1$ and $x_i$, with $i$ runs over $1,\dots,n$. We shall assume that
%
\begin{equation*}
