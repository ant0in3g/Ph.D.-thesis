...
\com{
Here we present the approach to the regularization problem we shall. We introducing a ``graphical way'' to write the time ordered product of different observables. To the treat the regularization issues we need to introduce a suitable regularisation scheme. 


The regularisation scheme we developed use the concept of minimal subtraction procedure that we shall define


We saw that the Epstein Glaser procedure \com{illustrated in section} \ref{p:CHAP_2_EPSTEIN_GLASER} is not convenient to perform \com{concrete computations}\sbar{in practise}.

We shall present another approach to this regularization problem, called the analytic regularization.
}
...
We can use the notion of scaling degree previously defined to give the scaling degree of $\tsf_\gamma$, introuced in \eqref{eq:kernel}, as follow
%
\begin{equation}
