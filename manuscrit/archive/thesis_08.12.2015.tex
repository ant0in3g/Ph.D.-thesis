...
%%TODO INTEGRATION !
%%TODO LEVI CIVITA CONNECTTION !
%%TODO SCALAR CURVATURE & RCCI TENSOR !
%%TODO GEODESIC & SYNGE'S WORLD FUNCT. !
...
There are two ways to see this algebra of classical observables, one algebraic, and one functional. The functional approach is an approach which permit to make computation. \par 

Let's start defining this algebra of classical obsevables. Usually we see states as objects which map observables to real numbers, but here we will look at observables as objects which map states to real numbers. At the classical level states will be view as smooth function on $\Mcal$, i.e. elements of $\Ecal(\Mcal)$. Then observable will be objects which map element from $\Ecal(\Mcal)$ to $\Rbb$, i.e. elements of $\Ecal(\Mcal)$. Then observables will be objects which map element from $\Ecal(\Mcal)$ to $\Rbb$, i.e. compactly supported supported distribution in $\Ecal^{\prime}(\Mcal)$. However we will start by looking at smooth obsevables in $\Dcal^{\prime}(\Mcal)$ and consider more singular one at the quantum level. \par

\begin{definition} \emph{Algebraic definitions of the \textbf{off-shell} algebra}
 By $\Ccal_0(\Mcal)$ we denote the off-shell algebra of classical observables of the scalar field. 
  
 
 Algebraic definition of $\Ccal_0(\Mcal)$. \\
  Here $\Ccal_0(\Mcal)$ is the vector space 
  \begin{equation}
   \Dcal_0 (\Mcal) = \overset{\infty}{\underset{n=0}{\bigoplus}} D(\Mcal^n),
  \end{equation}
  with $D(\Mcal^0) = \mathbb{R}$. A generic element a of $\Acal_0$ can be written as
  \begin{equation}
   a = \alpha_0 \oplus \overset{\infty}{\underset{i=1}{\bigoplus}} \ \overset{i}{\underset{j=1}{\bigotimes}} \ \alpha_i \ \phi_{i} (f_j),
  \end{equation}
  for example, 
  \begin{equation}
   a = \alpha_0 \ \oplus \ \alpha_1 \ \phi_1(f_1) \ \oplus \ \alpha_2 \ \phi_2(f_1) \otimes \phi_2(f_2) \ \oplus \ ... \ ,
  \end{equation}
  with $\alpha_i \in \mathbb{C}$, and $\phi_i \in D(\Mcal)$.
  The product of elements of this algebra is defined by the linear extension of the tensor product of $\Dcal(\Mcal^n)$. 
  Algebraic definition of $\Ccal_0(\Mcal)$  deformed.
   It's almost the same as before except that now
  \begin{equation}
   \Ccal_0(\Mcal) \doteq \bigoplus_{n=0}^{\infty} \ \Dcal_{\mathsf{s}}(\Mcal^n) \ ,
  \end{equation}
  with $\Dcal_{\mathsf{s}}(\Mcal^n)$ denoting the symmetric test functions in $n$ variables, $\Dcal_{\mathsf{s}}(\Mcal^0) \doteq \Rbb$. We equipped $\Ccal_0$ with the symetric product $._{\ssf}$
  \begin{equation} 
   f ._\ssf g \doteq \mathfrak{Sym} \left( f \otimes g \right)
  \end{equation}
  where $g, g \in \Ccal_0$ and $\mathfrak{Sym} \ : \ \Dcal(\Mcal^n) \to \Dcal_\ssf(\Mcal^n)$ is the total symmetrization projector. 

\end{definition}

\begin{definition} \emph{Functional definition of the \textbf{off-shell} algebra} \\
 As before by $\Ccal_0(\Mcal)$ we denote the off-shell algebra of classical observables of the scalar field. 
 
   Functional definition of $\Ccal_0(\Mcal)$. \ \\
  We can build the algebra over the smeared field 
  \begin{equation}
   \phi(f) = \sm{ \phi \ , \ f } 
  \end{equation}
  for $f \in D(\Mcal)$. The fields $\phi(f)$ are the generators of the algebra. A generic observable can be written as
  \begin{equation}
   b = \beta_0 + \sum_{i=1}^{\infty} \prod_{j=1}^{i} \ \beta_i \ \phi_{i} (f_j),
  \end{equation}
  for example, 
  \begin{equation}
   b = \beta_0 + \beta_1 \ \phi_1(f_1) + \beta_2 \ \phi_2(f_1) \phi_2(f_2) + ... \ .
  \end{equation}
  We will have to deal with product of fields such that $\phi(f)\phi(g)$.
  
   Functional definition of $\Ccal_0(\Mcal)$ deformed. \ \\
  This second definition is exactly the same as the first one at the classical level. I shouldn't write ``definition of $\Ccal_0(\Mcal)$ deformed'', because for now the product is not deformed, it's only at the quantum that it will be. I wrote it like this to keep in mind that at the quantum this is two different ways of working. I said it's exactly the same as before nevertheless instead to see the smearing product of $\phi$ and $f$ as a function of, we will see it as a function of $\phi$
  \begin{equation}
   F(\phi) = \sm{ \phi \ , \ f }
  \end{equation}
  We see that for now it's only a choice of ``reading'' the smearing product.  
 
\end{definition}

We will stay off shell at the classical level, we will only go on shell at the quantum level.



A very important remark, that I read for the first time in the PhD thesis of Thomas , is that a proper understanding of quantum fields on curved spacetimes requires to construct quantum field theory without having recourse to states, and hence, Hilbert spaces. Instead, one has to focus solely on the algebraic relations between fields and observable quantities first, and introduce states and related Hilbert space representations as secondary objects. \par

One usually takes the point of view that states map observables to real numbers. To define the algebra of classical observables, we reverse this way of thinking and regard observables as maps from states to the real numbers. In classical field theory, we regard states as being smooth functions on $\Mcal$, i.e. elements of $\Ecal(\Mcal)$. Observables are then maps from $\Ecal(\Mcal)$ to the real numbers and are therefore compactly supported distributions in $\Ecal^{\prime}(\Mcal)$. \par

The most simple algebra of observables is the Borchers-Uhlmman algebra, denoted by $\Acal(M)$. We will see that there are different ways to look at this algebra.\par

\begin{definition} \emph{Algebraic definition of the Borchers-Uhlmman algebra.}
 By $\Ccal_0(\Mcal)$ we denote the off-shell algebra of classical observables of the scalar field. 

 
 
  [{{[}BU-1{]} Firt way to descibe $\Acal(M)$.}] \ \\
 We can build the algebra over the smeared field 
 \begin{equation}
  \phi(f) = \langle \phi , f \rangle, 
 \end{equation}
 for $f \in D(\Mcal)$. We will have to deal with product of fields such that $\phi(f)\phi(g)$. We want to represent $\phi(f)$ as an operator of an Hilbert space, therefore we need an operation which encodes taking the adjoint with respect to a Hilbert space inner product. For this we define a $\ast$-operation such that
 \begin{eqnarray}
  && \phi(f)^{\ast} = \phi(\overline{f}) \\
  && \left[ \phi(f_1) ... \phi(f_n) \right]^{\ast} = \phi(f_n)^{\ast} ... \phi(f_1)^{\ast}.
 \end{eqnarray}
 The fields $\phi(f)$ are the generators of the algebra. A generic observable can be written as
 \begin{equation}
  a = \alpha_0 + \sum_{i=1}^{\infty} \prod_{j=1}^{i} \ \alpha_i \ \phi_{i} (f_j),
 \end{equation}
 for example, 
 \begin{equation}
  a = \alpha_0 + \alpha_1 \ \phi_1(f_1) + \alpha_2 \ \phi_2(f_1) \phi_2(f_2) + ... \ .
 \end{equation}
 Next point is the not the less important, we want to have a quantum field theory, it means that we need quantum fields. To have that we impose the value of the bracket of two fields in terms of the causal propagator
 \begin{equation}
  \phi(f),\phi(g) = i DisDelta(f,g). 
 \end{equation}
 And finally we want to work onshell\footnote{In physics, particularly in quantum field theory, configurations of a physical system that satisfy classical equations of motion are called on shell, and those that do not are called off shell. For instance, in classical mechanics in the action formulation, extremal solutions to the variational principle are on shell and the Euler-Lagrange equations are on shell equations (i.e., they do not hold off shell). Noether's theorem is also another on shell theorem.}, it means that we want to a dynamic theory. Therefore we impose
 \begin{equation}
  \phi(Pf) = 0. 
 \end{equation} 

  [{{[}BU-2{]} Second way to descibe $\Acal(M)$.}] \ \\
 We now consider the direct sum of $D(\Mcal^n)$.
 \begin{equation}
  \Acal_0 = \overset{\infty}{\underset{n=0}{\bigoplus}} D(\Mcal^n),
 \end{equation}
 with $D(\Mcal^0) = \mathbb{C}$. A generic element a of $\Acal_0$ can be written as
 \begin{equation}
  b = \beta_0 \oplus \overset{\infty}{\underset{i=1}{\bigoplus}} \ \overset{i}{\underset{j=1}{\bigotimes}} \ \beta_i \ \phi_{i} (f_j),
 \end{equation}
 for example, 
 \begin{equation}
  b = \beta_0 \ \oplus \ \beta_1 \ \phi_1(f_1) \ \oplus \ \beta_2 \ \phi_2(f_1) \otimes \phi_2(f_2) \ \oplus \ ... \ ,
 \end{equation}
 with $\alpha_i \in \mathbb{C}$, and $\phi_i \in D(\Mcal)$.
 The product of elements of this algebra is defined by the linear extension of the tensor product of $\Dcal(\Mcal^n)$. For the same reason as before we need a $\ast$-operation, it is defined as follow
 \begin{eqnarray}
  && f^{\ast}(x_1,...,x_n) = \overline{f}(x_1,...,x_n) \\
  && (f_1 \otimes ... \otimes f_n)^{\ast} = f_n^\ast \otimes ... \otimes f_1^\ast 
 \end{eqnarray}
 the $\ast$-operation is nothing more than the complec conjugation. As said befoe we also need to have a dynamical quantum fiel theory, it implies respectively the two following conditions
 \begin{eqnarray}
  && f \otimes g - g \otimes f = i DisDelta(f,g) \\
  && P f = 0.
 \end{eqnarray}
 We noticed that the elements of the form $-i DisDelta(f,g) \oplus (f \otimes g - g \otimes f)$ and $P f$ generate a closed $\ast$-ideal, that we will denote by $\Ical$. The qotient of $\Acal_0$ by $\Ical$ is then the full Borchers-Uhlmman algebra.
 \begin{equation}
  \Acal = \Acal_0 / \Ical \ .
 \end{equation}

  [{{[}BU-3{]} Third way to descibe $\Acal(M)$.}] \ \\
 It's almost the same as before except that now
 \begin{equation}
  \Acal^\prime_0 = \overset{\infty}{\underset{n=0}{\bigoplus}} D_S(\Mcal^n),
 \end{equation}
 where $D_S(\Mcal^n)$ denote the symetric test function in $n$ variables, and with $D_S(\Mcal^0)=\mathbb{C}$. We equipped $\Acal^\prime_0$ with the symetric product $._{S}$
 \begin{equation} 
  f ._S g \doteq \mathfrak{Sym} \left( f \otimes g \right)
 \end{equation}
 where $g, g \in \Acal^\prime_0$ and $\mathfrak{Sym} \ : \ \Dcal(\Mcal^n) \to \Dcal_S(\Mcal^n)$ is the total symmetrization projector. To care about the fact that we want a quantum theory we have to modify this product. Given a test function $f_n \in \Dcal(\Mcal^n)$, we define the contraction operator $\Gamma_B \ : \  \Dcal(\Mcal^n) \to \Dcal(\Mcal^{n-2})$ related to a bidistribution $B$ as
 \begin{equation}
  \left( \Gamma_B f_n \right)(x_1,...,x_{n-2}) \ \doteq \ \sum_{i=1}^{n-1} \sum_{j=i+1}^{n}
  \int d\boldsymbol{y}_1 \ d\boldsymbol{y}_2 \ B(\boldsymbol{y}_1,\boldsymbol{y}_2) \ f(x_1, ... , x_i , \boldsymbol{y}_1 , x_{i+1} , ... , x_{j-1} , \boldsymbol{y}_2 , x_{j} , ... , x_{n-2}) \ .
 \end{equation}
 For $n<2$ and $f_n \in \Dcal(\Mcal^n)$, we set $\Gamma_B f_n  = 0$. Given two elements in $\Acal^\prime_0$ we define a star product $\star_\Delta$ on  $\Acal^\prime_0$ by setting
 \begin{equation}
  f \star_\Delta g \ \doteq \ \mathfrak{Sym} \left( e^{\frac{i}{2}\Gamma_\Delta} (f \otimes g) \right) \ ,
 \end{equation}
 where $\Delta$ is the causal propagator. The two new products that we just defined are linked
 \begin{equation}
  \frac{1}{2} \ f \star_\Delta g \ \doteq \ f ._S g \oplus \frac{i}{2} DisDelta(f,g) \ ,
 \end{equation}
 and, hence
 \begin{equation}
  \frac{1}{2} \ ( f \star_\Delta g \ - \ g \star_\Delta f ) \ \doteq \ i DisDelta(f,g).
 \end{equation}
 For now we have the algebra of observables for a quantum field theory off shell. To implemente the dynamic we need to take the quotient of $\Acal^\prime_0$ by the ideal $\Ical^{\prime}$ generated by $Pf$.
 \begin{equation}
  \Acal = \Acal_0^\prime / \Ical^\prime \ .
 \end{equation} 
 $\Acal$ with the product $\star_\Delta$ form the Borchers-Uhlmman algebra. 
 
\end{definition}


We will introduce in this section the functional formulation of field theory. This formulation has the advantage that is less abstract that the previous ones, we can do computations. \par

The configuration space of classical field is the space of smooth functions
\begin{equation}
 \phi \ : \ \Mcal \to \mathbb{C}, \quad \phi \in \Ecal(\Mcal),
\end{equation}
and the observables are functionals on this space.
