...
\end{equation*}
%
coincide with the $\star$ property for $\supp(\Fsf_1)$ later that $\supp(\Fsf_2)$. It is well defined on $\Mcal^n \backslash d_n$. Now let us assume that we were able to construct the time ordered product of $k$ ``causal'' functionals, with $k \in \{1,\dots,n-1\}$. We supose it is well defined on the full space minus the total diagonal, and that it satisfies the causal factorisation property. We want now to define the time ordered product of $n$ functionals.


%\bigskip

We shall need a partition of $\Mcal^n \backslash d_n$. Let $\Jcal$ be the set of all non empty proper subset $I$ of $\{1,\dots,n\}$, and $I^c$ the complement set of $I$ in $\{1,\dots,n\}$. We define the subset $\Ccal_I$ of $\Mcal^n$ as follow
%
\begin{equation*}
...
\end{equation*}
%
is well defined on the full space up to the total diagonal $\Mcal^n \backslash d_n$. 
...
This procedure, knwon as the Epstein Glaser regularisation, is theoreticaly clear but quite difficult to implement in practise. The aim of the present work is to discuss a renormalisation scheme which is suitable for practical computations.
...
We saw that the Epstein Glaser procedure \ref{p:EPSTEIN_GLASER} is not convenient to perform in practise. We shall present another approach to this regularisation problem, called the analytic regularisation. In order to present this method we shall write the time ordered product in a ``graphical way''. It shall permit us to write the time ordered product of observables as a sum of Feynman diagram. 
...
\prod_{1 \leq i < j \leq n} \ \sum_{\ell_{ij}=1}^\infty \hbar^{\abs{\ell_{ij}}} \ \frac{D_{ij}^{\abs{\ell_{ij}}}}{\abs{\ell_{ij}} !} \ , \ \ \mbox{ and } \ D_{ij} \ = \ 
\sm{\Delta_{ij} \ , \ \frac{\delta^2 \hfil}{\delta \phi_i \ \delta \phi_j}} \ ,
\end{equation*}
%
where $\abs{\ell_{ij}}$ for $i$ fixed is the number of edges starting from the vertex $i$, $\phi_i = \phi(x_i)$, and $\Delta_{ij}=\Delta_\fsf(x_i,x_j)$. We shall now introduce the graphical way we shall use. Let $\Gcal_n$ be the set of graphs with $n=V(\Gcal_n)$ vertices and $E(\Gcal_n)$ edges. We will consider only the subgraphs $\gamma$ of $\Gcal_n$ such that
%
\begin{equation*}
...
%
\begin{equation*}
\Nsf(\gamma) = \hbar^{-\abs{\Esf(\gamma)}} \ \abs{\Esf(\gamma)} ! \ , 
%
\qquad 
...
\end{equation*}
%
Let us illustrate this graphical approach with an. We shall ``draw'' the time ordered product of three observables.
%


$\Fsf_1 \cdot_{\Tsf} \Fsf_2(\phi) \cdot_{\Tsf} \Fsf_3(\phi)$
...
\draw (4,0) circle (0.5pt and 0.1pt);
%\draw (5,0) circle (1cm and 0.6cm);
%
\end{tikzpicture}
...
%----------------------------------------------------------------------------%
\section{Extensions of distributions}
%----------------------------------------------------------------------------%


We have seen that the the regularisation problem can be reduced to find an extension of a distribution. We shall look at the situation on Minkowski spacetime. 


%\bigskip


We denote by $u$ the distribution for which we want to find an extension. $u \in \Dcal^\prime(\Rbb^d \backslash \left\{ 0\right\} )$ is a distribution defined for all test functions supported outside the origin. We call $\dot{u}$ an \textbf{extension} of $u$ if 
%
\begin{equation*}
\mbox{for } \ \dot{u} \in \Dcal^\prime(\Rbb^d), \ \mbox{ we have } \ \forall \phi \in \Dcal\left(\Rbb^d \backslash \left\{ 0\right\} \right), \ \dot{u}(\phi) = u(\phi) \ .
\end{equation*}


We cannot always find an extension of a distribution ill defined on a singular point, but when we can the extension is not unique. A distribution supported at the origin can be written as a polynomial in the derivatives of the Dirac distribution. Therefore we could add a polynom in the derivatives of this Dirac distribution to the extension and we would not change the extension of our distribution ill defined at the origin. We have to restrict the possibilities of extension by adding a constraint. A possible choice of a constraint is to to require the distributions we want to extend and their extensions have the same Steinmann scaling degree. Let us define the Steinmann scaling degree of a distribution.
...
%
\begin{equation*}
\sd(u) \ \doteq \ \inf\bigg\{ \ \omega \in \Rbb \ \bigg| \ \lim_{\rho \downarrow 0} \ \rho^\omega u_\rho \ = \ 0, \ \mbox{in the distributional sense } \bigg\} \ ,
\end{equation*}
%
with 
%
...
The Steinmann scaling degree has the the following properties. Let $u\in\Dcal^\prime(\Rbb^n)$, $v\in\Dcal^\prime(\Rbb^k)$, and $\alpha \in \Nbb^d$, then
%
\begin{itemize}
\setlength\itemsep{0pt}
\item $\sd(\partial^\alpha u) \leq \sd(u) + \abs{\alpha}$,
\item $\sd(x^\alpha u) \leq \sd(u) - \abs{\alpha}$,
\item $\forall f \in \Ecal(\Rbb^d)$, then $\sd(\phi u) \leq \sd(u)$
...
The Steinmann scaling degree permits us to predict the existence and the possible uniqueness of a distribution. If the scaling degree of the distribution $u$ is finite, then we can always define an extension of $u$, if in particular it is smaller than the dimension of the space, then the extension is unique. Let us summarize this important result in the following theorem.


\begin{theorem}[Existence and uniqueness of an extension] \label{th:extension_distribution}
Let $u \in \Dcal^\prime(\Rbb^d \backslash\left\{ 0\right\} )$, then
%
\vspace*{-5pt}
\begin{itemize}
\setlength\itemsep{0pt}
...
The extension to the total diagonal can be performed extending the distributions without altering the scaling degree towards the diagonal. In this procedure there is the freedom of adding finite local contributions supported on the total diagonal. This freedom is the regularisation freedom. 
