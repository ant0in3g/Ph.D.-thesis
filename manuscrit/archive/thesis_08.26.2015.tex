...
% Detexif, an attempt to simplify to search in the latex symbole list : available at http://detexify.kirelabs.org/classify.html
% --
...
We shall now look to interacting quantum field theory in a perturbative way. The functional approach shall still be used. The aim of this chapter is to intoduce the fundations of perturbative algebraic quantum field theory, in order to identify precisely the renormalization problem.


%----------------------------------------------------------------------------%
\section{Hadamard state and the wave front set}
%----------------------------------------------------------------------------%

%%TODO mistake compilation in this section
...
Let $X$ be an open set in $\Rbb^n$. A distribution $u$ is a linear form\footnote{A linear form is a linear map from a vector space to its field of scalars.} on $\Dcal(X)$ such that for every compact set $K \subset X$ there exist constant $C$ and $k$ suc that 
\begin{equation*}
\abs{u(\phi)} \leq C \sum_{\abs{\alpha} \leq k} \sup \abs{\partial^\alpha \phi} \ , \quad \phi \in \Dcal(X) \ .
\end{equation*}
The set of all distribution in $X$ is denoted by $\Dcal^\prime(X)$. 
\end{definition}
...
If $u \in \Dcal^\prime(X)$, then the singular support of $u$, denoted $\mathsf{singsupp}(u)$, is the set of points in $X$ having no open neighborhood to which the restriction of $u$ is a $\Ecal$ function. 
\end{definition}

Since every point in $X \backslash \mathsf{singsupp}(u)$ has a neighborhood where $u$ is a $\Ecal$ function, it follows that the restriction of $u$ to $X \backslash \mathsf{singsupp}(u)$ is a $\Ecal$ map. The wave front set gives more precise description of singularities than the singular support. It tells us not only at what points a singularities occur, but it also indicates the directions in the dual space from which the singularities are coming, i.e. the direction for which does not hold. Before defining the wave front set of a distribution we will recall some definitions.
...
We have denoted set of functionals by $\mathcal{F}_\sharp(\Mcal)$. 
...
We restrist ourselves to the case of a 4 dimensional globally hyperbolic spacetime $(\mathcal{M},g)$. The classical action of the Klein Gordon field can be written as%
%
\begin{equation*}
S = \int_\mathcal{M} d^4x \ \sqrt{\left|\mathsf{det}(g)\right|} \ \left( \frac12 \phi_{;\mu} \phi^{;\mu}  - \frac12 \left( \xi R + m^2 \right) \phi^2 + V \right) ,
\end{equation*}
%
where $\xi \in \mathbb{R}$ is a constant, $R$ is the scalar curvature, $m^2\geq 0$ is the mass of the field, and $V:\mathcal{M} \to \mathbb{R}$ is any smooth function. We can derive the free equation of motion (also called the Klein Gordon equation)%
%
\begin{equation*}
P \phi \doteq \left( \Box + \xi R + m^2 \right) \phi = 0 , 
\end{equation*}
%
where $P$ is the Klein Gordon operator, and $\Box$ denotes the d'Alembertian. The stress energy tensor related to the free action can be read as%
%
\begin{equation*}
...
\end{eqnarray*}
%
Indeed due to \eqref{eq:Ph} we have%
%
\begin{eqnarray*}
