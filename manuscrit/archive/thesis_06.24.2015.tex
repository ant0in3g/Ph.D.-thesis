...
The starting block for a physical theory is the notion of spacetime. To view space and time as an unique entity has been a turning point. Newton's physics treated them separately, but at the beginning of the last century Einstein introduced a completely new point of view of these two entities. In his theory of gravitation, the physical background (i.e. the spacetime) is an ``active actor''. Indeed gravitation roughly speaking can be viewed as a deformation of the spacetime. 

\bigskip

Therefore we will introduce the notion of spacetime starting from the very beginning. In mathematics the most fundamental way to define a space is to use the notion of topology. It is concerned with the intrinsec properties of spaces.

\begin{definition}[Topological space] 
Let $\Xsf$ be a set. A topology on $\Xsf$ is a collection $\Tcal$ of subsets satisfying the three axioms%
%
\begin{itemize}
\item \textbf{A1 - conventions on empty set} : $\emptyset , \ \Xsf \in \Tcal$
\item \textbf{A2 - arbitrary union} : $U_i \in \Tcal \mbox{ for } i \in I \Longrightarrow \bigcup_{i\in I} U_i \in \Tcal$, where $I$ is an arbitrary index set ;
\item \textbf{A3 - finite intersection} : $U_1 , \dots , U_n \in \Tcal \Longrightarrow U_1 \cap \dots \cap U_n \in \Tcal$ .
\end{itemize}
%
\end{definition}

The pair $(\Xsf,\Tcal)$ is called a topological space. We will often omit to precise the topology $\Tcal$, and say simply that $\Xsf$ is a topological space. Let give us some exam to illustrate this definition.
...
\end{equation*}
%
Therefore on says that a topological space is \textbf{second countable}\footnote{A set is said to be \textbf{countable} it there exists a one to one correspondence between the considered set and the set of integers $\Nbb$.} if it has a countable topological basis. A subset $A$ of a topological space $\Xsf$ is called \textbf{dense} in $\Xsf$ if any point $x$ in $\Xsf$ belongs to $A$ or is a limit point \footnote{A limit point $p$ of $A$ is a point such that every open set around it contains at least one point of $A$ different from $p$.} of $A$. Equivalently, $A$ is dense in $\Xsf$ if and only if the only closed subset of $\Xsf$ containing A is $\Xsf$ itself. 

And a topological space is \textbf{separable} if there exists a dense and countable subset of $\Xsf$.
...
Montrer qu’un espace métrisable X est séparable si et seulement s’il secound countable
est à base dénombrable d’ouverts
