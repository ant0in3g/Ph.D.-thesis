...
%%TODO ??
%The aim of this section is provide prêt-à-porter formulae for doing perturbative computations in the regularization scheme devised in the previous sections for the special case of Friedmann--Lemaître--Robertson--Walker (FLRW) spacetimes. We thus consider spacetimes $(\Mcal,g)$ of the form $\Mcal=I\times \Rbb^3\subset \Rbb^4$ and, in comoving coordinates,
...
%
\item[2 -- Symmetry.] $\Tcal_n$ is symmetric under a permutations of its arguments.
%
\item[3 -- Unitarity.] $\Tcal_n$ is unitary.
%
\item[4 -- Local and covariant.] $\Tcal_n$ is local and covariant.
%
\item[5 -- Microlocal spectrum.] $\Tcal_n$ satisfies the microlocal spectrum condition.
%
\item[6 -- Field Independence.] $\Tcal_n$ is $\phi$ independent.
%
\item[7 -- Leibniz rule.] $\Tcal_n$ satisfies the Leibniz rule.
%
\item[8 -- Principle of Perturbative Agreement.] $\Tcal_n$ satisfies the Principle of Perturbative Agreement for perturbations of the generalised mass term $\mu$ in the free Klein Gordon equation
%
\begin{equation*}
\Psf \phi = \left( - \Box + \mu \right) \phi = 0 \ . 
\end{equation*}
%
\item[9 -- Isometry invariance.] If the spacetime $\Mcal$ has non trivial isometries and if the Feynman propagator $\Hsf_\fsf$ is chosen such as to be invariant under these isometries, then $\Tcal_n$ is invariant under these isometries as well.
%
\end{description}
...
Inspired by the formula of Gell Mann and Low, we define the $S$ matrix for $A , B, F \in \Fcal_\Tsf(\Mcal)[[\hbar]]$ as follow
%
\begin{equation*}
S : \left\{ 
\begin{array}{ccl}
\Fcal_\Tsf(\Mcal)[[\hbar]] & \to & \Fcal_\Tsf(\Mcal)[[\hbar]] \\ 
\Fsf & \mapsto & S(\Fsf) = \exp_\Tsf(\Fsf) 
\end{array}
\right. \ . 
\label{eq:def_Smatrix}
\end{equation*}
%
...
\begin{description}
%
\item[S1 -- Causality.] $S(A+B)=S(A)\star S(B)$, if $\supp(A) \neq \supp(B)$.
%
\item[S2 -- Starting Elements.] This fixe the higher derivatives of $S$ at the origin partially on local functionals.
%
\begin{eqnarray*}
&& S(0) = 1 \ : \ \Ecal(\Mcal) \ \to \ 1 \in \Rbb, \\
&& S^{(1)}(0) = \mathsf{id} \ : \ \Fcal_{\mathsf{loc}}(\Mcal) \to \Fcal_{\mathsf{\mu c}}(\Mcal).
\end{eqnarray*}
%
\item[S3 -- $\phi$ locality.] The value of $S(F) \in \Fcal_{\mathsf{\mu c}}(\Mcal)$ at a given field configuration $\phi_{0}$ depends only on the Taylor expansion of $F \in \Fcal_{\mathsf{loc}}(\Mcal)$ around $\phi_{0}$, 
%
\begin{equation*}
S(F)(\phi_{0})=S(F_{\phi_{0}}^{[N]})(\phi_{0})+\mathcal{O}(\hbar^{N+1}) \ , 
\end{equation*}
%
where ${\displaystyle F_{\phi_{0}}^{\left[N\right]}(\phi) = \sum_{n=0}^{N}\frac{1}{n!} \left\langle F^{\left(n\right)}(\phi_{0}),\left(\phi-\phi_{0}\right)^{\otimes n}\right\rangle }$ denotes the Taylor expansion of $F$ around $\phi_0$ up to order $N$. It implies that only a set of finite terms contribute if we stop the pertubative expansion of the $\Ssf$ matrix at a given order in $\hbar$. \par
%
\item[S4 -- Field Independence.] $S$ depends only implicitly on the field configuration, 
%
\begin{equation*}
\forall \psi : \quad {\displaystyle \left\langle \frac{\delta S(F)}{\delta\phi},\psi\right\rangle = S^{(1)} \Big|_{F} . \bigg( \left\langle \frac{\delta F}{\delta \phi},\psi\right\rangle }\bigg)\ .
\end{equation*}
%
i.e. via the interaction $F$.
%
\end{description}


The freedom that we have when we want to write the $\Ssf$ matrix is described by the Stückelberg Petermann renormalization group $\Rcal$. It is the group of analytic maps
%
\begin{equation*}
 \Zsf : \Fcal_\Tsf(\Mcal)[[\hbar]] \ \to \ \Fcal_\Tsf(\Mcal)[[\hbar]] \ ,
\end{equation*}
%
with composition as group operation, and $\Zsf \in \Rcal$ having the following properties,
%
\begin{description}
%
\item[RG1] $\Zsf(0)=0$
%
\item[RG2 -- Starting Element.] $\Zsf^{\left(1\right)}(0)= \mathsf{id}$
%
\item[RG3] $\Zsf=\mathsf{id}+\mathcal{O}(\hbar)$
%
\item[RG4 -- Locality.] Let $A,B,C \in \Fcal_{\mathsf{loc}}(\Mcal)$ with $\supp(A)\cap\supp(C)=\emptyset$, then 
%
\begin{equation*}
\Zsf(A+B+C) = \Zsf(A+B) - \Zsf(B) + \Zsf(B+C)  
\end{equation*}
%
\item[RG5 -- $\phi$ locality.] $\Zsf(F)(\phi_{0}) = \Zsf(F_{\phi_{0}}^{\left[N\right]})(\phi_{0})+\mathcal{O}(\hbar^{N+1})$
%
\item[RG6 -- Field Independence.] $\Zsf$ depends only implicitly on the field $\phi$, 
%
\begin{equation*}
\forall \phi \in \Ecal(\Mcal): \ \frac{\delta \Zsf}{\delta\phi}=0 \ .
\end{equation*}
%
\end{description}


With all these notions we can write the main theorem of renormalization.

\begin{theorem}[Main Theorem of Renormalization]\label{th:main_renorm} 
Given two $S$ matrices $S_1$ and $S_2$ satisfying the conditions Causality, Starting Element, $\phi$ Locality, and Field Independence, (S1) -- (S4), there exists a unique $\Zsf\in\Rcal$ such that
%
\begin{equation*}
S_2 = S_1 \circ \Zsf \ . 
\end{equation*}
%
Conversely, given an $S$-matrix $S$ satisfying (S1) -- (S4) and a $\Zsf \in \Rsf$, then it defines a new $S$ matrix satisfying conditions (S1) -- (S4). 
\end{theorem}

We see with this theorem that all the $S$  matrices are related, and in particular the unrenormalized one can be linked to a renormalized $S$ matrix. And also all the renormalized $S$ matrices are related, all the renormalization constant are related with these $\Zsf$ function. In particular what we would do is to compute the renormalize $S$ matrix for particular values of the renormalization constants, and then due to the main theorem of renormalization ref{th:main-renorm} we will be able to get all the others $S$ matrix.
