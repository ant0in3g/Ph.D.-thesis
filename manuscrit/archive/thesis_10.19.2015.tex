...
\end{equation}
%
Furthermore if $\Delta f = 0$ then  $f = \Psf g$, for some $g\in\Dcal(\Mcal)$. Finally $\Psf$ is formally self adjoint, i.e. $\Delta(f,g) = - \Delta(g,f)$, where
%
\begin{equation}
...
%
\begin{equation*}
\com{\phi = \Delta f \ , \qquad  \psi   =  \Delta g}
\end{equation*}
%
\com{for some suitable $f,g \in \Dcal(\Mcal)$,}
then \com{the} symplectic form $\tau$ can be written in terms of the causal propagator $\Delta$ \sbar{such that}
%
\begin{equation*}
\tau(\phi,\psi) = \Delta(f,g) \ , \quad \mbox{\com{where} \sbar{with}} \qquad \Delta(f,g) = \sm{f,\Delta g} \ ,
\end{equation*}
%
\sbar{for suitable test functions. }
\end{lemma}
...
Let $\phi$ and $\psi$ be elements of the space \eqref{eq:sol_space}, then we have for suitable test functions $f$ and $g$, $\phi=\Delta f$ and $\psi=\Delta g$. 
 \com{ ... .   can you write how to find $f$ and $g$?  ....    something like 
 \[
 f =     P  \phi \chi
 \]
...
We would like to define \com{the } classical \com{algebra of a} free \com{field theory} \sbar{algebra}. The on shell algebra is obtained by taking the \sbar{quationt} \com{quotient} of the off shell algebra with respect to the ideal generated by the the equation of motion \eqref{eq:kg_eq}. 
\com{I don't understand this sentence.... what do you want to say???}


However\sbar{, we recall that we would like } \sbar{we are going} to describe an interacting real scalar field theory, where the interaction shall be treat by means of perturbation theory. Therefore the requirement that an equation of motion is satisfied shall be dropped. We shall work with the classical free off shell $\ast$--algebra of regular functionals, where the involution is defined ass follow
%
\begin{equation*}
...
\end{equation*}
%
where $E^\ast$ is the dual space of $E$. A such linear map $u$ is called continuous if for any $(\phi_n)$ with elements in $\Dcal(\Mcal,E^\ast)$ where $\phi_n \to \phi \in \Dcal(\Mcal,E^\ast)$, we have $u(\pi_n) \to u(\pi)$. The space formed by these continuous linear maps is denoted by $\Dcal^\prime(\Mcal,E,\Ysf)$. We shall restrict ourselves for now to the line vector \sbar{ vetor} bundle. Therefore with a small common abuse of notation we shall denote this space by $\Dcal^\prime(\Mcal)$. 
...
Moreover\com{,} as know $\Mcal$ looks locally like $\Rbb^n$, thus some definitions and properties obtained for distributions on $\Rbb^n$ are expected ``to be \sbar{transfered} \com{transferred}'' to the case of a manifold. In particular \eqref{eq:distibution_function} can be extended to $\Mcal$ \sbar{owing} \com{????? what do you want to say???   ... taking into account that} on Lorentzian manifold we have a distinguished volume element (cf. section \ref{p:INTEGRATION}).
...
\end{equation*}
%
with $k \in \Nbb \setminus \{0\}$, and where the first equality is obtained after performing $k$ successive integrations by part. A second example can be presented with the Heaviside function for \sbar{whic} \com{which} we associate the distribution $\Theta$ as follow\com{s}
%
\begin{equation*}
H(x) = \left\{
\begin{array}{l}
...
\end{equation*}
%
\sbar{with} \com{where} $\phi$ \com{is a} suitable test function. We compute its derivative as
%
\begin{equation*}
...
\end{equation*}
%
where $k \in \Rbb^n$, and $x \cdot k$ \com{is} the inner product in $\Rbb^n$. The pointwise product of two Fourier transforms of two integrable functions $f$ and $g$ over $\Rbb^n$ can be written as the Fourier transform of the convolution of $f$ and $g$, i.e.
%
\begin{equation*}
\hat{f} \cdot \hat{g} = \Frak( f \ast g )(x) \ , \quad \mbox{with} \qquad (f \ast g) := \int_{\Rbb^n} \dsf y \ f(y) g(x-y) \ . 
\end{equation*}
%
There exists a particular space of functions which behaves \sbar{particulary} \com{particularly} well under the action of $\Frak$. It is the subspace of $\Ecal(\Rbb^n)$ where all functions $\phi$ are \textbf{rapidly decreasing}, i.e.
%
\begin{equation*}
\com{\mbox{\sbar{for all} } \ \phi \in \Ecal(\Rbb^n) \ \mbox{ \sbar{we have} where, for all multiindices $\alpha$ and $\beta$,} \ \norm{\phi}_{\alpha\beta} := \sup_x \abs{x^\alpha \partial^\beta \phi(x)} < \infty }\ ,
\label{eq:norm_schwartz}
\end{equation*}
%
where we used \sbar{dor} \com{for} the multiindices $\alpha$ and $\beta$ the same convention as in \eqref{eq:conv_multiindex}. We call this function space the \textbf{Schwartz space}\index{Schwartz space} and denote it by $\Scal(\Rbb^n)$. It can be shown that $\Scal(\Rbb^n)$ is complete \com{in which topology?}. Furthermore the seminorm defined in \sbar{\eqref{eq:norm_schwartz} genrate}  \com{section \ref{eq:norm_schwartz} generate} a topology on $\Scal(\Rbb^n)$,  \com{...  now you say that the space is complete and then you write that it is metrizabel... }\sbar{thererfore} \com{therefore} this space is metrizable. We can say then the Schwartz space is a Fréchet space. In addition of being a subspace of $\Ecal(\Rbb^n)$ this new space is contained in $\Dcal(\Rbb^n)$, i.e.
%
\begin{equation*}
...
%
\begin{equation}
\phi \in \Dcal(\Rbb^n) \ \  \Longleftrightarrow  \ \ \abs{\hat{\phi}(k)} \leq \com{C_n} (1+\abs{k})^{-n} \ , \ \mbox{ for } \  \com{C_n} \in \Rbb \ , \ \ n \in \Nbb \setminus \{0\} \ , \ \mbox{ and } \ \ k \in \Rbb^n \ .
...
For $u \in \Dcal^\prime(\Rbb^n)$ and $\Xsf \subset \Rbb^n$, using the characterization \eqref{eq:distrib_smooth_funct}, we say that the \textbf{restriction} $\left.u\right|_{\Xsf}$ is \textbf{smooth, if and only if} for all $\phi$ in $\Dcal(\Xsf)$ defined as
%
\begin{equation*}
...
%
\begin{equation}
\abs{\sm{ u , \phi(x) }} \leq \com{C_n} (1+\abs{k})^{-n} \ , 
\label{eq:smooth_criterion_distrib}
\end{equation}
for $\com{C_n} \in \Rbb$, $n \in \Nbb \setminus \{0\}$, and $k\in \Rbb^n$.
\end{lemma}
...
\end{equation*}
%
The Fourier transform map $\Frak$\com{,} which maps $\Scal^\prime(\Rbb^n)$ to itself\com{,} is a sequentially continuous isomorphism, whereas its inverse is only sequentially continuous. We recall that a map $\fsf : \Xsf \to \Ysf$ where $\Xsf$ and $\Ysf$ are at least toplogical spaces, is said to be sequentially continuous if for any convergent sequence $(x_n) \to x$ in $\Xsf$, we have $(\fsf(x_n)) \to \fsf(x)$ in $\Ysf$. 
...
%
%
\item We can now define what is a \textbf{regular direction}\index{regular direction} of a \sbar{compaclty suppoted} \com{compactly supported } distribution $u\in\Ecal^\prime(\Xsf)$. It is a vector $k \in \Rbb\setminus\{0\}$ such that 
%
\begin{itemize}
\setlength\itemsep{0pt}
...
\end{itemize}
%
Conversely a \textbf{singular direction}\index{singular direction} of a distribution $u$ \com{is a direction which is not regular. The set of singular directions } \sbar{is the set of all directions which are not regular, this set} is denoted by $\Sigma (u)$, i.e.
%
\begin{equation}
...
Thus the set of singular directions of $u\in\Dcal^\prime(\Xsf)$ at a point $x$ is defined as
%
\begin{equation*}
\Sigma_x(u) := \underset{\phi \in \Dcal(X)}{\bigcap} \ \Sigma(\phi u) \ , 
\end{equation*}
%
with $\phi(x) \neq 0$. Since it is an intersection of closed conic sets, $\Sigma_x(u)$ is also a closed conic set. The set $\Sigma_x(u)$ encodes both notions \com{namely} the singular support and \com{the singular} direction\com{s} of $u$. This permits us to define the notion of wave front set (cf. \cite{hormander_analysis_1990}).
...
We now want to the extend definition of wave front set to the the case of distribution defined on \com{a} manifold $\Mcal$. We recall \com{that the tangent space at $x$ of $\Mcal$ is denoted by } \sbar{we denote by} $T_x\Mcal$  \sbar{the tangent space at $x$ of $\Mcal$},\com{while} \sbar{and by} $T^\ast \Mcal$ \com{indicates} the cotangent bundle.
...
Let us be more precise. We consider an atlas \com{$\{(U_i,\phi_i)\}$} covering $\Mcal$, and a local trivialization $(U_i,\psi)$ of the line bundle. The wave front set of a distribution $u\in\Dcal^\prime(\Mcal)$ whose restriction to $U_i$ is $u_i$ is defined as the union of the wave front set of all restriction on coordinate neighborhoods. The restriction $u_i$ has the following wave front set
%
\begin{equation*}
...
Let $u$ and $v$ be in \sbar{$\Dcal^\prime(\Rbb^n)$}  \com{$\Ecal^\prime(\Rbb^n)$}. We say that \sbar{$w\in\Dcal^\prime(\Rbb^n)$} \com{$w\in\Ecal^\prime(\Rbb^n)$} is the pointwise product of $u$ and $v$ if and only if 
\sbar{$\forall x \in \Rbb^n$, $\exists \phi \in \Dcal(\Rbb^n)$, such that} for all $k \in \Rbb^n \setminus \{0\}$ the following integral is absolutely convergent.
%
\begin{equation*}
\widehat{w}(k) := \left(\widehat{u} \ast \widehat{v}\right)(k) = (2\pi)^{-n} \int_{\Rbb^n} \dsf q \ \widehat{u}(q) \ \widehat{v}(k-q) , 
\end{equation*}
%
where using \eqref{eq:distrib_smooth_funct} we have $\widehat{w}(k) = \com{\sm{u , \phi}} $, with $\phi(x) = \esf^{-i x \cdot k}$, and the same for $\widehat{u}(k)$ and $\widehat{v}(k)$.
\end{lemma}


However \com{the definition} \sbar{result} \ref{lem:prod_distib_fourier} use the notion of Fourier transform which \com{cannot be generalized to $\Dcal'(\Mcal)$} \sbar{is dificult to generalize to the case of manifold,} in contrast to the wave front set as just seen above.
...
For that we need to \sbar{defined} \com{define} the space of distribution\com{s in} $\Dcal^\prime(\Mcal)$ with wave front set contained in a closed cone $\Sigma$, i.e.
%
\begin{equation*}
...
\end{equation}
%
can be extend\com{ed} to a sequentially continuous operator defined as 
%
\begin{equation*}
\com{\mathsf{op}:}\Dcal_{\Sigma_1}^\prime(\Mcal) \otimes \Dcal_{\Sigma_2}^\prime(\Mcal) \ \to  \ \Dcal_{\Sigma_3}^\prime(\Mcal) \ 
\end{equation*}
%
\sbar{where} \com{for every closed cones $\Sigma_1$ and $\Sigma_2$ such that
%
\[
...
The product of two \sbar{ditributions} \com{distributions}  $u\in\Dcal^\prime(\Mcal)$ and $v\in\Dcal^\prime(\Mcal)$ does exist if $\WF(u) \oplus \WF(v)$ does not contain terms of the form $(x,0) \in T^\ast\Mcal$ \com{furthermore, if $u\cdot v$ exists, 
\begin{equation*}
\WF(u \cdot v) \subseteq \WF(u) \oplus \WF(v) \cup \WF(u) \cup \WF(v) \ . 
\end{equation*} 
}
...
%%TODO EXOS
\begin{examples}[Examples of wave front sets of \com{some} distribution\com{s}]
\sbar{Let us denote \com{with} $\Xsf$ a subset of $\Rbb^n$ in these examples.}
\com{In this examples we shall fix an open subset $\Xsf$ of $\Rbb^n$ . }
\begin{itemize}
%
%
%
%
\item The product \com{of a} Dirac distribution $\delta\in\Dcal^\prime(\Rbb)$ with itself is not well defined, indeed 
\begin{equation*}
...
\end{equation*}
%
The quantization procedure we choose is a formal deformation of the (classical) pointwise product. The idea is to map the classical algebra to the same one with quantum corrections. For doing it we need to define quantum observables as power \sbar{seried} \com{series} in $\hbar$ having for coefficients classical observables. \com{Since we are working with formal power series no}\sbar{Any} convergence conditions is required. We summarize this new notion in the following definition.
...
\com{Notice that a}
\sbar{An} generic element $\Fsf \in \Fcal_\reg(\Mcal)[[\hbar]]$ can be written as
%
\begin{equation*}
...
\sbar{The quantum algebra is defined as the algebra of quantum observables to which we associate a new product, called the quantum product. It encodes the quantum structure \com{in which sense???s}. }
\com{The quantum algebra is obtained equipping $\Fcal_\reg(\Mcal)[[\hbar]]$ with a product which is called the quantum product and it is a formal deformation of the pointwise (classical) product.}
...
We notice that the power series in $\hbar$ in \eqref{eq:q_prod_reg} does not seem to converge. However\com{, usually the physically relevant} \sbar{we do work with} quantum observables \sbar{which do} have \com{only} a finite numbers of non vanishing functional derivatives, thus \com{at least for these elements} the series in the definition of $\star_\Delta$ product is always convergent. \com{Despite of this fact, the possibility to use formal power series will be crucial in the perturbative treatment of interacting observables.}


\bigskip


Let us \sbar{consider} \com{analyze} an example to illustrate the definition of the \sbar{$\star_\delta$} \com{$\star_\Delta$} product \sbar{We choose to work with} \com{and consider}  the following regular functional
%
\begin{equation*}
\Fsf(\phi) = \int_{\Mcal^2} \dsf x \ \dsf y \ f(x,y) \ \phi(x) \ \phi(y) \ ,
\end{equation*}
%
with $f \in \Dcal(\Mcal^2)$. We compute its first \com{and second functional }derivatives
%
\begin{eqnarray*}
...
\end{eqnarray*}
%
Therefore using \eqref{eq:q_prod_reg} we can write \sbar{explicitely} \com{explicitely}  the $\star_\Delta$ product \com{of $\Fsf$ with itself }as follow\com{s   .....     you need to write "as follows" everywhere.... please check it!!!!!}
%
\begin{eqnarray*}
...
\end{equation}
%
with $f \in \Dcal(\Mcal)$. \com{Its functional derivatives are }\sbar{It has for derivatives}
%
\begin{eqnarray*}
\Fsf^{(1)}(\phi) = 2 \ \phi(x) \ f(x) \ , \quad \Fsf^{(2)}(\phi) = 2 \ f(x) \ \delta(x,y) \ , \quad \mbox{and} \qquad \Fsf^{(3)}(\phi) = 0 \ .
\end{eqnarray*}
%
Then we write \sbar{explicitely} \com{ explicitly   } the $\star_\Delta$ product as follow.
%
\com{Antoine look at the following formula..... do you want to use $\Delta$ or $\Delta_+$ ???? I don't understand anything....}
\begin{eqnarray}
...
\end{eqnarray}
%
We see that we in both \com{what products are you using here????} product we have to consider pointwise product of distribution\com{s}. We need to know if they are well defined or not. For doing it we shall use lemma \ref{lem:prod_distrib_wf}. We work with local observables therefore knowing the wave front set of $\Delta$ \eqref{eq:wf_causal_prop} we have
%
\begin{equation}
(x,0) \in \WF(\Delta) \oplus \WF(\Delta) \ .
\end{equation}
%
Thus according to lemma \ref{lem:prod_distrib_wf} the pointwise product of the causal propagator\sbar{s} $\Delta$ is now not well defined among local functionals. 
...
Another candidate is the Hadamard distribution because in its wave front set there is an additional conditions which as we \sbar{shal} \com{shall} see permits to multiply it with itself. This so-called Hadamard distribution is a distribution which satisfies the following conditions.
%
\begin{enumerate}
...
\end{equation*}
%
with $\Theta$ the Heaviside function. $\Delta_\fsf$ shall be also called the Feynman propagator. It is well defined among regular functionals, however we \sbar{mean  working } \com{have to work} with local functionals. Therefore \com{we have to discuss the extension of time ordered products to more general functionals, }let us first consider an example.


We keep working with the same local functionals $\Fsf(\phi)$ defined in \eqref{eq:exo_loc_obs}. Using \eqref{eq:prod_time} we can write \sbar{explicitely} \com{ explicitly } the time ordered product for $\Fsf(\phi)$ with itself as follow
%
\begin{eqnarray*}
...
\begin{definition}[Explicit form of the two point function of an Hadamard state]
...
\end{equation*}
%
The distribution $\hsf_\epsilon$ is called Hadamrd parametrix and, $u, v, w$ the Hadamard coefficient.
\end{definition}
...
At each state $\omega$ we can associate a sesquilinear form on $\Acal$ defined as
%
\begin{equation*}
\omega(\Asf \Bsf^\ast) := (A,B)_\omega \ .
\end{equation*}
%
This form is degenerate, it thanks to this that we defined the so-called Gelfand ideal as
%
\begin{equation*}
...
%
\begin{equation}
\phi_\lambda = \lambda^{n} \ \phi\left(x_1,x_2(\lambda) , \dots , x_n(\lambda\right)) \ ,
...
\begin{example}
We consider the numerical disricbution $u \in \Dcal^\prime(\Rbb \backslash \{0\})$ defined as 
%
\begin{equation*}
u(x) = \frac{1}{\abs{x}} \ .
\end{equation*}
...
\begin{theorem}[Minimal substraction]
...
\end{equation*}
%
where $\pp$ is the principal part, and with the same scaling degree, $\sd(u_\ms) = \sd(u)$. The extension $u_\ms$ is called the minimal substraction of $u$.
\end{theorem}
...
\end{equation}
%
Moreover if the distribution $u \in \Dcal^\prime(\Rbb^n\setminus \{0\})$ is homogeneous of degree $\delta$, and $\delta$ is not an integer equal or smaller than $-n$, then $u$ has a unique extension $\exte{u} \in \Dcal(\Mcal^n)$ which is homogeneous of degree $\delta$, and $\sd(\exte{u})=\sd(u)$ \cite{hormander_analysis_1990}.
...
Wa shall here extend the concepts of extensions and scaling degree introduced in section \ref{p:CHAP_2_REG_NUMERRIC}
...
We can use this notion of scaling degree using the graphical method introduced in \ref{p:CHAP_2_PIC_REG_PB}. The Steinmann scaling degree of $\tsf_\gamma$ is equal to 
%
\begin{equation}
...
\label{p:CHAP_3_DIFFERENTIAL_EULER}
%----------------------------------------------------------------------------%
...
Here we present another approach to the regularization problem discussed in section \ref{p:CHAP_2_REG_PB}, by introducing a ``graphical way'' to write the time ordered product of different observables. To the treat the regularization issues we need to introduce a suitable regularisation scheme. 


First we shall The regularisation scheme we developed use the concept of minimal subtraction procedure that we shall define
...
\begin{definition}[An oriented/unoriented graph] 
 Any oriented graph $\Gamma$ is a set of vertices $V(\Gamma)$ and a set of edges $E(\Gamma)$ together with maps
 \begin{eqnarray}
  && \mathsf{s} \ : \ E(\Gamma) \ \to V(\Gamma) \quad \mathsf{(source)} \ ; \\
  && \mathsf{t} \ : \ E(\Gamma) \ \to V(\Gamma) \quad \mathsf{(target)} \ .
 \end{eqnarray}
 We give $\Gamma$ an orientation by assigning to any pair the value
 \begin{equation*}
  (\mathsf{e}:\mathsf{v}) \ \doteq \ 
  \left\{
  \begin{array}{cl}
   +1 & \mathsf{if} \quad \mathsf{t(e)=v} \\
   -1 & \mathsf{if} \quad \mathsf{s(e)=v} \\
   0 & \mathsf{otherwise}
  \end{array}
  \right.
 \end{equation*}
 A graph for which the orientation can be chosen freely is called an unoriented graph. We call $\mathsf{e}$ adjacent to $\mathsf{v}$ if $(\mathsf{e:v})\neq 0$. \\
 A graph for which the orientation map $(\mathsf{e},\mathsf{v}) \mapsto (\mathsf{e}:\mathsf{v})$ is multi valued is called a tadpole. \\
 
 \bigskip
 
 \emph{The set $\Gcal$.} \\
 Let $\Gcal$ denote the set of all unoriented graphs $\Gamma$ for which the orientation map is single valued. 
\end{definition}

\begin{definition} \emph{Epstein Glaser subgraph.} \\
 For $\Gamma \in \Gcal$, we define an Epstein Glaser subgraph $\gamma \subseteq \Gamma$ by
 \begin{equation*}
  \begin{array}{ll}
   \mathsf{the \ set \ of \ vertices} & V(\gamma) \subseteq V(\Gamma) \\
   \mathsf{the \ set \ of \ edges} & E(\gamma) = \left\{ e \in E(\gamma) \ | \ \left\{\mathsf{s(e)},\mathsf{t(e)}\right\} \subset V(\gamma) \right\}
  \end{array}
 \end{equation*}
 the orientation of $\gamma$ is inherited from $\Gamma$. 
\end{definition}

\begin{definition} \emph{BPHZ\footnote{BPHZ for Bogoliubov, Parasiuk, Hepp, and Zimmermann.} subgraph.} \\
 A BPHZ subgraph is any subgraph $\gamma \subseteq \Gamma$ given by a subset of
 
   the set of edges $E(\gamma) \subseteq E(\Gamma)$, 
   and all adjacent vertices $V(\gamma) = \left\{ v \in V(\Gamma) \ | \ \exists e \in E(\gamma) \ \mathsf{such \ that \ } (\mathsf{e}:\mathsf{v}) \neq 0 \right\}$ 
 
\end{definition}

The set of BPHZ subgraphs of a graph $\Gamma \in \Gcal$ is a superset to the set of Epstein-Glaser subgraphs. We can associate to any BPHZ subgraph a unique Epstein-Glaser
subgraph. 

\begin{definition} \emph{Full vertex part of a BPHZ subgraph and pure BPHZ subgraph.} \\
 Given a graph $\Gamma$ with BPHZ subgraph $\gamma \subseteq \Gamma$, we define the full vertex part $\overline{\gamma}$ of $\gamma$ to be the graph with
 
   the same set of vertices, $V(\overline{\gamma})=V(\gamma)$, 
   and all lines in $\Gamma$ connecting them, $E(\overline{\gamma})=\left\{ e \in E(\Gamma) \ | \ \mathsf{s(e)},\mathsf{t(e)} \in V(\Gamma)\right\}$ . 
 
 $\overline{\gamma}$ is an Epstein-Glaser subgraph. Any BPHZ subgraph, which is not a full vertex part, is called a pure BPHZ subgraph.
\end{definition}
...
\begin{eqnarray*}
(\Fsf \cdot_{\Tsf} \Gsf)(\phi) &=& \FG + \hbar \FoneG + \hbar^2 \FtwoG + \hbar^3 \FthreeG + \Ocal(\hbar^3)
\end{eqnarray*}


\begin{eqnarray*}
(\Fsf_1 \cdot_{\Tsf} \Fsf_2 \cdot_{\Tsf} \Fsf_3)(\phi) &=&
...
\end{eqnarray}
%
where at the last point the linearity of the operators $\Rsf_I$ is used, furthermore in the product over $I\in F$, the operator $\Rsf_I$ has to be applied before $\Rsf_J$ if $I\subset J$.
%
The limit $\alphabd \to 0$ is taken in two steps. First we set $\alpha_{I}= \alpha_\gamma$ for $I \in F$ before taking the sum over all forests. Second the limit $\alpha_\gamma \to 0$ is performed.
\end{theorem}
