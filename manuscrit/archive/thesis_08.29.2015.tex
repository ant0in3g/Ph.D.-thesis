...
% Algebraic and Noncommutative approaches to Quantum Field Theory.
% --
...
We therefore have now all the necessary ingredients to get a clear idea of the integral over $\Rbb^n$ in \ref{eq:int_manifold}.
...
We assume the universe is homogeneous and isotropic, which is the case on large scales. Isotropy means that the metric must be diagonal. We consider a spatially flat Friedmann Robertson Walker spacetime, i.e. a subset of $\mathbb{R}^4$ with the metric
\begin{equation*}
 ds^2 = dt^2 - a(t)^2 d\vec{x}^2 \ .
\end{equation*}
We notice that this metric is invariant under translation and rotation of the space coordinates. We have chosen a FRW\footnote{FRW for Friedmann, Robertson, and Walker.} spacetime without spatial curvature in order to simplify computations and because observations are compatible with the assumptions of vanishing spatial curvature. The variable $t$ is the cosmological time, whereas $a(t)$ is the scale factor whose expansion rate is the Hubble rate
\begin{equation*}
...
The definition \ref{def:spacetime_supp} shows that a functional $\Fsf$ does not ``feel'' fields which localized outside $\supp(\Fsf)$. The spacetime support of $\Fsf$ is the subset of $\Mcal$ where $\Fsf$ does see the ``fluctuations'' of the fields configuration.
...
\subsection{blablab intro}

\subsection{fourier}

\subsection{wave front set}

First we shall look at the case of $\Rbb^n$ and then generalize to $\Mcal$. We adopt as far as possible the notation of L. Hörmander in \cite{hormander_analysis_1990}. The set $\Xsf$ denotes for now on a subset of $\Rbb^n$. 


\bigskip


A \textbf{distribution}\index{distribution} $u$ on $\Xsf$ is a linear form, i.e. a linear map from a vector space to a scalar, on $\Dcal(X)$ such that for every compact set $K \subset \Xsf$ there exist constants $C$ and $k$ such that
%
\begin{equation*}
\abs{u(\phi)} \leq C \sum_{\abs{\alpha} \leq k} \sup \abs{\partial^\alpha \phi} \ , \quad \phi \in \Dcal(X) \ .
\end{equation*}
%
The set of all distribution in $X$ is denoted by $\Dcal^\prime(X)$. Then there exist different characterizations of a distribution which are form practical point of view useful to show that an object is a distribution. They are presented in \cite{hormander_analysis_1990} with their proof.


\bigskip


If $u\in\Dcal^\prime(X)$ then the support of $u$, denoted $\supp(u)$,\index{distribution!support} is the set of point in $\Xsf$ having no open neighborhood to which the restriction of $u$ is $0$. Then we define the singular support of $u$, $\singsupp(u)$, \index{distribution!singular support} as the set of points in $X$ having no open neighborhood to which the restriction of $u$ is a smooth function. 


The wave front set gives a more precise description of singularities than the singular support. It tells us not only at what points a singularity occurs, but it also indicates the directions in the dual space from which the singularities are coming, i.e. the direction for which does not hold. Before defining the wave front set of a distribution we will recall some notions.
...
A very useful tool for the distribution is the wave front set. Let $u$ : $\mathcal{D}(\mathbb{R}^{n}) \rightarrow \mathbb{R}^{n}$ be a ditribution. We call the singular support of the the distribution $u$ all the points $x$ where $u$ is not smooth in the neighborhoods of $x$. So the wave front set of $u$ is the set $(x,k)$  which are contained in $T^{*}\mathbb{R}^{n} \ {0}$, with $x$ in the singular support of $u$, and $k$ such that for every $f$ the fourrier transform of $fu$ does not decay rapidly in direction $k$. We know that the multiplication of two distributions $u$ and $v$ is not soemething easy to define. The wave front set give us a very powerfull tool to know if $u.v$ is defined, and if it is this case it's unique. So if $WF(u) \oplus WF(v) \notin 0$, then $u.v$ is well defined. A problem that we had if the multiplication of distributions. Before to think of a possible regularisation, we need a criterion which tells us when and where this product can be defined. This criterion has been formulated using the notion of wave front set. First let us recall the definition of a distibution. As Schwartz did, we will use the notation $\Ecal(X)$ for the space $\Ccal^\infty(X)$, and $\Dcal(X)$ for the space $\Ccal^\infty_\circ(X)$. 




The wave front set of a distribution $u \in \Dcal^\prime(\Mcal^n)$, roughly speaking, is a conic subset of the cotangent bundle $\WF(u) \subset \Tsf^\ast(\Mcal^n)\backslash\{0\}$ where the first component gives the singular support, $\mathsf{singsupp}(u)$, and the second gives the direction in which the Fourier transform of $u$ does not decrease rapidely. It is possible to show that the Fourier transform of a smooth compaclty supported function $u(x)$ decays faster than any negative power of the dual variable $\xi$, i.e. for every number $N$ there exists a constant such that
%
\begin{equation*}
 \abs{\hat{u}(\xi)} \leq C (1 + \abs{\xi})^{-N} \ . 
\end{equation*}
%
We can reformulate this property saying that if the Fourier transform of a distribution with compact support satisfies  then this distribution is induced by a smooth map. Therefore  can be seen as a characteristic property for smoothness. We will need the notion of singular support of a distribution, therefore we recall its definition. 




Since every point in $X \backslash \mathsf{singsupp}(u)$ has a neighborhood where $u$ is a $\Ecal$ function, it follows that the restriction of $u$ to $X \backslash \mathsf{singsupp}(u)$ is a $\Ecal$ map. 


\begin{definition}
A set $V \subset \Rbb^n \backslash \{0\}$ is called a conic set if, together with any point $\xi$, it contains all the points $t \xi$ with $t > 0$. A conic set is completely determined by its intersection with the unit sphere $S^{n-1}$ in $\Rbb^n$. By a conic neighborhood of a point $\xi \in \Rbb^n \backslash \{0\}$ we mean an open conic set that contains $\xi$. 
\end{definition}
%
We can now define what we will call the dual singular support of a distribution.


\begin{definition}[Dual singular support]
Let $u \in \Dcal^\prime(X)$ with $X \subset \Rbb^n$. We define the set $\Sigma(u) \in \Rbb^n \backslash \{0\}$ by saying that $\xi \notin \Sigma(u)$ if there exists a conic neighborhood $V$ of $\xi$ such that  holds in $V$. 
\end{definition}


It follows immediately that $\Sigma(u)$ is a closed conic set, and that the distribution $u$ is induced by a smooth function if and anly if $\Sigma(u) = \emptyset$. We will recall some definitions and theorem for the Fourier transform. 
...
%\begin{definition}
 If $f \in L^1(\Rbb^n)$ then the Fourier transform $\hat{f}$ is the bounded continuous function in $\Rbb^n$ defined by
 \begin{equation*}
  \hat{f}(\xi) = \int dx \ \esf^{-i\sm{x,\xi}} f(x) \ , \quad \xi \in \Rbb^n \ .
 \end{equation*}
 If $\hat{f}$ also happens to be integrable one can express $f$ in terms of $\hat{f}$ by Fourier's inversion formula
 \begin{equation*}
  f(x) = (2\pi)^{-n} \int dx \ \esf^{i\sm{x,\xi}} \hat{f}(\xi) 
 \end{equation*}
 so $\hat{f}(\xi)$ is the density of the character $\esf^{i\sm{x,\xi}}$ in the harmonic decompositionof $f$. 
%\end{definition}

To study the Fourier transform we shall consider functions in a subset of $\Ecal$ containing $\Dcal$.

%\begin{definition}
By $\Scal$ or $\Scal(\Rbb^n)$ we denote the set of all $\phi \in \Ecal(\Rbb^n)$ such that
\begin{equation*}
...
%\begin{theorem} 
The Fourier transform $F : \phi \to \hat{\phi}$ is an isomorphism of $\Scal$ with inverse given by Fourier's inversion formula .  
%\end{theorem}


We shall now prove some fundamental properties of the Fourier's transformation on $\Scal$.

%\begin{theorem} 
 If $\phi$ and $\psi$ are in $\Scal$, then
 \begin{eqnarray*}
  \int \hat{\phi} \psi &=& \int \phi \hat{\psi} \\
  \int \phi \overline{\psi} &=& (2\pi)^{-n} \int \hat{\phi} \overline{\hat{\psi}} \\
  \widehat{\phi \ast \psi} &=& \hat{\phi} \hat{\psi} \\
  \widehat{\phi\psi} &=& (2\pi)^{-n} \hat{\phi} \ast \hat{\psi} \ ,
 \end{eqnarray*}
 where $\ast$ indicate the convolution and the horizontal bar the complex conjugation. 
%\end{theorem}

%\begin{definition}
 A continuous linear form $u$ on $\Scal$ is called a tempered distribution. The set of all temperate distibutions is denoted by $\Scal^\prime$.  
%\end{definition}

%\begin{definition}
 If $u \in \Scal^\prime$, the Fourier transform $\hat{u}$ is defined by 
 \begin{equation*}
  \hat{u}(\phi) = u(\hat{\phi}) \ , 
 \end{equation*}
 for $\phi \in \Scal$. 
...
Then we can show that $\hat{u} \in \Scal^\prime$. The Fourier inversion formula as proved in theorem %\ref{thm:fourier-isomorphism} 
states that
\begin{equation*}
 \hat{\hat{\phi}} = (2\pi)^n \check{\phi} \quad \mbox{if } \ \phi \in \Scal \ \mbox{with } \ \check{\phi}(x) = \phi(-x) \ .
\end{equation*}
If $u$ is in $\Scal^\prime$ we obtain 
\begin{equation*}
 \hat{\hat{u}}(\phi) = u(\hat{\hat{\phi}}) = (2\pi)^n u(\check{\phi}) = (2\pi)^n \check{u}(\phi) \ .
\end{equation*}
Here $\hat{u}$ is the composition of $u$ and $x \to - x$. Therefore we have the following theorem.

%\begin{theorem}
 The Fourier transformation is an isomorphism of $\Scal^\prime$, and the Fourier's inversion formula $\hat{\hat{u}} = (2\pi)^n \check{u}$ is valid for every $u \in \Scal^\prime$.  
%\end{theorem}

%\begin{theorem}
 The Fourier transform of a distribution $u \in \Ecal^\prime(\Rbb^n)$ is the function
 \begin{equation*}
  \hat{u}(\xi) = u_x\left(\esf^{-i\sm{x,\xi}}
  \right)
 \end{equation*}
 where the index $x$ indicates that the integration is over the variable $x$. The right hand side is also defined for every complex $\xi \in \Cbb^n$ and is an entire analytic function of $\xi$, called the Fourier Laplace transform of $u$. 
%\end{theorem}

Now we come to our goal which is to define the wave front set of a distribution. But first we still need to introduce few results conserning the dual singular support.
...
%\begin{lemma}
 Let $u \in \Ecal^\prime(\Rbb^n)$ and $\phi_1, \phi_2 \in \Ecal(\Rbb^n)$. Suppose that $\phi_2(x) \neq 0$ when $x \in \supp(\phi_1)$. Then $\Sigma(\phi_1 u) \subset \Sigma(\phi_2 u)$. 
%\end{lemma}

If $X$ is an open set in $\Rbb^n$ and $u \in \Dcal^\prime$ we set for $x\in X$
\begin{equation*}
 \Sigma_x(u) = \underset{\phi}{\bigcap} \Sigma(\Phi u) \ , \ \ \phi \in \Dcal(X) \ , \ \ \phi(x) \neq 0 \ ,
\end{equation*}
as an intersection of closed conic sets. $\Sigma_x(u)$ is also a closed conic set. 

%\begin{theorem}
 Let $u \in \Dcal^\prime(X)$ with $X \subset \Rbb^n$, and $\Gamma$ be a conic neighborhood of $\Sigma_x(u)$. Then there exists a neighborhood $U$ of $x$ such that $\Sigma(\phi u) \in \Gamma$ for every function $\phi(x) \in \Dcal(U)$. 
%\end{theorem}

In fact we have 
\begin{equation*}
 \lim_{\supp(\phi) \to \{x\}} \Sigma(\phi u) = \Sigma_x(u) \ , \quad \mbox{with } \ \phi(x) \neq 0 \ .
\end{equation*}
With this remark we can now define the wave front set of a definition.

%\begin{definition}[Wave front set]
 If $u \in \Dcal^\prime(X)$, then the closed subset of $X \times (\Rbb^n \backslash 0)$ defined by
 \begin{equation*}
  \WF(u) = \bigg\{ (x,\xi) \in X \times (\Rbb^n \backslash 0) \ ; \ \xi \in \Sigma_x(u) \bigg\}
 \end{equation*}
 is called the wave front set of $u$. The projection in $X$ is $\mathsf{singsupp}(u)$. 
%\end{definition}

The set $\WF(u)$ is conic in the sence that it is invariant under multiplication of the second variable by positive scalars. It could therefore be considered as a subsetof $X\backslash S^{n-1}$ where $S^{n-1}$ is the unit sphere.

%\begin{proposition}
 If $u \in \Ecal^\prime(\Rbb^n)$ then the projection of $\WF(u)$ on the second variable is $\Sigma(u)$. 
%\end{proposition}
...
\end{equation*}
%
where $\Delta_+$ is a Hadamard distribution of the linear theory we are going to quantize, namely a distribution whose antisymmetric part is proportional to the commutator function $\Delta=\Delta_\rsf-\Delta_\asf$ and whose wave front set satisfies the Hadamard condition, see e.g. %\cite{Radzikowski, bfk:1996}
for further details and Section %ref{sec_propagators}
for our propagator conventions. $\star_H$ is associative.
...
We shall now look to interacting quantum field theory in a perturbative way. It is necessary in order to be able to work with non linear fields. The functional approach shall still be used. The aim of this chapter is to intoduce the fundations of perturbative algebraic quantum field theory, in order to identify precisely the renormalization problem.


\bigskip


We shall present the perturbative construction of an interacting quantum field theory on a generic curved spacetime $\Mcal$. The framework of perturbative algebraic quantum field theory (\textbf{pAQFT}) has been recently developed in \cite{brunetti_perturbative_2009,fredenhagen_perturbative_2015,fredenhagen_batalin-vilkovisky_2013} based on earlier work. In particular it used notions of microlocal analysis that we shall present.
...
\begin{definition}[Hadamard state]
 Let $\omega$ be a state a state on the Borchers Uhlmman algebra. We say that $\omega$ fulfils the Hadamard condition and is therefore a Hadamard state if its two point $\omega_2$ fulfils
 \begin{equation*}
...
\begin{definition}[Local Hadamard form]
 Let $\omega_2$ be the two point function of a state on the Borchers Uhlmman algebra, let $t$ be time function on $(\Mcal,g)$, let
 \begin{equation*}
  \sigma_\epsilon(x,y) \ \doteq \ \sigma(x,y) \ + \ 2i\epsilon \left( t(x) - -t(y) \right) \ + \ \epsilon^2 \ ,
 \end{equation*}
 and let $\lambda$ be an abitrary length scale. We say that $\omega_2$ is of local Hadamard form if, 
 \begin{center}
  \emph{
  for every $x_0 \in \Mcal$ there exists a geodesicaly convex neighbourhood $\Ocal$ of $x_0$ such that \\
  $\omega_2(x,y)$ on $\Ocal \times \Ocal$ is of the form
  }
 \end{center}
 \begin{eqnarray*}
  \omega_2(x,y) &=& \lim_{\epsilon \to 0} \ \frac{1}{8\pi^2} \bigg( \frac{U(x,y)}{\sigma_{\epsilon}(x,y)} + V(x,y) \log\left( \frac{\sigma_{\epsilon}(x,y)}{\lambda^2} \right) + W(x,y) \bigg) \\
  &=& \lim_{\epsilon \to 0} \ \bigg( H_\epsilon(x,y) + W(x,y) \bigg)  \ .
 \end{eqnarray*}
 Here, the Hadamard coefficients $u$, $v$, and $w$ are smooth, real valued biscalars, where $v$ is given by a series expansion $\sigma$ as
 \begin{eqnarray*}
  V(x,y) \ = \ \sum_{n=0}^{\infty} \ V_n \ \sigma^n(x,y)  \ ,
 \end{eqnarray*}
 with smooth biscalar coefficients $v_n$. The bidistribution $H_\epsilon$ shall be called Hadamard parametrix, indicating that is solves the Klein Gordon equation  up to smmooth terms. 
\end{definition}
...
% \vspace{-8pt}
% \centering
% \QuadrantsComplexPlane
% \caption{The quadrants of the complex plane.}
% \label{fig:QuadrantsComplexPlane}
%\end{figure}
...
%----------------------------------------------------------------------------%
\chapter{Moyal field theory on a nutshell}
%----------------------------------------------------------------------------%
