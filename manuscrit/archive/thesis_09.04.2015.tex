...
Let us first look at the situation on Minkowski spacetime $\Mbb$. As we know now the Borchers Uhlmman algebra of the free scalar field contains simple observables. It considers only linear combinations of products of free fields at separate points. But we wwould like to study the interacting situation, we need to make sense of product of fields at the same point. It is done via a notion called normal ordering. We illustrate the necessity of the normal ordering with the two point function of the massless scalar field on Minkowski spacetime. 
%
\begin{equation}
...
We will define a time ordered product between observables. It is done in a similar way we defined the $\star$ product. To avoid problems we will first build this product for regular functionals. The time ordering between two functionals is defined as follow
%
\begin{equation*}
\Fsf \cdot_T \Gsf = 
\left\{
...
In the free case we worked only with regular functionals, but they do not contain the interaction functionals, those functionals that we would like to work with. Therefore we have to defined a new space of functionals, it is done by imposing a less restrictive condition on the wave front set, we set that the wave front set of $\Fsf^{(n)}$ does not intersect the set $\mathcal{M} \times (\overline{V^n_+} \cup \overline{V^n_-})$ where $\overline{V_\pm}$ denotes the closed forward and backward light cone, respectively. It forms the space of microcausal functional $\mathcal{F}_\mathsf{\mu c}(\Mcal)$. And this time the interaction functionals are contained in it.


\begin{definition}[Space of microcausal functional]
We define the space of microcausal functionals as follow
%
\begin{equation*}
...
A generic local functional is a polynomial $P(\phi)(x)$ in $\phi$ and its derivatives integrated against a smooth and compactly supported tensor. The functionals whose functional derivatives are compactly supported smooth functions are instead called regular functionals and indicated by $\mathcal{F}_\reg$.


\bigskip


The quantum theory is specified once a product among elements of $\mathcal{F}_\loc$ and a $*-$operation (an involution) are given. For the case of free (linear) theories the product can be explicitly given by a $\star$ product
%
\begin{equation*}
(\Fsf \star_\Delta \Gsf)(\phi) = \Fsf(\phi) \cdot \Gsf(\phi) + \sum_{n=1}^\infty \frac{\hbar^n}{n!} \sm{ \Fsf(\phi)^{(n)}, \Delta_+^{\otimes n} \Gsf(\phi)^{(n)}} \ ,
\end{equation*}
%
where $\Delta_+$ is a Hadamard distribution of the linear theory we are going to quantize, namely a distribution whose antisymmetric part is proportional to the commutator function $\Delta = \Delta_\rsf - \Delta_\asf$ and whose wave front set satisfies the Hadamard condition

\bigskip

Owing to the properties of $H_+$, iterated $\star_H$--products of local functionals $F_1 \star_H \dots \star_H F_n$ are well defined and $\star_H$ is associative.
