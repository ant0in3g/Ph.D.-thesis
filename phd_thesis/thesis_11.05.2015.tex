...
%----------------------------------------------------------------------------%
\section*{Quantum field therory and regularization}
\addcontentsline{toc}{section}{Quantum field therory and regularization}
%----------------------------------------------------------------------------%


The physical sytem most well known is maybe the oscillator harmonic, but it has a finite degree of freedom. However more complicated systems have attracted attentions, in particular systems with an infinite degree of freedom. We can naively imagine to appropriate a such system with a large number of oscillator harmonic. Knowing completely the physics of one should permit us to determine the physcis of a large number, but the computation appears to be to be simply too complicated. Those systems could not be treated as usual, the notion of fields had to be introduced. Fields are objects which associate a value to every point in space and time. One on the most popular example of field is the electromagnetic field governed by Maxwell’s equations, where the theoretical predictions coincide wery well with the experimental results. At this point quantum structure was remaining to add in order to be able to describe atomic physics and electromagnetism with the same framework. It is what we call now quantum field theory on Minkowski spacetime. Minkowski spacetime is a four dimensional space where the coordinates denote the three spatial coordinates and the time. It is an euclidean space of dimension four endowed with a metric of Lorentzian signature. On this spacetime it is possible to define a vacuum, therefore a perturbation of the theory around this lowest state of energy permit to go beyong the case of free theory and treat interactions. Nonetheless in the perturbative treatment arise divergences, the infrared (IR) one at very low energy and ultraviolet (UV) at very high energy. This problems are known as the regularization problems. In order to treat (UV) divergences on Minkowski spacetime different methods are available, the momentum cuttoff, the Pauli Villar \cite{PV_1949}, the analytic \cite{S_1968}, and the dimensional regularizations. The latest attracted our atention, it has benn developed by Bollini and Giambiagi \cite{BGP_2008}, and, 't Hooft and Veltman \cite{HV_1972}.\par%


In the present work we shall work with quantum field on curved spacetime, where the spacetime is treated as a Lorentzian manifold with some further assumptions. Quantum field theory on curved background can be view as a first to enlarge the theory to general realtivity, or more modestly just to a generalization of theory in Minkowski spacetime. In these situation the perturbative construction of models encounters time--ordered products of field polynomials which are a priori ill--defined due to the appearance of UV divergences. Several renormalization schemes which deal with these divergences in the presence of non--trivial spacetime curvature have been discussed in the literature, such as for example local momentum space methods \cite{B_1981}, dimensional regularisation in combination with heat kernel techniques \cite{L_1982,toms_renormalization_1982}, differential renormalisation \cite{comellas_generic_1995,prange_epstein-glaser_1999}, zeta--function renormalisation \cite{bilal_multi-loop_2013}, generic Epstein--Glaser renormalisation \cite{brunetti_microlocal_2000,hollands_local_2001,HW_2005}, and, on cosmological spacetimes, Mellin--Barnes techniques \cite{hollands_correlators_2010} and dimensional regularisation with respect to the comoving spatial coordinates \cite{baacke_coupled_2010}. Some of these schemes, such as heat kernel approaches, zeta--function techniques and local momentum space methods  are based on constructions which are initially only well--defined for spacetimes with Euclidean signature. These constructions can be partly transported to general Lorentzian spacetimes by local Wick--rotation techniques developed in \cite{moretti_proof_2000}. However, whereas the Feynman propagator is essentially unique on Euclidean spacetimes, this is not the case on Lorentzian spacetimes where this propagator has a non--unique contribution depending on the quantum state of the field model. Consequently, the Euclidean renormalisation techniques are able to capture the correct divergent and geometric parts of Feynman amplitudes, but a priori not their non--geometric and state--dependent contributions.
...
During these three years i also worked on another pojects. In praticular i was interested with the noncommutative approach to quantum field theory. These works has been done in colaboration with Tajron Juri\'c\footnote{Ru\dj er Bo\v{s}kovi\'c Institute, Theoretical Physics Division, Zagreb, Croatia}, Patrizia Vitale\footnote{Dipartimento di Fisica, Universit\`a di Napoli Federico II, Italy}, and Jean-Christophe Wallet\footnote{Laboratoire de Physique Th\'eorique, CNRS and Universit\'e Paris-Sud 11, Orsay, France}. The results have been subject to three articles, \cite{GW_2014}, \cite{GJW_2015}, and \cite{GVW_2014}.
...
\end{equation*}
%
It has been shown in \cite{BGP_2008} that the \textbf{operator} $\Psf$ has unique \textbf{retarded} $\Delta_\rsf$ and \textbf{advanced} $\Delta_\asf$ \textbf{fundamental solutions}. In other words, there are unique continuous maps 
%
\begin{equation*}
...
This previous definition of Hadamard states is abstract and inconvenient for computations. We shall give another definition more computationally friendly, which are equivalent \cite{R_1996}.
...
We shall present now the perturbative construction of an interacting quantum field theory on a generic curved spacetime $\Mcal$. We shall work in the framework of perturbative algebraic quantum field theory (\textbf{pAQFT}) which has been recently developed in \cite{brunetti_perturbative_2009,fredenhagen_perturbative_2015,fredenhagen_batalin-vilkovisky_2013} and it is based on earlier ideas of Steinmann, Epstein, and Glaser \cite{S_1971,EG_1973}.
...
%
Notice that this split of $\Delta$ is not unique. However, thanks to the work of Radzikowski \cite{R_1996} two different Hadamard distributions differs only by a smooth function. 
...
The renormalization scheme we propose is inspired by the works \cite{keller_dimensional_2010,DFKR_2014} recalled in section \ref{p:REG_PB}, which deal with perturbative QFT in Minkowski spacetime. In these works, the authors introduce an analytic regularization of the position space Feynman propagator in Minkowski spacetime which is similar to the one discussed in \cite{BGP_2008}. 
...
\end{equation*}
%
$G(\lambda, \cdot)$ is again a family of distributions on $\Ncal_n$ which depends smoothly on $\lambda$ for positive $\lambda$. Furthermore, $\lambda^a G(\lambda, \phi)$ vanishes in the limit $\lambda \to 0$ for every $a > s_2$ and every $\phi\in\Dcal(N_n)$. Hence, $\lambda^{\alpha-1} G(\lambda, \cdot)$ tends to $0$ in $\Dcal^\prime(\Ncal_0)$ for $\lambda\to 0$ and, additionally, the Banach--Steinhaus theorem \cite{B_1987} implies that 
%
\begin{equation}
...
The correct recursion procedure is implemented by the so called \textbf{Epstein Glaser forest formula}, which is a position space analogue of the Zimmerman forest formula \cite{DFKR_2014}. 
...
\begin{remark}
The construction we are going to present, namely the construction of certain distributions by means of powers of the geodesic distance, is in some way similar to the extension of Riesz distributions to curved spaces presented in \cite{BGP_2008}. In particular, here we shall discuss the boundary value of 
%
\begin{equation*}
...
We conclude the general analysis of the regularization scheme introduced in this work by demonstrating that this scheme satisfies (up to one property we shall mention at the end of this section) all axioms of \cite{hollands_local_2001,hollands_existence_2002,HW_2005} that, as argued by Hollands and Wald, any physically meaningful scheme to regularize time ordered products should satisfy. These axioms are recalled in section \ref{p:INT_Q_ALG}. In addition to showing these properties of the scheme, we also argue that it preserves invariance under any spacetime isometries present.
...
In particular we can use the mode expansion in order to give explicit expressions for the various propagators of the free Klein-Gordon quantum field in a pure, Gaussian, homogeneous and isotropic state $\Omega$ (see \cite{P_2011,zschoche_chaplygin_2014}
