...
However we shall rather quantize the classical theory with the ideae to give a large importance to the algebra structure, instead of using the symplectic structure just introduced above. The procedure used is the formal deformation present in section \ref{p:CHAP_2_Q_DEFORM}. Classical and quantum observables shall be seen as in the same vector sapce but endowed with different products, which will give different alegbras. Therefore we now define the classical off shell $\ast$-algebra of a free field theory on regular functionals, where the involution is defined as follow
%
\begin{equation}
...
We shall construct observables of the interacting theory in section \ref{p:CHAP_2_INT_Q_ALG} by means of perturbation theory, namely when the free algebra is perturbed by a non linear local potential. However, up to now, we have worked with regular functionals only, unfortunately the case of interacting potentials constructed with regular functionals are of limited interest. We aim to be able to work with local non linear interacting potentials like
%
\begin{equation}
...
\end{eqnarray}
%
We see that a pointwise product of distributions is present, and as we know such products can be ill defined. In order to know if this pointwise product is well defined or not we shall use the notion of wave front set introduced in section \ref{p:CHAP_2_SING_WF} and in particular the result from lemma \ref{lem:prod_distrib_wf}. The wave front set of $\Delta_+$ is given by \eqref{eq:wf_hadamard}, the additional requirement $k_x \triangleright 0$ prevents the point $(x,0)$ to be contained in $\WF(\Delta_+) \oplus \WF(\Delta_+)$. Therefore owing lemma \ref{lem:prod_distrib_wf} the pointwise product is well defined.
\end{example}
...
\end{equation}
%
where $\supp(\Fsf)$ is said to be  later than   $\supp(\Gsf)$ if $J^+(\supp(\Fsf)) \cap \supp(\Gsf)=\emptyset$. Let us look at this product for linear functionals. Let us consider
%
\begin{equation*}
\Fsf(\phi) = \int_\Mcal \dsf x \ \sqrt{\abs{g}} \ f(x) \ \phi(x) \ , \qquad \Gsf(\phi) = \int_\Mcal \dsf x \ \sqrt{\abs{g}} \ g(x) \ \phi(x) \ .
\end{equation*}
...
\left\{
\begin{array}{ll}
\Delta_+(f,g) \quad \mbox{if } \ \supp(f) \ \mbox{ is later than  } \ \supp(g)  \\
\Delta_+(g,f) \quad \mbox{if } \ \supp(g) \ \mbox{ is later than  } \ \supp(f) 
\end{array}
\right. \ ,
\end{equation*}
%
...
%
\begin{equation}
\Delta_\fsf(f,g) = \int_\Mcal \dsf x \ \dsf y \ \bigg( \Theta(t_x-t_y) \Delta_+(x,y) + \Theta(t_y-t_x) \Delta_-(x,y) \bigg) f(x) g(y) \ .
...
\end{equation*}
%
Therefore the product is well defined according to lemme \ref{lem:prod_distrib_wf}. Let us give the \hilight{wave front set of $\Delta_\fsf$} as you can expect is more involved than the wave front set of $\Delta_+$,
%
\begin{equation}
\WF(\Delta_\fsf) = \ .
\label{eq:wf_feynman}
...
Because the construction of interacting field theory with regular is of limited interest, we shall in the next section \ref{p:CHAP_2_INT_Q_ALG} look at the situation for microcausal functinals, and in particular local functinals.
...
\begin{example}
We compute here the two point function of the interacting field in a gaussian Hadamard state $\omega$ of the free field, cf section \ref{p:CHAP_2_STATES}. We shall consider a potential, look at \eqref{eq:obs_pot_calculus-2pf}, and perform the computation up to the second order in $\lambda$. \par
%
%
...
Due to the causal factorisation property stated as one of the axioms that every time ordered product has to satisfy presented in section \ref{p:CHAP_2_INT_Q_ALG}, the time ordered product \eqref{eq:time_ordered_op} is well defined if the support of $\Fsf_1, \ ... \, \Fsf_n$ are pairwise disjoint
%
\begin{equation*}
...
In this section first we shall extend the time ordered product up to the total diagonal, i.e. when all supports of $\Fsf_1, \ ... \, \Fsf_n$ coincide. Second we will characterize this remaining divergence and recall a way to regularize it.


\com{Describe better the content of this section... add references to ideas of Epstain and Glaser and to Steinmann. }
...
We consider $\forall i \in \{1,\dots,n\}$ the functionals $\Fsf_i(\phi) \in \Fcal_\Tsf(\Mcal)[[\hbar]]$. The tme ordered product shall be written using the map $\Tcal_n$ defined in section \ref{p:CHAP_2_INT_Q_ALG} that we recall here
%
\begin{equation*}
\Tcal_n \ : \ 
\left\{
...
\com{I think in that case you can say that you can extend the distribution by continuity... maybe you can say something more...}
...
\end{equation*}
%
where $\omega_\gamma$ are smooth compactly supported functions such that $\partial^\gamma\omega_\beta(0) = \delta^\gamma_\beta$. Then for any distribution $u\in\Dcal^\prime(\Rbb^n\setminus\{0\})$ there exits extensions with the same scaling degree, which given the $W$--projection is defined on the functions $\omega_\gamma$. \com{I don't understand the previous sentence}


\com{What happens if you change $r_W$? how are related the different extensions }

\com{ Before discussing other regularization methods, you should discuss the freedom.... }

%\bigskip
\newpage

\com{Say what you want to do now! You want to present an altarnative way to extend distributions.}
Let us now define the \textbf{regularization} of a distribution $u \in \Dcal^\prime(\Mcal^n\setminus \{0\})$. It is a family $\left\{ u^{\alpha}\right\}$ of distributions $u^{\alpha}\in\Dcal^\prime(\Mcal^n)$ if
%
\begin{equation*}
...
In the next chapter, we shall use a similar method to extend the distributions which defines the time ordered products. However, we already notice in this example that the extraction of the possible pole of an analytic regularization is not an easy task.


\newpage


\com{What do you want to say now? Do you want to discuss the extension of homogeneous distributions???? I don't understand the logic}
We notice that the distributions in the time oredered products \com{in this section you are discussing extension of distributions over a point. This discussion has nothing to do with time ordered product, so you should not write time ordered products here} are not homogeneous but can still satisfy a \sbar{sacale} \com{scale} property.
