...
\end{equation*}
%
for all $\phi \in \Dcal(\Ncal)$. An important remark is that a distribution on a manifold is completely supported by its restriction. Moreover as know $\Mcal$ looks locally like $\Rbb^n$, thus some definitions and properties obtained for distributions on $\Rbb^n$ are expected ``to be transfered'' to the case of a manifold. In particular \eqref{eq:distibution_function} can be extended to $\Mcal$ owing on Lorentzian manifold we have a distinguished volume element (cf. section \ref{p:INTEGRATION}).
...
\end{equation*}
%
with $k \in \Nbb \setminus \{0\}$, and where the first equality is obtained after performing $k$ successive integrations by part. A second example can be presented with the Heaviside function for whic we associate the distribution $\Theta$ as follow
%
\begin{equation*}
H(x) = \left\{
\begin{array}{l}
...
\end{equation*}
%
with $\phi$ suitable test function. We compute its derivative as
%
\begin{equation*}
...
%
\begin{equation}
\phi \in \Dcal(\Rbb^n) \ \  \Longleftrightarrow  \ \ \abs{\hat{\phi}(k)} \leq C (1+\abs{k})^{-n} \ , \ \mbox{ for } \ C \in \Rbb \ , \ \ n \in \Nbb \setminus \{0\} \ , \ \mbox{ and } \ \ k \in \Rbb^n \ .
...
%
\begin{equation}
\abs{\sm{ u , \phi(x) }} \leq C (1+\abs{k})^{-n} \ , 
\label{eq:smooth_criterion_distrib}
\end{equation}
for $C \in \Rbb$, $n \in \Nbb \setminus \{0\}$, and $k\in \Rbb^n$.
\end{lemma}
...
\end{equation*}
%
for $\phi \in \Scal(\Rbb)$, thus $\hat{\delta}=1$. The inversion formula \eqref{eq:inversion_fourier_distrib} gives us $\delta = (2\pi)^{-n} 1$.
...
%
%
\item We can now define what is a \textbf{regular direction}\index{regular direction} of a compaclty suppoted distribution $u\in\Ecal^\prime(\Xsf)$. It is a vector $k \in \Rbb\setminus\{0\}$ such that 
%
\begin{itemize}
\setlength\itemsep{0pt}
...
\end{itemize}
%
Conversely a \textbf{singular direction}\index{singular direction} of a distribution $u$ is the set of all directions which are not regular, this set is denoted by $\Sigma (u)$, i.e.
%
\begin{equation}
...
\end{equation*}
%
with $\phi(x) \neq 0$. Since it is an intersection of closed conic sets, $\Sigma_x(u)$ is also a closed conic set. The set $\Sigma_x(u)$ encodes both notions the singular support and direction of $u$. This permits us to define the notion of wave front set (cf. \cite{hormander_analysis_1990}).
...
We now want to the extend definition of wave front set to the the case of distribution defined on manifold $\Mcal$. We recall we denote by $T_x\Mcal$ the tangent space at $x$ of $\Mcal$, and by $T^\ast \Mcal$ the cotangent bundle.
...
\begin{lemma}[Pointwise product of numerical distributions]\label{lem:prod_distib_fourier}
Let $u$ and $v$ be in $\Dcal^\prime(\Rbb^n)$. We say that $w\in\Dcal^\prime(\Rbb^n)$ is the pointwise product of $u$ and $v$ if and only if $\forall x \in \Rbb^n$, $\exists \phi \in \Dcal(\Rbb^n)$, such that for all $k \in \Rbb^n \setminus \{0\}$ the following integral is absolutely convergent.
%
\begin{equation*}
\widehat{w}(k) := \left(\widehat{u} \ast \widehat{v}\right)(k) = (2\pi)^{-n} \int_{\Rbb^n} \dsf q \ \widehat{u}(q) \ \widehat{v}(k-q) , 
\end{equation*}
%
where using \eqref{eq:distrib_smooth_funct} we have $\widehat{w}(k) = \sm{u , \phi(x)}$, with $\phi(x) = \esf^{-i x \cdot k}$, and the same for $\widehat{u}(k)$ and $\widehat{v}(k)$.
\end{lemma}


However this result \ref{lem:prod_distib_fourier} use the notion of Fourier transform which is dificult to generalize to the case of manifold, in contrast to the wave front set as just seen above.
...
For that we need to defined the space of distribution $\Dcal^\prime(\Mcal)$ with wave front set contained in a closed cone $\Sigma$, i.e.
%
\begin{equation*}
...
\end{equation}
%
can be extend to a sequentially continuous operator defined as 
%
\begin{equation*}
\Dcal_{\Sigma_1}^\prime(\Mcal) \otimes \Dcal_{\Sigma_2}^\prime(\Mcal) \ \to  \ \Dcal_{\Sigma_3}^\prime(\Mcal) \ .
\end{equation*}
%
where 
%
...
The product of two ditributions  $u\in\Dcal^\prime(\Mcal)$ and $v\in\Dcal^\prime(\Mcal)$ does exist if $\WF(u) \oplus \WF(v)$ does not contain terms of the form $(x,0) \in T^\ast\Mcal$.
\end{lemma}
...
%%TODO EXOS
\begin{examples}[Examples of wave front sets of distribution]
Let us denote $\Xsf$ a subset of $\Rbb^n$ in these examples.
\begin{itemize}
%
%
%
%
\item The product Dirac distribution $\delta\in\Dcal^\prime(\Rbb)$ with itself is not well defined, indeed 
\begin{equation*}
...
\end{equation*}
%
The quantization procedure we choose is a formal deformation of the (classical) pointwise product. The idea is to map the classical algebra to the same one with quantum corrections. For doing it we need to define quantum observables as power seried in $\hbar$ having for coefficients classical observables. Any convergence conditions is required. We summarize this new notion in the following definition.
...
An generic element $\Fsf \in \Fcal_\reg(\Mcal)[[\hbar]]$ can be written as
%
\begin{equation*}
...
The quantum algebra is defined as the algebra of quantum observables to which we associate a new product, called the quantum product. It encodes the quantum structure. 
...
We notice that the power series in $\hbar$ in \eqref{eq:q_prod_reg} does not seem to converge. However we do work with quantum observables which do have a finite numbers of non vanishing functional derivatives, thus the series in the definition of $\star_\Delta$ product is always convergent. 


\bigskip


Let us consider an example to illustrate the definition of the $\star_\delta$ product. We choose to work with the following regular functional
%
\begin{equation*}
\Fsf(\phi) = \int_{\Mcal^2} \dsf x \ \dsf y \ f(x,y) \ \phi(x) \ \phi(y) \ ,
\end{equation*}
%
with $f \in \Dcal(\Mcal^2)$. We compute its first derivatives
%
\begin{eqnarray*}
...
\end{eqnarray*}
%
Therefore using \eqref{eq:q_prod_reg} we can write explicitely the $\star_\Delta$ product as follow
%
\begin{eqnarray*}
...
\end{eqnarray*}
%
whith $g$ a suitable test function. We see that we in the product pointwise products of distributions are present, and as we know such products can be ill defined. In order to know if these pointwise product are well defined or not we shall use the notion of wave front set introduced in section \ref{p:SING_WF} and in particular the result from lemma \ref{lem:prod_distrib_wf}. The causal propagator $\Delta$ has the following wave front set 
%
\begin{equation}
...
\end{eqnarray*}
%
Then we write explicitely the $\star_\Delta$ product as follow.
%
\begin{eqnarray}
...
\end{eqnarray}
%
We see that we in both product we have to consider pointwise product of distribution. We need to know if they are well defined or not. For doing it we shall use lemma \ref{lem:prod_distrib_wf}. We work with local observables therefore knowing the wave front set of $\Delta$ \eqref{eq:wf_causal_prop} we have
%
\begin{equation}
...
Another candidate is the Hadamard distribution because in its wave front set there is an additional conditions which as we shal see permits to multiply it with itself. This so-called Hadamard distribution is a distribution which satisfies the following conditions.
%
\begin{enumerate}
...
\end{equation*}
%
with $\Theta$ the Heaviside function. $\Delta_\fsf$ shall be also called the Feynman propagator. It is well defined among regular functionals, however we mean working with local functionals. Therefore let us first consider an example.


We keep working with the same local functionals $\Fsf(\phi)$ defined in \eqref{eq:exo_loc_obs}. Using \eqref{eq:prod_time} we can write explicitely the time ordered product for $\Fsf(\phi)$ with itself as follow
%
\begin{eqnarray*}
