...
\end{itemize}
%
then $d$ is called a metric. We can show that every pseudometric space $(\Xcal,d)$ is also a topological space for which the topology is induced by the collection of open sets in $\Xcal(=\Xsf)$. A topological space $(\Xsf),\Tcal)$ is said to be \textbf{metrizable}\index{metrizable} if there is a pseudometric on $\Xsf$ for which the induced topology is $\Tcal$.
...
\end{equation*}
%
It is called the \textbf{causal factorisation property}\index{causal factorisation property}. Because we restricted ourselves to hyperbolic manifold, we can always find a Cauchy surface, and therefore make sense of a support of a functional which is later or earlier to another support. Let us write explicitely the time ordering product
%
\begin{equation}
...
We also define the  time ordering product as the following map
%
\begin{equation*}
\Tsf_n \ : \ 
\left\{
...
The Feynman propagator $\Delta_\fsf$ can be writen using the Hadamard representation, denoted $\Hsf_\fsf$. For even dimension $d$ of the spacetime $\Mcal$ we have
%
\begin{equation*}
...
\end{equation*}
%
where $\Vsf(\phi)$ is a (functional) interacting potential. 

It can be rewritten using the graphical approach. 
%
\begin{eqnarray*}
S(\Fsf) &=& \Tsf \ \exp\left( \frac{i}{\hbar} \Tsf^{-1} \Fsf \right) \ = \ \Tsf \bigg( \sum_{n=0}^{\infty} \frac{i^n}{n! \ \hbar^n} (\Tsf^{-1} \Fsf)^n \bigg) \\
&=& \sum_{n=0}^{\infty} \frac{i^n}{n! \ \hbar^n} \Tsf_n(\Fsf^{\otimes n}) \ = \ \sum_{n=0}^{\infty} \frac{i^n}{n! \ \hbar^n} \msf_\nsf \circ \Tcal_n(\Fsf^{\otimes n}) \\
\Ssf(\Fsf) &=& \sum_{n=0}^{\infty} \frac{i^n}{n! \ \hbar^n} \msf_\nsf \circ \sum_{\gamma \in \Gcal_n} \frac{1}{\mathsf{Sym}(\gamma)} \sm{\tsf_\gamma \ , \ \delta_\gamma(F^{\otimes n})} \ .
\end{eqnarray*}
%
An interaction $\Vsf(\phi)$ may written as 
%
\begin{equation*}
...
paragraphe 4, p. 18 -- 21 \cite{brunetti_microlocal_2000}.

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! \par


We shall work for now on with the algebra $\Acal_\Tsf^\csf(\Mcal)[[\hbar]]$. And using the Bogoliubov's formula we can map it to the free algebra. The problem we have with $\Rcal_\Vsf(\Fsf)$ to get the interacting algebra is that we have to compute time oredered products of local observables, and as we know it is ill defined. It can be solved using the Epstein Glaser procedure, where the time ordered product of local functionals is build in a recursively. 




The time ordered product can be expanded in terms of distributions smeared with compactly supported smooth functions which play the role of coupling constants (multiplied by a spacetime--cutoff). At each recursion step the causal factorisation property permits to construct the distributions defining the time ordered product up to the total diagonal. The extension to the total diagonal can be performed extending the distributions previously obtained without altering the scaling degree towards the diagonal. In this procedure there is the freedom of adding finite local contributions supported on the total diagonal. This freedom is the well known renormalisation freedom. In addition to the properties already discussed, the renormalised time--ordered product is required to satisfy further physically reasonable conditions. We refer to %\cite{Hollands:2001b,Hollands:2004yh}
for details on these properties and the proof that they can be implemented in the recursive Epstein--Glaser construction.

In spite of the theoretical clarity of this construction, the Epstein--Glaser renormalisation is quite difficult to implement in practise. The aim of the prsent work is to discuss a renormalisation scheme which is suitable for practical computations.

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\par

Let us first look at the situation on Minkowski spacetime $\Mbb$. We have the \textbf{Steinmann scaling degree}\index{Steinmann scaling degree} at a point of a distribution $u$ with with respect to the origin. It is denoted $\sd(u)$ and defined as
%
\begin{equation*}
...
The scaling degree of a distribution $u$ obeys the following properties
%
\begin{itemize}
\item Let $u \in \Dcal^\prime(\Rbb^d)$ have $\sd(u) = \omega$ at $0$, then
\begin{enumerate}
\item Let $\alpha \in \Nbb^d$ be any multiindex, then $\sd(\partial^\alpha u) \leq \omega + \abs{\alpha}$.
\item Let $\alpha \in \Nbb^d$ be any multiindex, then $\sd(x^\alpha u) \leq \omega - \abs{\alpha}$.
\item $\mathsf{3.}$ Let $\phi \in \Ccal^\infty(\Rbb^d)$, then $\sd(\phi u) \leq \sd(u)$. 
\end{enumerate}
%
\item For $u_1 \in \Dcal^\prime(\Rbb^{d_1})$ and $u_2 \in \Dcal^\prime(\Rbb^{d_2})$, we have $\sd(u_1\otimes u_2) = \sd(u_1) + \sd(u_2)$.
\end{itemize}
