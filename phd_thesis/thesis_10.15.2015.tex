...
\begin{figure}
\centering
\includegraphics[scale=0.8]{./chap1.pdf}
% chap1.pdf: 595x842 pixel, 72dpi, 20.99x29.70 cm, bb=0 0 595 842
\caption{Summarize.}
\label{fig:chap_1}
\end{figure}
...
The off shell configuration space over $\Mcal$ is the space of real valued smooth maps, $\phi \in \Ecal(\Mcal)$. It is equipped with the Frechet topology.
\end{definition}
...
\end{equation*}
%
Using L. Schwartz's notation we denote the space of real valued smooth functions on $\Omega$ by $\Ecal(N)$. We equip $\Ecal(N)$ with the following family of seminorms
%
\begin{equation*}
...
\end{eqnarray*}
%
This space is a locally convex topological vector space which appears to be a Frechet space. 
...
\end{equation*}
%
with $\phi \in \Ecal\left(\Mcal \right)$. It defines the locally convex topology on $\Ecal\left(\Mcal , E \right)$. A sequence $(\phi_n)$ converges to $\phi$ with respect to this topology if and only if for any compact set $K \subset U_i$, $\left(\partial^\alpha\phi_m\right)$ converges uniformly on $K$ to $\partial^\alpha\phi$.
...
\end{equation}
%
with $\phi$ a test function, i.e. element of $\Ecal(\Mcal)$. It is an still algebra. We shall for now work with regular functionals. The space $\Ecal^\prime(\Mcal^{n})$ is the dual space of $\Ecal(\Mcal^{n})$\footnote{We will give more details on this space in the section \ref{p:DISTRIB}.}
...
%----------------------------------------------------------------------------%
\section{Explicit computations of wave front sets}
%----------------------------------------------------------------------------%
...
We shall define the notion of state which refers to the preparation of the physical system before performing any experiments. The prescription on how we shall prepare a system is called the \textbf{state} of system. When we perform an experiment the corresponding observable gives us values which can vary on the prepared system. Therefore for a system we evaluate the observable $\Fsf$ in a state $\omega$, it called the expectation value of $\Fsf$ in the given system $\omega$. For instance if we prepare the system such that the uncertainty of the value od the measurements is minimal, we have defined a pure state.
...
Up to now we have presented the time ordered products among regular functionals only. Unfortunately the case of interacting potentials constructed with regular functionals are of limited interest. We aim to be able to work with local non linear interacting potentials like
%
\begin{equation*}
\Vsf(\phi) = \int_\Mcal \dsf x \ \sqrt{\abs{g}} \ \sum_{n\geq 2} \frac{\lambda_n(x)}{n!} \phi^n(x) \ ,
\end{equation*}
%
hence, we have have to enlarge the space of functionals we are considering. The first new space is  called space of \textbf{microcausal functionals} and it is defined as follows
%
\begin{equation}
...
\index{$d_n$}
%
Again the quantum version $\Fcal_{\mathsf{loc}}(\Mcal)[[\hbar]]$ will be considered.
...
We shall conclude this section with a practical computation where a graphical representation of the Bogoliubov formula is displayed. We shall compute the two point function of the interacting field in Gaussian Hadamard state $\Omega$ of the free field. We consider a quartic potential \eqref{eq:pot_quartic} and perform the computation up to the second order in $\lambda$. In practice we need to compute the $\star$ product of $\Rsf_\Vsf(\phi)$ and $\Rsf_\Vsf(\phi)$ and evaluate in the state $\Omega$. The functionals $\Fsf$, $\Gsf$, and the potential $\Vsf$ are given by
%
\begin{equation*}
...
\begin{figure}
\centering
\includegraphics[scale=0.8]{./chap2.pdf}
