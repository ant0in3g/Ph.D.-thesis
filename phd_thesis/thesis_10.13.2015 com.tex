...
In the last decades \textbf{(QFT)}\index{quantum field theory (QFT)} has been tested with very sophisticated \sbar{experimetnation}\com{experiments}, and predictions made by the theory coincide with very high precision to the \sbar{exprimental} \com{experimental} results. One of the missing block to this robust theoretical framework is implementing gravitation. QFT on curved spacetime \textbf{(CST)}\index{curved spacetime (CST)} is a first step in that direction. \com{In this work, for} \sbar{For} simplicity we will \sbar{restarict} \com{restrict} ourselves to the case of \com{a quantum} scalar field \com{propagating over a curved background}. 
\sbar{It is maybe physically the least relevant model, nonetheless the treatment of this simple case will give us a clear idea of the structures in QFT} \com{... I think that the last sentence is too strong. The Higgs field is a scalar field, so I suggest you to write:.... Despite of the simplicity of this model, its treatment will give us a clear idea of the mathematical structure necessary to describe quantum field theories on curved backgrounds. }.\par%
...
We shall focus first on the free theory, and describe the mathematical elements necessary to describe quantum field theories on a curved spacetime $\Mcal$. That is to say \com{we shall} introduce the notion of fields and observables within this \sbar{appoach} \com{approach}, then the classical field theory, and finally present the quantization procedure used. 
\par%
...
\com{At the end of this chapter 
we shall analyze the case of interacting theories, where the interaction will be treated perturbatively.}
 \sbar{Then we shall look to interacting quantum field theories, where interactions will be treated perturbatively.} 
\com{In the perturbative approach to interacting quantum field theories observables are given as formal power series in the coupling constant. 
These kind theories are affected by three class of problems: Ultraviolet divergences, infrared problems and finally convergence problems of the  perturbative series. At the present time we have the first problem under good mathematical control thanks to the theory of renormalization, the second problem can be tackled using for example the adiabatic limit. Unfortunately it is still not known in which sense this series could converge if it converges at all. In this chapter we shall concentrate on the first class of problems and presenting the status of the art.}
\sbar{This approach introduce divergences but it has permit to build model which appears to be experimentally well verified. The goal shall be here to identify precisely the regularization problem.}
...
%Scalar fields configuration space

%----------------------------------------------------------------------------%
\subsection{\com{Configuration space of a real scalar field theory}}
%----------------------------------------------------------------------------%
...
In this thesis we would like to describe an interacting real scalar field theory, hence a possible field configuration is described by a scalar function $\phi$ over \sbar{$M$}\com{$\Mcal$}. Furthermore, since we shall treat the interaction by means of perturbation theory, we need to enlarge the space of admissible field configurations dropping the requirement that an equation of motion is satisfied.


%\bigskip


\com{In order to be able to work easily with the theory, some }\sbar{Some} regularity needs to be requested for the admissible field configurations. In particular, we shall assume that the admissible functions are smooth.   
...
%----------------------------------------------------------------------------%
\subsection{ \com{Topological aspects of the configuration space} }
%----------------------------------------------------------------------------%


In the general case the configuration space over $\Mcal$ can be defined as the space of sections of some vector bundle $E$ over $\Mcal$. \sbar{But We} \com{Since we} shall work only with real scalar fields, \sbar{it means the vector bundle chosen is the real line bundle} \com{the chosen vector bundle is simply the real line bundle $\Ecal(\Mcal)$}. 


%\bigskip


We would like \com{now to discuss some mathematical aspects of } \sbar{to characterize the space} $\Ecal(\Mcal)$, i.e. \com{we shall discuss} \sbar{defined} its topology and the notion of convergence on it. As we now know $\Mcal$ looks locally like $\Rbb^n$, and there are coordinates maps $\Phi$ defined on $\Mcal$ and having value on a subset $\Omega \subset \Rbb^n$. 
\sbar{Thus it should be possible to find a way to map $\Phi$ to another function taking value on $\Omega$} \com{I don't understand this sentence at all!!! What do you want to say?....}. 
We shall be more precise on this later. Nonetheless we decide to start \sbar{by} looking at the space of functions \sbar{defined as} \com{of the form} 
%
\begin{equation*}
f : \Omega \subset \Rbb^n \to \Rbb \ . 
\end{equation*}
%
Using L. Schwartz's notation we denote the space of real valued smooth functions on $\Omega$ by $\Ecal(\Omega)$\index{$\Ecal(\Omega)$}. We \sbar{equipe} \com{equip} $\Ecal(\Omega)$ with the following family of seminorm\com{s}
%
\begin{equation*}
\Pcal = \left\{ p_{K,r}(f) \ , \ \mbox{ with compact subset } K \subset \Omega \ , \mbox{ and } r \in \Nbb \right\} \ ,
\end{equation*}
%
where the seminorms \com{$p_{K,r}$} are defined as
%
\begin{eqnarray*}
&& p_{K,r}(f) = \sup \left\{ \abs{\partial^\alpha f(x)} \ \mbox{ with } \ x\in K \ \mbox{ and } \ \abs{\alpha} \leq r  \right\} \ , \\
&& \mbox{\sbar{with} \com{where the multiindex} } \ \alpha \in \Nbb^n, \ \abs{\alpha} = \sum_{i=1}^n \alpha_i, \ \mbox{ and } \ \partial^\alpha = \frac{\partial^{\alpha_1}}{\partial x_1^{\alpha_1}} \dots \frac{\partial^{\alpha_n}}{\partial x_n^{\alpha_n}} \ .
\end{eqnarray*}
%
The family of seminorms $\Pcal$ endow $\Ecal(\Omega)$ with a locally convex topology.  On this space a sequence $(f_n)$ \sbar{is convergent} \com{converge} with limit $f$ \com{if} \sbar{means} $\forall \alpha$ and $\forall K$, $(\partial^\alpha f_n)$ converge uniformly to $(\partial^\alpha f)$, i.e. $\forall \epsilon > 0$ there is $N_\epsilon \in \Nbb$ such that $\forall x \in K$ and $\forall n \geq N_\epsilon$, we have $\abs{\partial^\alpha f_n(x) - \partial^\alpha f(x)} < \epsilon$. This space is a lctvs \com{... have you already defined this acronym? if not you should do it now...} which appears to be a Frechet \com{ is it Fr\'echet??? if you change it you have to change it everywhere in the thesis} space.
...
\end{equation*}
%
We know now how to define the topology on every single member\sbar{s} of the Cartesian product of the right hand side. It is due to fact that $\Phi_i(U_i)$ is a subset of $\Rbb^n$. The topology of the all Cartesian product is the product topology, for which the open sets are $\emptyset$ and the union of Cartesian products $\times_i W_i$ with $W_i$ \com{open sets in }\sbar{subset of} the topology \sbar{in} \com{of} $ \Ecal\left(\Phi_i(U_i) , \Rbb \right)$. The topology of the left hand side is the induced topology. \com{As for $\Ecal(\Rbb^n)$ the topology of $\Ecal(\Mcal)$ can alternatively be induced by a family of seminorms. }Let set $\gamma = (i, K, r)$, with $i$ to index the opens $U_i$, $K \subset \Phi_i(U_i)$ compact, and $r \in \Nbb$. We define the seminorm $p_\gamma$ on $\Ecal(\Mcal)$ as follow
%
\begin{equation*}
...
For later \sbar{puposes} \com{purposes} we consider real valued smooth compactly supported functions $\Dcal(\Mcal)$, endowed with the locally convex topology implemented as follow
%
\begin{equation*}
\com{\Dcal(\Mcal) = \bigcup_{K} \Dcal_K(\Mcal)} \ ,
\end{equation*}
%
where in the right hand side \sbar{it is} the union \com{is taken} over all compact set $K \subset \Mcal$. $\Dcal_K(\Mcal) \subset \Ecal(\Mcal)$ is the space of all smooth functions supported in $K$, endowed with the topology induced from $\Ecal(\Mcal)$. On $\Dcal(\Mcal)$ we have the inductive limit topology. It is a lctvs but non metrizable, therefore it is not a Frechet space. 
...
\com{In this section we shall introduce the set of observables of a scalar field theory over a curved background. We recall that an observable models a possible experimental apparatus by means of which we can test the physical system we are considering. In other words, it must be possible to associate an observable to every detectable property of an element of the configuration space over $\Mcal$. Examples of observables are the local energy density, the local temperature, the local momentum and the local density of the field.  Hence, roughly speaking, observables are functions on the configuration space with values in ordinary numbers. }



\com{Once we have detected a sensible set of observable which describe a quantum field theory on curved background, we shall analyze the relation among them. 
Here we aim to individuate the algebraic properties satisfied by the set of observables, this is necessary in order to use standard algebraic method to quantize the system. Notice that the algebra of observables we want to obtain will not depend on the particular state of the system. }

\sbar{Let us say what do we mean by observable. We have defined on a spacetime $\Mcal$ the configuration space, now we want to describe the physics on it. It is done roughly speaking by defining functions which take value on the configuration space and give numbers (like a temperature for intance).
}

%\bigskip


\sbar{We want to study QFT on curved spacetime, to be able to do it we use the algebraic method. It permits to build observables which do not dependent on the state. Because those objects are not local, and we are able to describe things precisely on $\Mcal$ only locally.}


%\bigskip


We shall use the functional \sbar{aproach} \com{approach to define mathematically the observables}, \com{this method is less abstract and more} \sbar{it is also a less abstract method and more} \sbar{comptational} \com{computationally} friendly \com{... you should put some reference here and above....}. \com{Hence, we} \sbar{We }shall view for now on an observable as a \sbar{funtional} \com{functional} \sbar{. It is defined as} 
%
\begin{equation*}
\Fsf : \left\{
\begin{array}{ccc}
...
\com{The generic space of observables is thus the set of all possible functionals and it is denoted by $\Fcal(\Mcal)$}.
\sbar{A generic space of observables, view as functionals, is denoted by $\Fcal(\Mcal)$.} 
\com{We shall now discuss some further restrictions we have to request in order to be able to work with these objects.}


%\bigskip


The laboratory where the \sbar{measuremts} \com{measurements} are made is finite dimension\com{al} in space and time, thus we shall consider only functionals which \com{are} defined on a finite subspace of $\Mcal$. \com{For this } \sbar{It the} reason we need a concept which permits to localize functionals in \com{a} certain region of spacetime.
...
\com{According to definition \ref{def:spacetime_supp}  a functional $\Fsf$ does not ``feel'' field configurations which are localized outside $\supp(\Fsf)$ $\supp(\Fsf)$}
\sbar{The definition \ref{def:spacetime_supp} shows that a functional $\Fsf$ does not ``feel'' fields which are localized outside $\supp(\Fsf)$.} \com{In other words the} \sbar{The} spacetime support of $\Fsf$ is the subset of $\Mcal$ where $\Fsf$ does see the ``fluctuations'' of the field configuration.


%\bigskip

\com{As discussed above, the}
\sbar{The} space $\Fcal(\Mcal)$ is still \sbar{very} \com{too} general. We should consider functionals with compact spacetime support over $\Mcal$, \com{this set is} denoted by $\Fcal_0(\Mcal)$. \com{Notice that this} \sbar{This} news space can be endowed with an algebraic structure. 



\begin{definition}[Algebra of compactly supported functionals] \label{def:algebra_comp_supp_func}
Let $\Fsf$ and $\Gsf$ be compactly supported functionals, i.e. elements of $\Fcal_0(\Mcal)$ \com{ then we define the following operations }  
\sbar{. The space $\Fcal_0(\Mcal)$ is a comutative unital $\ast-$algebra with the following structure}
%
\begin{itemize}
...
\com{The space $\Fcal_0(\Mcal)$ equipped with the operations/elements listed above defines a commutative unital $\ast-$algebra.}
\end{definition}

\com{ In order to ensure the well posedness of the previous definition, we}
\sbar{We} can check that the algebraic operations \sbar{defined} \com{introduced} in \com{definition} \ref{def:algebra_comp_supp_func} do not modify the spacetime support.


\begin{lemma}[``Rigidity'' of the spacetime support]
The algebraic operations \sbar{defined} \com{introduced} in \com{definition} \ref{def:algebra_comp_supp_func} \sbar{do} preserve the spacetime support of a functional. \com{In particular}
%
\begin{itemize}
...
\sbar{We want to study in this present work interacting quantum field theory. 
In order to define the adapted spaces of observables for this, we will have to consider some regularities of the derivatives of these functionals. }
\com{The set of functionals present in $\Fcal_0$ is still too general, in order to be able to properly work with a functional we have to be able to compute all its functional derivatives and these functional derivatives have show some regularity.}
...
\com{Let us start formalizing this discussion giving the definition of }
\sbar{Now let us give the definition of the} functional derivative which will be used all along in the present work. 
Let $E$ be a locally convex space, \sbar{and} a subset $U \subseteq E$, \sbar{which} is also \com{ said to be .... is it this what you intend here????} locally convex if every point $x \in U$ has a neighborhood $V$ in $U$. 
...
\sbar{Let us \sbar{precise} \com{specify} what is a jointly \sbar{continous} \com{continuous} map on a product space.}
\com{We recall that a} \sbar{A} map $f : V \times U \to W$ at $(x,y) \in V \times U$ is jointly \sbar{continous} \com{continuous} if for each neighborhood $W^\prime$ of $f(x,y)$ there exists a product of open sets $U^\prime \times V^\prime \subseteq U \times V$ containing $(x,y)$ such that $f(U^\prime \times V^\prime) \subseteq W^\prime$. For later purposes we notice that the \sbar{derivates} \com{derivatives} of a functionals are distributions.
...
%
\com{... you should also add ... the directions in the subsequent example.... }

\com{Consider a $\lambda \in \Dcal(\Mcal)$ and
\begin{equation}\label{eq:pot_quartic}
\Vsf(\phi) \com{\doteq} \int \dsf x \ \sqrt{\abs{g}} \ \frac{\lambda(x)}{4!} \phi(x)^4 \ ,
\end{equation}
then
\[
\Vsf^{(1)}(\phi)[\psi] = \int \dsf x \ \sqrt{\abs{g}} \frac{\lambda(x)}{3!} \phi(x)^3  \psi(x)\ , \qquad
%
...
\]
}\sbar{with $\lambda \in \Dcal(\Mcal)$, and} \com{where} $\delta$ \com{is} the Dirac distribution. We can show the main results in differential calculus still hold in this framework.
...
\begin{lemma}
%
Let $\Fsf$ and $\Gsf$ be two functionals at least \sbar{continously} \com{continuously} differentiable, and let $\phi , \psi_{\sharp}$ contained in a locally convex topological vector space \com{....which one?...}. 
%
\begin{itemize}
%
...
\sbar{We shall consider derivatives of functionals. Therefore in order to always work with well defined expression we define a new space of observables. }

\com{The are now ready to introduce the space of observables we shall work with as the following subset of }
\sbar{It is a subset of} $\Fcal_0(\Mcal)$, called the space of regular functionals,
%
\begin{equation*}
...
\com{In the previous sections we have introduced the set of observables of a scalar field theory in the functional approach, we discuss now how to describe a  classical field theory in this framework, therefore, we have as equation of motion the generalised Klein Gordon equation}

\sbar{We presented the functional approach which shall be used here. We formulate now the clasical field theory. We work with scalar fields on curved spacetime, therefore we have as equation of motion the generalised Klein Gordon \sbar{eqation}\com{equation}.%
}
%
\begin{equation} 
...
\end{equation}
%
\com{I removes the star in the previous equation!}
The coefficient $m$ denotes the (positive real) mass of the theory, $\xi \in \Rbb$ \com{describes the coupling with} \sbar{, $\Rsf$} the scalar curvature \com{$R$}, and $V^\prime_\lambda$ \com{is} a potential which vanishes for $\lambda$ equal to zero. In this section we shall only consider $V^\prime_0=0$. We require in the case of vanishing curvature ($\xi=0$) \com{in such a way} that \eqref{eq:kg_eq} reduces to the Klein Gordon equation of the free scalar field theory on Minkowski spacetime. The case $\xi=0$ is called minimal coupling, and $\xi=\frac16$ the conformal\sbar{ly} coupling (for more details look at \cite{waldGR}).


%\bigskip


\sbar{We defined the spacetime $\Mcal$ as} \com{We recall that $\Mcal$ is chosen to be} a globally hyperbolic spacetime, therefore the differential equation \eqref{eq:kg_eq} admits \com{an} unique solution once we \com{fix enough} \sbar{give sufficient} initial \sbar{data} conditions \com{on a generic Cauchy surface $\Sigma$}. It is \com{actually a well posed} \sbar{an} initial value problem \sbar{on a Cauchy surface $\Sigma$} on $\Mcal$,
%
\begin{equation}
...
\end{equation}
%
where $f \in \Dcal(\Mcal)$, $\phi_1, \phi_2 \in \Dcal(\Sigma)$, and $n$ is a future directed timelike vector \com{orthogonal to} \sbar{of} the smooth Cauchy surface $\Sigma$ of $\Mcal$\sbar{considered}. The unique solution $\phi \in \Dcal(\Mcal)$ has the following support property
%
\begin{equation*}
...
\end{eqnarray*}
%
The support of $\Delta_{\mathsf{\rsf\setminus\asf}} f$ is in the causal future (respectively past) of the test function $f$. The difference of the two fundamental solutions is called \sbar{ the  causal propagator} \com{{\bf causal propagator}}  of $\Psf$, \com{actually }it is defined as
%
\begin{equation*}
\Delta \com{\doteq} \Delta_\rsf - \Delta_\asf \ .
\end{equation*}
%
Its support \com{is} the union of the causal future and \com{the causal} past of the element applied to it. For all solutions $\phi$ of $\Psf \phi =0$ with compactly supported initial datas on $\Sigma$, \com{it is possible to find a (non unique) compactly supported smooth function $f$ such that} $\phi = \Delta f$. Moreover if $\Delta f =0$ then $f = \Psf g$. \sbar{And f} \com{F}inally if $\Psf$ is self adjoint, then $\Delta$ is skew adjoint, i.e. $\sm{\Delta f,g} = - \sm{f,\Delta g}$. These results hold for all $f, g \in \Dcal(\Mcal)$. 
\com{ .... I cancelled something because it seemed to me a repetition..
%We shall consider the following conventions
%
%\begin{eqnarray*}
...
Before quantizing the free theory, and in order to characterize the space of solutions of  \eqref{eq:kg_eq}, we shall introduce a symplectic structure. It is the space \com{$\mathcalligra{S} \ \ (\Mcal)$} of fields \sbar{$\phi \in \Dcal(\Mcal)$} \com{$\phi \in \Ecal(\Mcal)$}  solution\com{s} of \eqref{eq:kg_eq} \com{with compactly supported initial data equipped }with a strongly non degenerate map $\tau$,
%
\begin{equation*}
\tau : \left\{
\begin{array}{ccl}
...
The definition of the symplectic form is independent of the choice of $\Sigma$ we make. Indeed we can identify $\tau$ as the integral \com{over $\Sigma$} of the current $j^\mu$ defined as
%
\begin{equation*}
j^\mu (\phi,\psi) = \psi (\nabla^\mu \phi) - \phi (\nabla^\mu \psi) \ . 
\label{eq:current}
\end{equation*}
%
Using the Klein Gordon equation we can show that $j^\mu$ is conserved, i.e. $\nabla_\mu j^\mu = 0$. \com{Hence the divergence theorem} \sbar{It} implies that the Cauchy surface $\Sigma$ we choose does not matter. 


%\bigskip


\com{As already stated above, if}
\sbar{If} \sbar{$\phi \in \Scal(\Mcal)$} \com{$\phi \in \mathcalligra{S} \ \ (\Mcal)$}
then,  for any bounded \sbar{neighbourhood} \com{ neighborhood  }  $\Ocal(\Sigma)$ of a Cauchy surface $\Sigma$, we can find a test \sbar{funstion}\com{ function} $g\in \Dcal(\Mcal)$ with $\supp\left(g\right) \subset \Ocal(\Sigma)$ and $\phi = \Delta g$.
\com{Furthermore, it holds also that } 
\sbar{Therefore for $\phi , \psi \in \mathcalligra{S} \ \ (\Mcal)$, } in any bounded neighbourhood $\Ocal(\Sigma)$\com{,} the symplectic form $\tau(\phi, \psi)$ can be written in terms of the causal propagator as $\tau(\phi,\psi) = \sm{f , \Delta g}$ for suitable test functions $f$ and $g$.  \com{ .... How do you prove it???? }
...
We mention\com{ed} in the section \ref{p:OBS} that the differentiability of functionals shall be used to define spaces of observables having the correct structure to perform quantization of the theory. We shall see in \com{section} \ref{p:Q_DEFORM} that we \com{need to } \sbar{shall} consider product of derivatives of \com{ non regular} functionals, i.e. product of distribution. 
...
\com{Finally we shall discuss a criterion due to H\"ormander, see e.g. \cite{??????}, which permits to define the pointwise product of distributions.}
...
The set $\Xsf$ denotes for now on a \com{open/closed/generic???} subset of $\Rbb^n$. As defined previously the spaces $\Dcal(\Xsf)$ and $\Ecal(\Xsf)$ denote respectively the space of compactly supported smooth functions on $\Xsf$ and the space of smooth functions on $\Xsf$.


%\bigskip

\com{We recall that a}
\sbar{A} \textbf{distribution}\index{distribution} $u$ on $\Xsf$ is a linear form, on $\Dcal(\Xsf)$ such that for every compact set $K \subset \Xsf$ there exist constants $C$ and $k$ such that
%
\begin{equation*}
\abs{u(\phi)} \leq C \sum_{\abs{\alpha} \leq k} \sup \abs{\partial^\alpha \phi} \ , \quad \phi \in \Dcal(\Xsf) \ .
\end{equation*}
%
The set of all distribution in $\Xsf$ is denoted by $\Dcal^\prime(\Xsf)$. It is \com{actually} the dual space of $\Dcal(\Xsf)$. \com{Thanks to linearity, we} \sbar{We} can also characterize a distribution in the \sbar{equivalent} following \com{equivalent} way. A linear form $u$ on $\Dcal(\Xsf)$ is a distribution if and only if 
%
\begin{equation*}
...
If $u\in\Dcal^\prime(X)$ then the \textbf{support} of $u$, denoted \com{by} $\supp(u)$,\index{distribution!support} is the set of point\com{s} in $\Xsf$ having no open neighborhood to which the restriction of $u$ is $0$. If a distribution $u$ has compact support, then $u \in \Ecal^\prime(\Xsf)$ is called \sbar{compaclty} \com{compactly} supported distribution.


%\bigskip


Let us now generalize this notion to the case of a manifold $\Mcal$. We already introduced $\Dcal(\Mcal)$ and $\Ecal(\Mcal)$. Then the \com{spaces of distributions} \sbar{distribution space} are the dual of these two sets, we denote them by $\Dcal^\prime(\Mcal)$ and $\Ecal^\prime(\Mcal)$.
...
Let $f$ be a\com{n} integrable function on $\Xsf \subset \Rbb^n$ then the Fourier transform $\hat{f}$ is the continuous function in $\Rbb^n$ defined by
%
\begin{equation*}
...
\end{equation*}
%
The Fourier transform of the convolution of two integrable functions is the pointwise product of their \sbar{fourier} \com{Fourier} transform,
%
\begin{equation*}
...
%
The Fourier transform can be used to give \com{a} criterion \sbar{on} \com{for} the smoothness of functions. The Fourier transform of a smooth \sbar{compaclty} \com{compactly} supported function $u$ decays faster than any negative power of the dual variable $\xi$, i.e
%
\begin{equation}
...
\end{equation*}
%
for $f \in \Scal$. The space of \sbar{compaclty} \com{compactly} supported distribution\com{s} $\Ecal^\prime(\Xsf)$ is a subspace of $\Scal^\prime(\Xsf)$, therefore the results holding for $\Scal^\prime(\Xsf)$ also hold for $\Ecal^\prime(\Xsf)$. 
...
We will give here a way to characterize the singularities of a distribution \com{introducing the notion of wave front set}. \sbar{It is called the wave front set.} \sbar{It gives a precise description of \com{the} singularities.} \com{We shall see that it} \sbar{It} tells us not only at which points a singularity occurs, but it also indicates the directions in the dual space from which the singularities are ``coming'' from. We shall first analyze the particular case $\Mcal = \Rbb^n$ and then generalize it to a generic curved spacetime $\Mcal$. 


%\bigskip

\com{Before introducing the precise definition of the wavefront set, let us start recalling some preliminary important notions.  }
\sbar{Before defining the wave front set of a distribution we will recall some impotant notions.} We define the \textbf{singular support}\index{distribution!singular support} of a distribution $u$, $\singsupp(u)$, as the set of points in $X$ having no open neighborhood to which the restriction of $u$ is a smooth function. 


%\bigskip


A set $\Sigma \subset \Rbb^n \setminus \{0\}$ is called a \textbf{conic set}\index{conic set} if for any point $\xi \in \Sigma$, it contains all the points $a \xi$ with $a > 0$. A conic set is completely determined by its intersection with the unit sphere $S^{n-1}$ in $\Rbb^n$. By a \textbf{conic neighborhood}\index{conic neighborhood} of a point $\xi \in \Xsf \setminus \{0\}$ we mean an open conic set that contains $\xi$. Using this new notion we can define what is a \textbf{regular direction}\index{regular direction} of a compaclty suppoted distribution $u\in\Ecal^\prime(\Xsf)$. It is a vector $\xi \in \Rbb\setminus\{0\}$ such that there exists an open canonical neighborhood $\Sigma$ of $\xi$ and such that \eqref{eq:crit_smooth_fourier} holds \com{for $\xi$ in this set}. \sbar{Conservely} \com{Conversely} a \textbf{singular direction}\index{singular direction} of a distribution $u$ is the set of all directions which are not regular, \com{this set is} denoted \com{by }$\Sigma (u)$.


%\bigskip


The singular support gives the location of the singularities \com{in space} and the set of singular directions gives the high frequencies which are source of these singularities. The idea is to combine these two notions.


%\bigskip

Let $u \in \Ecal^\prime(\Rbb^n)$ and $\phi \in \Ecal(\Rbb^n)$, then $\Sigma(\phi u) \subset \Sigma(u)$ \com{Remind the precise lemma/theorem or Hormander}\cite{hormander_analysis_1990}. Due to this result the set of singular directions of $u\in\Dcal^\prime(\Xsf)$ at a point $x$ is defined as
%
\begin{equation*}
\Sigma_x(u) \com{\doteq} \underset{\phi}{\bigcap} \Sigma(\phi u) \ , 
\end{equation*}
%
with $\phi \in \Dcal(X)$ and $f(x) \neq 0$. \com{Since} \sbar{As} it is an intersection of closed conic sets, $\Sigma_x(u)$ is also a closed conic set. This permit\com{s} us to define the notion of wave front set of a distribution. 
...
%
\begin{equation*}
\WF(u) \com{\doteq} \bigg\{ (x,\xi) \in X \times (\Rbb^n \setminus 0) \ ; \ \xi \in \Sigma_x(u) \bigg\}
\end{equation*}
is called the \textbf{wave front set}\index{wave front set} of $u$. The projection of \com{$\WF(u)$ to} \sbar{in} $X$ is $\mathsf{singsupp}(u)$. If $u \in \Ecal^\prime(\Rbb^n)$ then the projection of $\WF(u)$ on \sbar{the second variable} \com{$(\Rbb^n \setminus 0)$} is $\Sigma(u)$. 


%\bigskip

The main properties of the wave front set are the following \cite{hormander_analysis_1990}\com{Remind the precise lemma/theorem of Hormander}
%
\begin{eqnarray}
...
We would like now to be able to compute the wave front set of the \com{pointwise} product of two distributions. \com{Consider $u,v\in \Dcal'$, we notice that the pointwise product $uv$ }
\sbar{We notice that \sbar{$u(x)v(y)$} \com{$u(x)v(x)$}} \com{if meaningful, it could be defined as } \sbar{is} the projection of the tensor product \sbar{$u(x) \otimes v(y)$} \com{$u \otimes v$} to the diagonal.
\com{....Antoine, notice that it has in general meaningless to write $u(x)$ unless the distribution is described by an ordinary function. Despite of this fact the notation $u(x)$ is sometime useful.... you should be more precise on this point...}
 Therefore we shall first \com{analyze} \sbar{ define} the wave front set of the tensor product of two distributions. For $\Xsf$ and $\Ysf$ two subsets of $\Rbb^n$, the tensor product of $u\in\Dcal^\prime(\Xsf)$ and $v\in\Dcal^\prime(\Ysf)$ has the following wave front set \com{(for a proof, see e.g. theorem tal dei tali in \cite{?} )}
%
\begin{eqnarray*}
...
\end{eqnarray*}
%
The \sbar{priduct} \com{product} $uv$ is defined as the pullback of $u \otimes v$ \com{under the ``diagonal'' map}. \com{We recall that the} \sbar{. The} pullback \com{of $u\otimes v$} \sbar{$u v$} by the map $d : \Xsf \times \Xsf \to \Xsf$ is well defined if 
%
\begin{equation*}
...
We now generalize this powerfull tool the framework of manifold. The wave front set of a distribution $u \in \Dcal^\prime(\Mcal)$, roughly speaking, is a conic subset of the cotangent bundle $\WF(u) \subset T^\ast\Mcal\setminus\{0\}$ where the first component gives the singular support, $\singsupp(u)$, and the second gives the direction in which the Fourier transform of $u$ does not decrease \sbar{rapidely} \com{rapidly}. Let us be more precise and consider two atlas ${(U,\phi)}$ and ${(V,\psi)}$ covering both $\Mcal$ \com{Why do you need two atlas? By the way, this looks like two coordinate charts}
\com{Let us be more precise and consider an atlas $\{(U_i,\phi_i)\}$. The wave front set of a} \sbar{A} distribution $u\in\Dcal^\prime(M,E)$ whose restriction to $U_i$ is $u_i$ is defined as the union of the wave front set of all restriction on the \sbar{corrdinates neighbourhoods} \com{coordinate neighborhoods}. The restriction $u_i$ has the following wave front set
%
\begin{equation*}
...
\sbar{As already said we want to study QFT, therefore w} \com{W}e shall \com{now pass to discuss the quantization of} \sbar{quantize} the classical free theory \sbar{we} presented in \com{section} \ref{p:CLASSICAL}. In other words we shall quantize \sbar{in the functional approach} the \sbar{alegra} \com{algebra} $\Acal_\reg(\Mcal)$ \com{of classical observables} \com{in the functional approach}.


%\bigskip

\com{For later purposes the}
\sbar{The} quantum observable\com{s} shall be defined as a power series in $\hbar$ with coefficients in $\Fcal_\reg(\Mcal)$. The space of these quantum observables will be denoted by $\Fcal_\reg(\Mcal)[[\hbar]]$. \com{As for the classical counterparts, an }\sbar{An} involution $\ast$ is required\sbar{,} \com{and }it is defined as 
%
\begin{equation*}
%\Fsf(\phi)^\ast =  \Fsf(\phi^\ast) \ .
\com{\Fsf^\ast(\phi) =  \overline{\Fsf(\overline{\phi})} \ .}
\end{equation*}
...
The quantization procedure we choose is a formal deformation of the pointwise product. We define a product between elements in $\Fcal_\reg(\Mcal)[[\hbar]]$, called the $\star$ product\sbar{. It has the following form}\com{, in the following way}
%
\begin{equation}
(\Fsf \star_{\com{\Delta_+}} \Gsf)(\phi) \com{\doteq} \Fsf(\phi) \cdot \Gsf(\phi) + \sum_{n=1}^\infty \frac{\hbar^n}{n!} \sm{ \Fsf^{(n)} , \Delta_+^{\otimes n} \Gsf^{(n) } } \ ,
\label{eq:q_prod}
\end{equation}
%
...
%
\begin{itemize}
\item \com{The a}\sbar{A}ntisymmetric part \com{is} proportional to the commutator function $\Delta$, i.e. 
%
\begin{eqnarray*}
...
\item \com{The m}\sbar{M}icrolocal spectrum condition \com{holds, namely}
%
\begin{equation*}
\com{\WF(\Delta_+) \ = \ \bigg\{ \bigg( x, y ; k_x, k_y \bigg) \in \Tsf^\ast\Msf^2 \setminus \{0\} \ \bigg| \ (x,k_x) \sim (x,\com{-}k_y), \ k_x \triangleright 0 \bigg\} } 
\label{eq:hadamard_condition}
\end{equation*}
%
\com{where} $(x,k_x) \sim (x,k_y)$ implies that there exists a null geodesic $\gamma$ connecting $x$ to $y$ such that $k_x$ is coparallel and cotangent to $\gamma$ at $x$ and $k_y$ is the parallel transport of $k_x$ from $x$ to $y$ along $\gamma$. \com{Furthermore,} \sbar{And} $k_x \triangleright 0$ means that the covector $k_x$ is futur\com{e} directed.

\end{itemize}


We notice that in the product \eqref{eq:q_prod} we get \com{in general} product\com{s} of distribution\com{s}. 
\com{As a first observation, we notice that here we are considering regular functionals, hence the distributions involved are actually regular functions 
and the problems with pointwise multiplications with $\Delta_+^n$ are avoided.}
\sbar{First remark is that we work with regular functionals, therefore it avoids all product to be ill defined, because the coinciding points are excluded.} 

\com{As a second remark, we notice that the wave front set of $\Delta_+$ could allow us to consider products among more general functionals like those whose functional derivatives are always supported on the diagonal. Actually, thanks to the condition $k_x \triangleright 0$, the pointwise product of the Hadamard distribution with itslef can be considered.}

\sbar{Second remark is that the wave front set requirement of $\Delta_+$ allow us to consider product of it on the full space. Indeed thanks to the condition $k_x \triangleright 0$, product of the Hadamard distribution can be considered. Therefore the $\star$ product is always well defined.}

\com{....You have not jet defined neither the microcausal functionals nor the local one.. so the last remark is a bit strange ... }

%\bigskip

\com{The product introduced in \eqref{eq:q_prod} depends on the choice of $\Delta_+$, however,}
\sbar{In the expression of \eqref{eq:q_prod} we write $\star_\Delta$, but it does not depend on $\Delta_+$. Indeed} if we choose a different $\Delta^\prime_+$ with the same properties, we have that $w:=\Delta^\prime_+ - \Delta_+$. Moreover, \com{thanks to the work of Radzikowski (add a citation)} $w$ is real, smooth and symmetric and
%
\begin{eqnarray*}
...
\end{eqnarray*}
%
Thus the algebras \sbar{associated t} \com{constructed with} $\star_{\Delta_+}$ and $\star_{\Delta^\prime_+}$ are isomorphic via $\alpha_{w}$ \com{and thus the construction does not really depend on this choice.}. 


%\bigskip


An important remark concerning the definition of the $\star$ product is that the power series in $\hbar$ does not seem to converge, 
\com{this is one of the reason why we chose to work with formal power series}
\sbar{despite of this fact, usually the physically interesting functionals } 
\sbar{the functional we choose to work with} have a finite numbers of non zero functional derivatives, \com{and } thus \com{the series in the $\star$ product among them is always convergent} \sbar{it always converge}. 
We also notice that applying the limit $\hbar \to 0$ to the $\star$ product we get the classical pointwise product
%
\begin{equation*}
...
\sbar{We} \com{All in all we have }defined the regular quantum algebra as the off shell $\ast\com{-}$algebra of regular functionals endowed with a $\star$ product, denoted
%
\begin{equation*}
\Acal_\reg(\Mcal)[[\hbar]] = \left(\Fcal_\reg(\Mcal)[[\hbar]] , ^\ast , \star \right) \ . 
\end{equation*}
%
It is \com{a} noncommutative, associative $\ast\com{-}$algebra.
...
We shall present \com{now} the perturbative construction of an interacting quantum field theory on a generic curved spacetime $\Mcal$. \com{We shall work in the }\sbar{The} framework of perturbative algebraic quantum field theory (\textbf{pAQFT}\index{perturbative algebraic quantum field theory (pAQFT)}) \com{which }has been recently developed in \cite{brunetti_perturbative_2009,fredenhagen_perturbative_2015,fredenhagen_batalin-vilkovisky_2013} \com{and it is based on earlier ideas of Esptain and Glaser and Steinmann..... } \sbar{based on earlier work}.


%\bigskip


\sbar{We are going to \sbar{look at} \com{discuss} \sbar{interactng} \com{interacting} theories where the time evolution is solved perturbatively. We choose to write the interaction functionals using a time ordering prescription. It is defined via a time ordered product between observables in a similar way as the $\star$ product where we require the following conditions}

\com{In every perturbative construction of an interacting theory there is the need of writing products of the perturbation potential where the factors are ordered in time. Hence, a necessary tool to construct a perturbative interacting theory is the time ordered product. Let us start constructing it in the case of regular functionals where it is completely characterized by the \textbf{causal factorisation property}\index{causal factorisation property}, namely by the condition}
%
\begin{equation}
\Fsf \cdot_\Tsf \Gsf = 
\left\{
\begin{array}{ll}
\Fsf \star \Gsf \quad \mbox{if } \ \supp(\Fsf) \ \mbox{ \com{is} later than  } \ \supp(\Gsf)  \\
\Gsf \star \Fsf \quad \mbox{if } \ \supp(\Gsf) \ \mbox{ \com{is} later than  } \ \supp(\Fsf) 
\end{array}
\right. \ .
\label{eq:causal_factorization}
\end{equation}
%
\sbar{It is called the \textbf{causal factorisation property}\index{causal factorisation property}. }
\com{where $\supp(\Fsf)$ is said to be  later than   $\supp(\Gsf)$ if $J^+(\supp(\Fsf)) \cap \supp(\Gsf)=\emptyset$.}
\sbar{Because we restricted ourselves to hyperbolic manifold, we can always find a Cauchy surface, and therefore make sense of a support of a functional which is later or earlier to another support. }

%\bigskip

\com{Unfortunately the case of interacting potentials constricted with regular functionals are of limited interest. We aim to be able to work with local non linear interacting potentials like $V(\phi)=\int d\mu \sqrt{|g|} \phi^n\lambda$ with $n>1$, hence, we have have to enlarge the space of functionals we are considering. This}
\sbar{Considering regular functionals is not enough. It does not contain the interaction functionals, those functionals that we would like to work with. Therefore we have to defined a new space of functionals, it }
is done by imposing \sbar{a} less restrictive condition\com{s} on the wave front set \com{of admissible functionals}. \com{The new space is  called space of} \sbar{We shall call this new space the space of} \textbf{microcausal functionals} \com{and it is defined as follows}
%
\begin{equation*}
\Fcal_{\mu\csf}(\Mcal) \com{\doteq} \left\{ 
\Fsf(\phi) \ \bigg| \ 
\begin{array}{l}
...
%
We shall work with its quantum version $\Fcal_{\mu\csf}(\Mcal)[[\hbar]]$. This space contains the \sbar{interaction} functionals \com{representing the local interaction potentials} but not only. For instance the regular functionals are still contained in it. The space which contains only the interaction \com{potentials} \sbar{functionals} is called \sbar{the local} space $\mathcal{F}_\mathsf{loc}$ \com{of local functionals}. We define it as the space of microcausal functionals having as support for their derivatives the total diagonal $d_n$
%
\begin{equation*}
\Fcal_{\mathsf{loc}}(\Mcal) \com{\doteq} \left\{ \Fsf(\phi) \in \Fcal_{\mu\csf}(\Mcal) \ \bigg| \ \supp\left(\Fsf(\phi)^{(n)}\right) \subset d_n \right\} \ ,
\end{equation*}
\index{$\Fcal_{\mathsf{loc}}(\Mcal)$}
%
\sbar{with}\com{where} the \textbf{total diagonal}\index{diagonal!total} $d_n$ \com{is} defined as
%
\begin{equation}
...
\index{$d_n$}
%
Again the quantum version $\Fcal_{\mathsf{loc}}(\Mcal)[[\hbar]]$ will be considered. We then can defined the space of ``causal'' functionals denoted $\Fcal_\Tsf(\Mcal)[[\hbar]]$\index{$\Fcal_\Tsf(\Mcal)$} \sbar{. It is} \com{as} the space of functionals which can be written as time ordered product\com{s} of quantum local functionals. It is a subspace of the microcausal causal space.
\com{ .... here there is a problem because you have not yet defined what is a time ordered product among local functional and you use it to define new objects.... I don't know exactly how to solve this problem, maybe we shall discuss it,  ..   or maybe the next paragraph solves this issue... }
...
%
\begin{equation*}
\Acal_\Tsf(\Mcal)[[\hbar]]\{I\} \com{\doteq} \left( \Fcal_\Tsf(\Mcal)[[\hbar]] , ^\ast , \star , \cdot_\Tsf \right) \ . 
\end{equation*}
\index{$\Acal_\Tsf(\Mcal)[[\hbar]]\{I\}$}

\com{Of course, up to now we have constructed the time ordered products among regular functionals only, and we have not yet presented the construction of time ordered products among local functionals. We shall come back to this construction in the subsequent part of this chapter in section (where????). For the moment let us assume to have a well defined time ordered product among local functionals and let us discuss how it is possible to construct observables of the interacting theory by means of perturbation theory, namely, when }
\sbar{We use the perturbative approach, namely when } the free algebra is perturbed by a non linear local potential $\Vsf$\sbar{,}\com{. In the latter case} the interacting algebra is represented on the free algebra by means of the \com{{\bf Bogoliubov formula}}\sbar{.} \com{which } \sbar{It} is given in terms of the local $S$ matrix, 
%
\begin{eqnarray}
...
\end{eqnarray}
%
where $\Vsf(\phi)$ is a \sbar{(functional)} \com{local functional representing the} interacting potential. An \com{example of local }interaction $\Vsf(\phi)$  \com{is } \sbar{ may written as} 
%
\begin{equation*}
\Vsf(\phi) = \int_\Mcal \dsf x \ \sqrt{\abs{\det(g)}} \ \sum_{n\geq 2} \frac{\lambda_n(x)}{n!} \phi^n(x) \ ,
\end{equation*}
%
\sbar{with} \com{where} $\lambda_n \in \Dcal(\Mcal)$ \com{are }the coupling constants. The observables are mapped from the interacting off shell algebra to the free off shell algebra with the Bogoliubov formula \com{which is} defined as follow\com{s}
%
\begin{equation}
\Rcal_\Vsf(\Fsf(\phi)) \com{\doteq} S(\Vsf)^{\star-1} \star \left( S(\Vsf) \cdot_\Tsf \Fsf(\phi) \right) \ ,
\label{eq:bogoliubov}
\end{equation}
%
where $S^{\star-1}(\Vsf)$ is the inverse of $S(V)$ with respect to the $\star$ product. 

\com{Maybe at this point you should prove that $\Rcal_\Vsf(\Fsf_f(\phi))$ solves the interacting equation of motion weakly.}

\bigskip

\com{In order to conclude the perturbative construction of interacting theories it remains to discuss the extension of the time ordered product to a product among local functionals. Unfortunately this extension is not at all trivial and it requires the introduction of the theory of renormalization. In order to highlight the problems in this extension let }
\sbar{Let} us write \sbar{explicitely} \com{explicitly} the time ordering product \com{, which is well defined among regular functionals, and let us naively try to apply the obtained expression to generic local functionals }
%
\begin{equation}
...
\end{eqnarray}
%
\com{We notice that the}
\sbar{The} product defined in \eqref{eq:time_ordered_prod} can be ill defined. A reason for
this \com{is that pointwise products of $\Delta_\fsf$ with itself can appear if the considered functionals are not regular. 
 T}\sbar{t}he wave front set of the integral kernel of $\Delta_\asf$ contains the wave front set the $\delta$ distribution because $\Psf \Delta_\asf = \Ibb$, and then pointwise product\com{s} of $\Delta_\fsf$ \com{with itself} are ill defined on coinciding points. 
\com{... I don't understand why at this point you write the following sentence....}
We should mention that the time ordered product need to satisfy different axioms explained in \cite{hollands_existence_2002}.
...
\com{ ..... I think that the text up to **** is useless here. If it is necessary it should be moved somewhere else ..........}

The Feynman propagator $\Delta_\fsf$ can be \sbar{writen} \com{written} using the Hadamard representation, denoted $\Hsf_\fsf$. We chose to work in dimension $4$, therefore 
%
\begin{equation}
...
\com{We shall come back to the problem of renormalization in section XXXX, we shall conclude this section with a practical computation where a graphical representation of the Bogoliubov formula is displayed.}
\sbar{Let us give an example for the Bogoliubov formula, and give its graphical representation.} We shall compute the two point function of the interacting field in Gaussian Hadamard state $\Omega$ of the free field. We consider a quartic potential \eqref{eq:pot_quartic} \sbar{up} and perform the computation up to the second order in $\lambda$. 
%
\begin{eqnarray*}
...
%----------------------------------------------------------------------------%
\section{\com{Time ordered products among local functionals}}
...
Our task here is to give the necessary requirements to build time ordered product in order to get a \sbar{theory} physically meaningful \com{theory}. It has been done in \cite{hollands_local_2001,hollands_existence_2002}. We shall just recall the main ideas adapted to the framework we use.
...
%
\begin{equation}
\Asf_\gamma^{(\boldsymbol{\alpha})} = \prod_{(i,j)\in\gamma} \frac{1}{\sigma_\fsf^{\ell_{ij}(1+\alpha_{ij})}} \in \Dcal^\prime(\Mcal^n\setminus D_n)
\label{eq:amplitude_sigma_reg}
\end{equation}
\index{$\Asf_\gamma^{(\boldsymbol{\alpha})}$}
%
are multivariate analytic functions. The distribution \eqref{eq:amplitude_sigma_reg} only displays the most singular contribution of $\Asf_\gamma^{(\boldsymbol{\alpha})}$, but the subleading contributions are clearly of the same form up to replacing some of the factors $(1+\alpha_{ij})$ in the exponents by $\alpha_{ij}$ or $0$.


\item In order to show that $\Asf_\gamma^{(\boldsymbol{\alpha})}$ can be uniquely extended from $\Mcal^n\setminus D_n$ to $\Mcal^n$ in a weakly meromorphic fashion, i.e. that the singularities relevant for the forest formula are poles of finite order, we follow a strategy similar to the one used in \cite{hollands_local_2001} and consider a scaling expansion with respect to a suitable scaling transformation. We first argue in Proposition \ref{prop:regularization} that an analytically regularised distribution $u^{(\alpha)}\in\Dcal^\prime(\Mcal^n\setminus d_n)$, which can be written as a sum of homogeneous terms with respect to this scaling transformation plus a sufficiently regular remainder, can be extended to $\Mcal^n$ in a weakly meromorphic way, were the uniqueness of the extension follows from its weak meromorphicity. In Proposition \ref{prop:set}, we give a sufficient condition for the existence of such a homogeneous expansion and we demonstrate in Proposition \ref{prop:almost_homo} that the distributions $\Asf_\gamma^{(\boldsymbol{\alpha})}$ satisfy this condition.


\item The above mentioned results are proved by means of generalised Euler operators which can be written abstractly in terms of a scaling transformation, but also in terms of covariant differential operators whose explicit form can be straightforwardly computed as we argue in Section \ref{p:DIFFERENTIAL_EULER}. In Proposition \ref{prop:expose_poles} we use these operators in order to demonstrate how the full relevant pole structure of $\Asf_\gamma^{(\boldsymbol{\alpha})}$ can be computed, thus showing the practical feasibility of the $\MS$ scheme. We find that our regularization scheme corresponds in fact to a particular form of differential regularization and expand on this by computing a few examples in Section \ref{p:EXOS}.
...
The next step in the strategy outlined in section \ref{p:PROGRAM} is to extend the distributions $\Asf^{(\boldsymbol{\alpha})}_\gamma$, which are à priori defined only outside of the union of all partial diagonals $D_n$ in a normal neighbourhood $\Ncal$ of the total diagonal to $D_n \cap \Ncal$ and show that this extension is weakly meromorphic in $\alpha_I$ upon setting $\alpha_{ij}=\alpha_I$ for all $i,j\in I\subset \{1,\ldots,n\}$. As anticipated, we shall prove this by using particular homogeneity properties of $\Asf^{(\boldsymbol{\alpha})}_\gamma$ with respect to the scaling transformations \eqref{eq:geo_scaling_transfo}. Even if $\Asf^{(\boldsymbol{\alpha})}_\gamma$ is not homogeneous in the strong sense of definition \ref{def:homogeneous}, it has weaker homogeneity properties which are still strong enough in order to obtain the wanted results. In this section we analyse analytically regularized distributions satisfying this weaker homogeneity condition, provide sufficient conditions for this weaker homogeneity to hold and show how the principal part of a distribution of this type can be efficiently computed.
...
The distribution $\Asf_\gamma^{(\boldsymbol{\alpha})}$ introduced in \eqref{eq:amplitude_sigma_reg} can be written as a sum of homogeneous distributions with respect to scaling towards the total diagonal $d_n$ plus a remainder. 
%
\begin{equation*}
\Asf_\gamma^{(\boldsymbol{\alpha})} = \sum_k \ \ \left( \Asf_\gamma^{(\boldsymbol{\alpha})} \right)_k + \left(r_\gamma^{(\boldsymbol{\alpha})}\right)_k \ .
\end{equation*}
%
The degrees of homogeneity of the homogeneous distributions $\left( \Asf_\gamma^{(\boldsymbol{\alpha})} \right)_k$ are contained in the following set
%
\begin{equation*}
