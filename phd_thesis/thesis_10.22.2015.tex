...
\end{equation}
%
where $\sigma_\fsf(x,y) = \sigma(x,y) + i \epsilon$, where $\sigma(x,y)$ is the Synge's world function defined in section \ref{p:CHAP_1_CONNEX_GEOD_CURV}, and $M$ is an arbitrary mass scale. The Hadamard coefficients $\usf$ and $v$ are purely geometric and thus state independent, whereas the smmooth coefficient $w$ is state dependent.
...
\end{eqnarray*}
%
This expression is only well defined locally beacuse $\sigma$ used in the expression of $\Hsf_\fsf$ \eqref{eq:hadamard_rep} is defined on geodesically convex normal neighborhood. But in section \ref{p:CHAP_3_EXT_SD} we show that $\Mcal$ admits a covering of open geodesically convex sets. Therefore in order to define $\tsf_\gamma$ globally, we may employ suitable partitions of unity. 
...
%
\item\label{item:3_wf_h_reg} \com{$\sd\left(\Hsf^{(\alpha)}_\fsf\right) \to - \infty \ \mbox{ when } \ \Re\left(\alpha\right) \to \infty$}
\end{enumerate}
%
\end{proposition}
...
\end{equation*}
%
The result \ref{item:2_sigma_1} tells us that \eqref{eq:inverse_sigma_reg_distrib} is homogeneous of degree $-2\alpha$, thus as we notices in section \ref{p:CHAP_3_EULER} we have 
%
\begin{equation*}
\sd(\Hsf^{(\alpha)}_\fsf) = 2\alpha \ ,
\end{equation*}
%
and by analiticity we can take the limit $\Re(\alpha) \to \infty$ which gives us $\sd(\Hsf^{(\alpha)}_\fsf)\to\infty$.
%
%
