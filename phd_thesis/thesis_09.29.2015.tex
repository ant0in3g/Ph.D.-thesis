...
\begin{itemize}
%
\item $\overline{\Zsf}$, the \textbf{closure}\index{closure} of $\Zsf \subset \Xsf$, is the intersection of all closed sets\footnote{A set $\Csf \subset \Xsf$ is \textbf{closed} if $\Xsf \backslash \Csf$ is open.} containing $\Zsf$.% 
%
...
%----------------------------------------------------------------------------%
\section{Observables as functionals over field configurations}\label{sec:obs}
%----------------------------------------------------------------------------%
...
\end{equation*}
%
with $\phi$ a test function, i.e. element of $\Ecal(\Mcal)$. It is an still algebra. We shall for now work with regular functionals. The space $\Ecal^\prime(\Mcal^{\otimes n})$ is the dual space of $\Ecal(\Mcal^{\otimes n})$\footnote{We will give more details on this space in the section \ref{sec:distrib}.}



%----------------------------------------------------------------------------%
\section{Classical free field theory}\label{sec:classical}
%----------------------------------------------------------------------------%
...
%
\begin{equation*}
\Delta_{\rsf\backslash\asf} : \Dcal(\Mcal) \to \Dcal(\Mcal)  
\end{equation*}
%
satisfying 
%
\begin{eqnarray*}
&& \Psf \circ \Delta_{\rsf\backslash\asf} = \Delta_{\rsf\backslash\asf} \circ \Psf = \Ibb \ , \ \mbox{ and } \ \supp(\Delta_{\rsf\backslash\asf} f) = J^{\pm} \left(\supp(f) , \Mcal\right) \ .
\end{eqnarray*}
%
The support of $\Delta_{\mathsf{\rsf\backslash\asf}} f$ is in the causal future (respectively past) of the test function $f$. The difference of the two fundamental solutions is called the causal propagator of $\Psf$, it is defined as
%
\begin{equation*}
...
We mention in the section \ref{sec:obs} that the differentiability of functionals shall be used to define spaces of observables having the correct structure to perform quantization of the theory. We shall see in \ref{sec:q_deform} that we shall consider product of derivatives of functionals, i.e. product of distribution. 
...
%----------------------------------------------------------------------------%
\subsection{Distributions on a manifold}\label{sec:distrib}
%----------------------------------------------------------------------------%
...
%----------------------------------------------------------------------------%
\section{Formal deformation}\label{sec:q_deform}
%----------------------------------------------------------------------------%


As already said we want to study QFT, therefore we shall quantize the classical free theory we presented in \ref{sec:classical}. In other words we shall quantize in the functional approach the alegra $\Acal_\reg(\Mcal)$.
...
%
\begin{equation*}
\Fsf \star_{\Delta^\prime_+} \Gsf = \alpha_w \left(\alpha_{-w}(\Fsf) \star_{\Delta_+} \alpha_{-w}(\Gsf)\right) \ , \ \mbox{ with } \ \alpha_{w}(\Fsf) = \exp\left(\hbar \sm{ w(x,y) , \frac{\delta}{\delta\phi(x)} \ \frac{\delta}{\delta\phi(y)} } \right) \Fsf \ .
\end{equation*}
%
...
%
\begin{equation*}
\Fcal_{\mathsf{loc}}(\Mcal) = \left\{ \Fsf(\phi) \in \Fcal_{\mu\csf}(\Mcal) \ \bigg| \ \supp\left(\Fsf(\phi)^{(n)}\right) \subset d_n = \left\{ (x,\dots,x) \subset \Mcal^n \right\} \right\} \subset \Fcal_{\mu\csf}(\Mcal) \ .
\end{equation*}
%
...
We can also view the  time ordering product as the following map
%
\begin{equation*}
\Tsf_n \ : \ 
\left\{
\begin{array}{lcl}
\Fcal_\sharp(\Mcal)[[\hbar]]^{\otimes n} & \to & \Fcal_\sharp(\Mcal)[[\hbar]] \\
\Fsf_1 \otimes \ ... \ \otimes \Fsf_n & \mapsto & \Fsf_1 \cdot_{\Tsf} \ ... \ \cdot_{\Tsf} \Fsf_n
\end{array}
\right. \ .
\end{equation*}
%
...
\end{equation*}
%
we can rewrite the time ordering product of $n$th argument as follow
%
\begin{equation}
\Tsf_n(\Fsf_1 \otimes \ ... \ \otimes \Fsf_n)(\phi) \ = \ \Tsf \ \msf_\nsf \left( \Tsf^{-1} \Fsf_1 \otimes \ ... \ \otimes \Tsf^{-1} \Fsf_n \right)(\phi) \ ,
...
%
\begin{equation*}
\Tsf \ = \ \esf^{\frac{1}{2} \Dsf} \ , \ \  \mathsf{and} \qquad \Dsf \ = \ i \hbar \sm{\Delta_\fsf , \frac{\delta^2}{\delta \phi^2}} \ .
\end{equation*}
%
We can go further and rewrite this time ordering product as
%
\begin{equation*}
\Tsf_n (\Fsf_1 \otimes \ ... \ \otimes \Fsf_n)(\phi) \ = \ \msf_\nsf \circ \Tcal_n \bigg( \Fsf_1(\phi_1) \otimes \ ... \ \otimes \Fsf_n(\phi_n) \bigg) \bigg|_{\phi_1 = ... = \phi_n = \phi} \ ,
\end{equation*}
%
with 
%
\begin{equation*}
 \Tcal_n \ = \ \exp\bigg(\frac{1}{2} \sum_{i<j}^{n} \Dsf_{ij}\bigg) \ , \qquad \mathsf{and} \quad \Dsf_{ij} \ = \ 
 \sm{i \hbar \ \Delta_\fsf \ , \ \frac{\delta^2 \hfil}{\delta \phi_i \ \delta \phi_j}} \ .
\end{equation*}
%
We shall now introduce a graphical way of working with this. Let $\Gcal_n$ be the set of graph, with $n=V(\Gcal)$ vertices and $E(\Gcal)$ edges. We will consider only the subgraphs $\gamma$ of $\Gcal_n$ such that
%
\begin{equation*}
V(\gamma) \ = \ \left\{ 1, ... , k \right\} \ , \quad \mathsf{and} \quad E(\gamma) \ = \ \bigg\{ l_{ij} \ \bigg| \  i \in V(\gamma) , \ i\neq j \in \partial l_{ij} \bigg\}
\end{equation*}
%
with $k\leq n$, and $\partial l_{ij} = \{i,j\}$. We will denote by $\abs{E(\gamma)}$ and $\abs{V(\gamma)}$ respectively the number of elements in the set $E(\gamma)$ and $V(\gamma)$. Therefore the time ordered product can be written as follow
%
\begin{equation}
\Tcal_n \ = \ \sum_{\gamma \in \Gcal_n} \Tsf_\gamma \ , \qquad \mbox{with} \quad \Tsf_\gamma \ = \ \frac{1}{\Nsf(\gamma)} \ \sm{\tsf_\gamma \ , \ \delta_\gamma}
...
%
\begin{equation*}
\Nsf(\gamma) \ = \ \abs{\Esf(\gamma)} ! \ , 
\qquad \tsf_\gamma = (i\hbar)^{\abs{\Esf(\gamma)}} \prod_{\substack{i < j, \\ i \in V(\gamma), \\ j \in \partial l_{ij}}} \Delta_\fsf(x_i,x_j) \ , 
\qquad \delta_\gamma = \frac{\delta^{2\abs{E(\gamma)}}\hfill}{\underset{\substack{i \in V(\gamma), \\ j \in \partial l_{ij}}}{\prod} \delta \phi_i^{\abs{l_{ij}}}} \ ,
\end{equation*}
%
where $\phi_i \doteq \phi(x_i)$ and $\abs{l_{ij}}$, for $i$ fixed, is the number of edges starting from the vertex $i$. We thus have a graphical interpretation of the time oredering product.
...
&& R_v(\phi)=\phi-i V\star \phi+i V\cdot_T\phi+\frac12\left( V\cdot_T V\right)\star \phi-V\star V\star\phi-\frac12 V\cdot_T V\cdot_T \phi+V\star(V\cdot_T\phi)+O(\lambda^3)\,.
\end{eqnarray*}
%
...
We have build successfully an interacting quantum field theory perturbatively. We had to consider the Bogoliubov formula \eqref{eq:bogoliubov} to build it, which use time ordered products. 


\bigskip


These time ordered products can be ill defined. It is what we call the regularization problem. It can be solved using the Epstein Glaser procedure \cite{brunetti_microlocal_2000}, where the time ordered product is build recursively. 


\bigskip


Let us present this recursive construction of the time ordered product. In order to be able to do we shall need a partition of the configuration space minus the total diagonal $\Mcal^n \backslash d_n$.


\bigskip

Let $\Jcal$ be the set of all non empty proper subset $I$ of $\{1,\dots,n\}$, and $I^c$ the complement set of $I$ in $\{1,\dots,n\}$. We define the subset $\Ccal_I$ of $\Mcal^n$ as follow
%
\begin{equation*}
...
%
\begin{equation*}
\bigcup_{I \in \Jcal} \Ccal_I = \Mcal^n \backslash d_n \ .
\end{equation*}
%
We consider the time ordered operator defined in \eqref{eq:time_ordered_op}. We impose 
%
\begin{equation*}
\Tsf_0(\Ibb)=\Ibb \ \mbox{ and } \ \ \Tsf_1(\Fsf)=\Fsf
\end{equation*}
%
and for $1 \leq k \leq n-1$, we assume the time ordered product $\Tsf_k$ has been constructed. Using the causal factorisation property we have
%
\begin{equation*}
\Tsf_k(\Fsf_1 \otimes \cdots \otimes \Fsf_k) = \Tsf_r(\Fsf_1 \otimes \cdots \otimes \Fsf_r) \star \Tsf_{k-r}(\Fsf_{r+1} \otimes \cdots \otimes \Fsf_k) \ ,
\end{equation*}
%
for $\Fsf_1, \dots , \Fsf_r$ lying in the past of $\Fsf_{r+1}, \dots , \Fsf_k$. We would like now to build $\Tsf_n$ on $\Mcal^n \backslash d_n$. Then for any set $x \in \Ccal_I$ we have
%
\begin{equation*}
\Tsf_I = \Tsf^I \star \Tsf^{I^c} \ , \ \mbox{ using the notation } \ \Tsf^I = \Tsf\left(\bigotimes_{i \in I} \Fsf_i \right) \ .
\end{equation*}
%
Accordind to the induction hypothesis $\Tsf_I$ is well defined on $\Ccal_I$.  Two set $\Ccal_{I_1}$ and $\Ccal_{I_2}$ can overlap. But we can show that for any set $I_1 , I_2 \in \Jcal$ such that $\Ccal_{I_1} \cap \Ccal_{I_2} \neq \emptyset$ we have the restriction on $\Ccal_{I_1} \cap \Ccal_{I_2}$ of $\Tsf_{I_1}$ and $\Tsf_{I_2}$ coincide. Therefore if we define a smooth partition of unity $\{ g_I\}$  subordinate to $\{\Ccal_I\}$. Then
%
\begin{equation*}
\Tsf_n =\sum_{I\in\Jcal} g_I \Tsf_I \ .
\end{equation*}
%
is well defined on the full space up to the total diagonal $\Mcal^n \backslash d_n$. At each recursion step the causal factorisation property permits to construct the distributions defining the time ordered product up to the total diagonal. 


\bigskip


This procedure, knwon as the Epstein Glaser regularisation, is theoretical clear but quite difficult to implement in practise. The aim of the present work is to discuss a renormalisation scheme which is suitable for practical computations.
...
To extend to the full space we shall present the situation on Minkowski spacetime $\Mbb$.
...
We have the \textbf{Steinmann scaling degree}\index{Steinmann scaling degree} at a point of a distribution $u$ with with respect to the origin. It is denoted $\sd(u)$ and defined as
%
\begin{equation*}
\sd(u) \ \doteq \ \inf\bigg\{ \ \omega \in \Rbb \ \bigg| \ \lim_{\rho \downarrow 0} \ \rho^\omega u_\rho \ = \ 0, \ \mathsf{in \ the \ sense \ of \ } \Dcal^\prime(\Mbb) \bigg\} \ ,
\end{equation*}
%
with 
%
\begin{eqnarray*}
u_\rho(\phi) &=& \sm{u_\rho , \phi} \ = \ \int_{\Rbb^n} \dsf^n x \ u(\rho x) \ \phi(x) \ = \ \rho^{-d} \ \sm{u , \phi_{\rho^{-1}}} \ = \ u(\phi_{\rho^{-1}})
\end{eqnarray*}
...
In order to discuss the analytic regularisation of time ordered products, we employ the notation introduced in \eqref{eq:time_ordered_prod_graph}. We recall we have 
%
\begin{equation*}
\Tcal_n \ = \ \sum_{\gamma \in \Gcal_n} \Tsf_\gamma \ , \qquad \mbox{with} \quad \Tsf_\gamma \ = \ \frac{1}{\Nsf(\gamma)} \ \sm{\tsf_\gamma \ , \ \delta_\gamma}
\end{equation*}
...
Moreover, $\tsf_\Gamma$  is a distribution which is well--defined outside of all partial diagonals, namely on
...
The correct recursion procedure is implemented by the so called Epstein Glaser forest formula, which is a position space analogue of the Zimmermann forest formula, see  (Hollands:2010pr, Keller, dfkr) for a careful analysis of the subject. We shall here follow the treatment discussed in cite(dfkr). To this end, we consider the set of indices $\overline{n} := \{1,\dots , n\}$ and define a forest $F$ as 
\begin{equation*}
...
%----------------------------------------------------------------------------%
\section{Regularisation of the Feynman propagator on curved spacetimes}
%----------------------------------------------------------------------------%
...
Let us illustrate the graphical way that we just introduced.

{ex}
 We consider only the $\Ssf$ matrix up to third oreder in the coupling constant $\lambda$.
 \begin{equation*}
 \Ssf(F) \ = \ 1 \ + \ \frac{i}{\hbar} \Tsf_1(F)(\phi) \ - \ \frac{1}{\hbar^2} \Tsf_2(F \otimes F)(\phi) \ - \ \frac{i}{\hbar^3} \Tsf_3(F \otimes F \otimes F)(\phi) \ + \ \Ocal(\lambda^4) 
 \end{equation*}
 Now we write , using our graphical procedure, the time ordering products
% \begin{equation*}
%  \Tsf_2(F \otimes F)(\phi) \ = \ \twoEzeroL \quad + \quad \hbar \quad \twoEoneL \quad + \quad \frac{\hbar^2}{2} \quad \twoEtwoL \quad + \quad \Ocal(\hbar^3)
% \end{equation*}
% \begin{equation*}
%  \begin{array}{c}
%   \Tsf_3(F \otimes F \otimes F)(\phi) \ = \ \threeEzeroL  \quad + \quad \hbar \quad \bigg( \quad \threeEoneLa \quad + \quad \threeEoneLb \quad + \quad \threeEoneLc \bigg) \\
%   + \quad \hbar^2 \quad \Bigg\{ \threeEtwoLa \quad + \quad \threeEtwoLb \quad + \quad \threeEtwoLc \\ 
%   + \quad \frac{1}{2} \quad \bigg( \threeEtwoLd \quad + \quad \threeEtwoLe \quad + \quad \threeEtwoLf \bigg) \Bigg\} \quad + \quad \Ocal(\hbar^3)
%  \end{array} 
% \end{equation*}
 It is interesting to notice that we only have to regularize the graphs with more than one edge between two same vertices. Because in this case se have to deal with powers of the Feynman Hadamard propagator, which is ill defined. 


We know functionals which are local have their supports on the small diagonal, it implies we have to extend the distribution to the small diagonal $\Delta_n$. For now $\tsf_\gamma$ is well defined on $\Dcal^\prime\left((\Mcal \backslash \Delta_n)^{\abs{E(\gamma)}}\right)$, but we would like to extend it to $\Dcal^\prime(\Mcal^{\abs{E(\gamma}})$. \par
...
In terms of $\Ssf$ we can define the following linear map, which descibes the transition from the free action to the action with additional interaction term $V$. 
\begin{equation*}
 \Rsf_\vsf(F) \doteq \Ssf(V)^{-1} \star \left( \Ssf(V) \ ._\Tsf \ F \right) \ .
\end{equation*}
