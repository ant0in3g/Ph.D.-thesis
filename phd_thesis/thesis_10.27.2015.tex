...
We call the evaluation of $\Asf$ in $\Acal$ on a state $\omega$ the \textbf{expectation value} of $\Asf$ in the given state $\omega$, and we denote it by $\omega(\Asf)$. \com{The expectation value of an observable represent the mean values of all possible outcomes of an experiment which test that observable on the state $\omega$.  }

\com{Antoine, here you are using the state before defining it. I think you should revert the order of the two definitions. 1) the state (Definition 2.7) 2) the notion of expectation values}
...
\com{The $\ast$--algebras we are considering in this work represent observables of a scalar field theory. In this case, a state $\omega$ can be completely determined by its $n$--point (\textbf{correlation}) functions $\omega_n \in \Dcal'(\Mcal^n)$ i.e. the expectation values of the product of $n$ linear functionals, namely, for every $\Asf\in\Fcal_\reg$  
\begin{equation*}
\omega( \Asf ) := \sum_{n} \frac{1}{n!}  \left\langle \omega_n, \Asf^{(n)}(0)   \right\rangle \ , 
\end{equation*}
}

\sbar{We shall specify the algebras we shall consider. For now on the algebra $\Acal$ shall denote the $\ast$--algebra of observables associated to a field theory.
...
%
\sbar{
where the $\star$ product is the ``quantum'' product that we shall implement in section \ref{p:Q_DEFORM} in order to implement the quantum structure to the field theory that we shall construct. 
}
We can impose conditions on the \sbar{corrlation} \com{correlation} function of a state in order to have states with particular physical properties.
\com{This part of the discussion is done for state over a free quantum theory. We shall be more precise below, for the moment we have just to remember that the $n-$point function of a free theory satisfy weakly some linear hyperbolic equation of motion.}
