...
Let $\Xsf$ be a set. A topology on $\Xsf$ is a collection $\Tcal$ of subsets satisfying the three axioms%
%
\begin{itemize}
...
The pair $(\Xsf,\Tcal)$ is called a topological space. We shall often omit to precise the topology $\Tcal$, and simply say that $\Xsf$ is a topological space. %
%
One says $\Xsf$ is \textbf{second countable}\footnote{A set is said to be \textbf{countable} it there exists a one to one correspondence between the considered set and the set of integers $\Nbb$.} if it admits a countable topological basis ; \textbf{Hausdorff} if any two points have disjoint neighborhoods ; separable if it has a dense and countable subset ; \textbf{locally compact} if it is Hausdorff and if every point admit a compact neighborhood ; \textbf{$\sigma$ compact} if it can be written as the union of countably compact subsets ; and \textbf{paracompact} if every open cover has a refinement by an open covering that is locally finite.

Let us give some example to illustrate this definition.

\begin{example}
$\Xsf = \{\emptyset , \Xsf \}$ is a topology, called trivial topology. \\
If $\Xsf$ contains all open subsets, then $\Xsf$ is the discrete topology. \\
(blablabla)
\end{example}
...
On a set one can also define the notion of a metric, which make sense of distance between every pair of points in the set we work with.

\begin{definition}[Metric space]
Let $\Ysf$ be a set. A metric on $\Esf$ is a map $d : \Ysf \times \Ysf \to \Rbb_+$ such that for all $x, y, z \in \Ysf$,%
%
\begin{itemize}
\item \textbf{A4 - separation} : $d(x,y) = 0 \Leftrightarrow x=y$ ; 
\item \textbf{A5 - symetry} : $d(x,y) = d(y,x)$ ;
\item \textbf{A6 - triangle inequality} : $d(x,y) \leq d(x,z) + d(z,y)$ .
\end{itemize}
%
A set $\Ysf$ with a metric $d$ is called a metric space.
%
\end{definition}

And then it is possible to implement the topological structure on generic metric spaces.

\begin{proposition}[Induced topology by a metric]
The collection $\Tcal$ of subsets on $\Xsf$ such that 
%
\begin{equation*}
\forall x \in U , \ \exists \epsilon > 0, \ B(x,\epsilon) = \left\{ y \in U , d(x,y) < \epsilon \right\}\subset U
\end{equation*}
is a topology on $\Xsf$, called the induced topology by the metric $d$.
%
\end{proposition}

A topological space $(\Xsf,\Tcal)$ is said to be \textbf{metrizable} if there is a metric on $\Xsf$ for which the induced topology is $\Tcal$. A \textbf{base} $\Bcal$ of a topological space $(\Xsf,\Tcal)$ is a collection of open sets in $\Tcal$ such that every open subsets of $\Xsf$ can be written as  finite union of elements of the base%
%
\begin{equation*}
\forall U \in \Tcal, \ \forall x \in U, \ \exists V \in \Bcal , \mbox{ such that } x \in V \subset U . 
\end{equation*}
%
Therefore on says that a topological space is 

A subset $A$ of a topological space $\Xsf$ is called \textbf{dense} in $\Xsf$ if any point $x$ in $\Xsf$ belongs to $A$ or is a limit point \footnote{A limit point $p$ of $A$ is a point such that every open set around it contains at least one point of $A$ different from $p$.} of $A$. Equivalently, $A$ is dense in $\Xsf$ if and only if the only closed subset of $\Xsf$ containing A is $\Xsf$ itself. 

\begin{proposition}
A metrizable space $\Xsf$ is separable if and only if it is secound countable
\end{proposition}

$\Xsf$ is connected if it cannot be written as the union of two disjoint open sets. 

\begin{proposition}
Every metrizable topological space is Hausdorff.
\end{proposition}

We shall now give an important lemma which gives us the possibility to introduce more mathematical structure.

\begin{lemma}
Let $\Xsf$ be a topological spce in which all points admit a metrizable meighborhood. Then the following two properties are equivalent
%
\begin{itemize}
...
We shall now see how what we introduced until now is linked to the notion of manifold.

\begin{definition}[Topological manifold]
A topological manifold of dimension $n$ is a metrizable and separable topological space in which every points admit an open neighborhood homeomorphic to a subset of $\Rbb$.
\end{definition}
...
For later we introduce the notion of continous maps and homeomorphism between topological spaces.
...
%----------------------------------------------------------------------------%
%\chapter{An analytic regularisation scheme}
%----------------------------------------------------------------------------%
