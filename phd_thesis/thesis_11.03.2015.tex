...
\begin{eqnarray}
Z_{f;\varphi}(J) 
&=& \int \Dcal A \ \exp\bigg( - \left( S_2(\varphi) + \tr_j\left(\varphi_\mu^j \ J^j_\mu \right) \right) \bigg) \nonumber \\
&=& \exp\left( \frac18 \sum \ (\widetilde{J}_i)^{j_1}_{mn} \ P^{j_1 j_2}_{mn;kl} \ (\widetilde{J}_i)^{j_2}_{kl} \right) \label{eq:functional_free_gauge}, \\
%
...
%
\begin{equation*}
A^j_{mn} = A^{\prime j}_{mn} - \frac14 P^{jj}_{nm;kl} \widetilde J^j_{kl} \ , \quad
...
\end{equation}
in view of \eqref{functionalconnected}, by further expanding the functional logarithm contribution in the RHS of \eqref{formalW} in which $e^{-S_{int}}$ is understood as a formal series in $S_{int}$ as usual. In the following, the expression for the terms contributing to our one-loop analysis reduces to
\begin{equation}
...
We now use the perturbative framework to compute the tadpole one-point function for the gauge potential. It receives contribution from ghost and gauge potential loops corresponding respectively to the ghost-gauge vertex and the trilinear vertex in the gauge-fixed action \eqref{gaugefixfinal}. The computation is a bit cumbersome as the tadpole contributions for each $\varphi_\mu$ must be considered separately. 


We start with the computation of the tadpole with external $\varphi$, and first consider the corresponding ghost loop contribution. The relevant ghost-gauge part of the action \eqref{gaugefixfinal} can be written as
%
\begin{equation}
S_{int}^{\varphi_3\phi\pi}=\frac{k}{\lambda}\sum_{j,m,n,p}(j+1) (m-n)\bar C^j_{mn}((\varphi_3)^j_{np}C^j_{pm}-C^j_{np}(\varphi_3)^j_{pm})\label{sintghost}
\end{equation}
%
...
\end{equation}
%
for which each of the two terms in the sum over $i$ will contribute equally to the 1-point function. The 1-point function with external $A_3$ is then given by
\begin{equation}
...
Next, eqn. \eqref{renor-wavefunct1} shows that some but not all the terms linear in $A_i$, $\sim\widetilde\tr(\eta_i A_i)$, that were absent from the classical action by construction are restored at one-loop order. Only $A_3$ has vanishing tadpole function. 
...
In order to make contact with some notations of ref. \cite{Wallet:2007c}, we set%
%
\begin{equation}
