...
%\draw[fill=green!40!white] (-4,4) -- (4,4) -- (2,0) -- (-2,0) -- cycle;
%\draw[fill=red!40!white] (-4,-4) -- (4,-4) -- (2,0) -- (-2,0) -- cycle;
%\draw [fill=green!20!white,opacity=1] (0,4) ellipse (4cm and 0.6cm);
%\draw [fill=black!20!white,opacity=1,dashed] (0,0) ellipse (2cm and 0.4cm);
%\draw [fill=red!20!white,opacity=1,dashed] (0,-4) ellipse (4cm and 0.6cm);
%\end{tikzpicture} 
%\caption{Supports.}
...
%
We now define the causal propagator \index{Causal propagator} of P. We will denote it by $\Delta$ \index{$\Delta$}. It is the difference between the advanced and retarded fundamental slutions of $P$. And as $\Gsf_{\mathsf{r}}$ or $\Gsf_{\mathsf{a}}$ it is still a map from $\Dcal(\Mcal)$ to $\Ecal(\Mcal)$.
%
\begin{eqnarray}
...
on which live objects called scalar field $\phi$. These scalar fileds $\phi$ are maps from $\Mcal$ to $\Rbb$, they are the states in field theory. Let's recall an example presented in the thesis of Thomas %\cite[Sect. II.1.3, p. 56]{ThomasPhD}
...
The above result show us the link between test function and states $\phi$, elements of $\Scal(\Mcal)$. Let's come back to the example of Thomas %\cite[Sect. II.1.3, p. 56]{ThomasPhD} 
...
\begin{lemma} %\cite[Lem. II.1.2.3, p.54]{ThomasPhD} \\
