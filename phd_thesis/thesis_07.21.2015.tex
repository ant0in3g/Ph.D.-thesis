...
\begin{itemize}
\item !!!! CONNECTTION !!!!
\item !!!! GEODESIC $\to$ SYNGE'S WORLD FUNCTION !!!!
\item !!!! CURVATURE !!!!
\end{itemize}
...
We assing to our spacetime the configuration space $\Crak(\Mcal)$ of fileds defined on it. $\Crak(\Mcal)$ can be defined as the space of sections of some vector bundle over $\Mcal$, i.e. $\Crak(\Mcal) = \Gamma(\Mcal)$. Moreover  we shall work only with real scalar  fields, it means the vector bundle chosen is the real line bundle. Therefore the configuration is the space of real valued smooth maps. We shall for now on denote $\Crak(\Mcal)$ using L. Schwartz's notation $\Ecal(\Mcal)$. And in the case of compactly supported fields, we shall denote the corresponding space by $\Dcal(\Mcal)$.
